...
uint8 USBUART_HandleStandardRqst(void) 
{
    ...
    uint32 bmRequestType = USBUART_bmRequestTypeReg;
...
void USBUART_ConfigReg(void) 
{
    ...
    USBUART_ARB_CFG_REG |= (uint32)  USBUART_ARB_CFG_CFG_CMP;
    ...
    USBUART_SIE_EP_INT_EN_REG = (uint32) USBUART_DEFAULT_SIE_EP_INT_EN;
}
    ...
    if (USBUART_configuration > 0u)
    {
        ...
        */
        ...
        for (i = 0u; i < ep; i++)
        {
            ...
            {
                curEp  = ((uint32) pEP->addr & USBUART_DIR_UNUSED);
                epType = ((uint32) pEP->attributes & USBUART_EP_TYPE_MASK);
            ...
            #if (USBUART_EP_MANAGEMENT_DMA_AUTO)
                /* Clear IN data ready. */
                USBUART_ARB_EP_BASE.arbEp[curEp].epCfg &= (uint32) ~USBUART_ARB_EPX_CFG_IN_DATA_RDY;

                /* Select endpoint number of reconfiguration */
                USBUART_DYN_RECONFIG_REG = (uint32) ((curEp - 1u) << USBUART_DYN_RECONFIG_EP_SHIFT);
                ...
                if (0u != (pEP->addr & USBUART_DIR_IN))
                {
                    ...
                    USBUART_EP_TYPE_REG &= (uint32) ~(uint32)((uint32) 0x01u << (curEp - 1u));
                ...
                {
                    /* Set endpoint type: 0 - IN and 1- OUT. */
                    USBUART_EP_TYPE_REG |= (uint32) ((uint32) 0x01u << (curEp - 1u));
                ...
                */
                USBUART_DYN_RECONFIG_REG &= (uint32) ~USBUART_DYN_RECONFIG_ENABLE;
...
uint8 USBUART_ClearEndpointHalt(void) 
{
    ...
    if ((ep > USBUART_EP0) && (ep < USBUART_MAX_EP))
    {
        ...
        USBUART_SIE_EP_BASE.sieEp[ep].epCnt0 &= (uint32) ~(uint32)USBUART_EPX_CNT_DATA_TOGGLE;
