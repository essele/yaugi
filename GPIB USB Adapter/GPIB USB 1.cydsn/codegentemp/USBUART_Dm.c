...
#include "USBUART_Dm.h"
    ...
    #define USBUART_Dm_SetP4PinDriveMode(shift, mode)  \
    do { \
        USBUART_Dm_PC =   (USBUART_Dm_PC & \
                                (uint32)(~(uint32)(USBUART_Dm_DRIVE_MODE_IND_MASK << \
                                (USBUART_Dm_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (USBUART_Dm_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define USBUART_Dm_SetP4PinDriveMode(shift, mode)  \
        do { \
            USBUART_Dm_USBIO_CTRL_REG = (USBUART_Dm_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(USBUART_Dm_DRIVE_MODE_IND_MASK << \
                                    (USBUART_Dm_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (USBUART_Dm_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		USBUART_Dm_SetP4PinDriveMode(USBUART_Dm__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(USBUART_Dm_DR & (uint8)(~USBUART_Dm_MASK));
    drVal = (drVal | ((uint8)(value << USBUART_Dm_SHIFT) & USBUART_Dm_MASK));
    USBUART_Dm_DR = (uint32)drVal;
}
...
{
    return (uint8)((USBUART_Dm_PS & USBUART_Dm_MASK) >> USBUART_Dm_SHIFT);
}
...
{
    return (uint8)((USBUART_Dm_DR & USBUART_Dm_MASK) >> USBUART_Dm_SHIFT);
}
...
void USBUART_Dm_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  USBUART_Dm_INTCFG & (uint32)(~(uint32)position);
    USBUART_Dm_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(USBUART_Dm_INTSTAT & USBUART_Dm_MASK);
	USBUART_Dm_INTSTAT = maskedStatus;
    return maskedStatus >> USBUART_Dm_SHIFT;
}
