    ...
    if ((tempReg & USBUART_MODE_ACKD) != 0u)
    {
        ...
        if (modifyReg != 0u)
        {
            ...
            if ((tempReg & USBUART_MODE_SETUP_RCVD) == 0u)
            {
                ...
                tempReg = (uint32) USBUART_ep0Toggle | USBUART_ep0Count;
...
void USBUART_HandleSetup(void) 
{
    ...
    */
    ...
    USBUART_EP0_CR_REG = (uint32) requestHandled;
    requestHandled = (uint8) USBUART_EP0_CR_REG;
...
void USBUART_LoadEP0(void) 
{
    ...
    {
        USBUART_EP0_DR_BASE.epData[ep0Count] = (uint32) *USBUART_currentTD.pData;
...
void USBUART_NoDataControlStatusStage(void) 
{
    if (0u != USBUART_deviceAddress)
    {
        ...
        USBUART_CR0_REG = (uint32) USBUART_deviceAddress | USBUART_CR0_ENABLE;
