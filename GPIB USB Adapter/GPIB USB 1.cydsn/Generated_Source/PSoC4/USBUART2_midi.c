...
#if (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF)
    /* The Size of the MIDI messages (MIDI Table 4-1) */
    static const uint8 CYCODE USBUART2_MIDI_SIZE[] = {
...
#endif  /* USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF */
...
***************************************/
...
#if (USBUART2_MIDI_IN_BUFF_SIZE > 0)
    #if (USBUART2_MIDI_IN_BUFF_SIZE >= 256)
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 USBUART2_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 USBUART2_midiInPointer;                             /* Input endpoint buffer pointer */
    #endif /* (USBUART2_MIDI_IN_BUFF_SIZE >= 256) */
    ...
    volatile uint8 USBUART2_midi_in_ep;
    ...
    uint8 USBUART2_midiInBuffer[USBUART2_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
#endif /* (USBUART2_MIDI_IN_BUFF_SIZE > 0) */

#if (USBUART2_MIDI_OUT_BUFF_SIZE > 0)
    ...
    volatile uint8 USBUART2_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 USBUART2_midiOutBuffer[USBUART2_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (USBUART2_MIDI_OUT_BUFF_SIZE > 0) */

#if (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF)

    static USBUART2_MIDI_RX_STATUS USBUART2_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 USBUART2_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 USBUART2_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF)
        static USBUART2_MIDI_RX_STATUS USBUART2_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 USBUART2_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref USBUART2_MIDI1_InqFlags*/
        volatile uint8 USBUART2_MIDI2_InqFlags;                             /* Device inquiry flag */
    #endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF) */
#endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF) */
...
#if (USBUART2_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: USBUART2_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBUART2_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. USBUART2_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref USBUART2_midiOutBuffer: This buffer is used by the
*    USBUART2_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref USBUART2_callbackLocalMidiEvent function.
*
*  \ref USBUART2_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref USBUART2_midi_in_ep: Used as an IN endpoint number.
*
*   \ref USBUART2_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void USBUART2_MIDI_Init(void) 
{
   ...
   USBUART2_midiInPointer = 0u;
        ...
        USBUART2_LoadInEP(USBUART2_midi_in_ep, USBUART2_midiInBuffer,
                                                               USBUART2_MIDI_IN_BUFF_SIZE);
    #endif  /* (USBUART2_MIDI_IN_BUFF_SIZE > 0) */

    #if (USBUART2_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)USBUART2_ReadOutEP(USBUART2_midi_out_ep, USBUART2_midiOutBuffer,
                                                                       USBUART2_MIDI_OUT_BUFF_SIZE);
    #endif /* (USBUART2_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO) */

#if (USBUART2_MIDI_OUT_BUFF_SIZE > 0)
    USBUART2_EnableOutEP(USBUART2_midi_out_ep);
#endif /* (USBUART2_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF)
    USBUART2_MIDI_InitInterface();
#endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF) */
}
...
#if (USBUART2_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBUART2_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref USBUART2_midiOutBuffer: Used as temporary buffer between USB
    ...
    *
    *******************************************************************************/
    void USBUART2_MIDI_OUT_Service(void) 
    {
    #if (USBUART2_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (USBUART2_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (USBUART2_OUT_BUFFER_FULL == USBUART2_GetEPState(USBUART2_midi_out_ep))
        {
        #if (USBUART2_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = USBUART2_GetEPCount(USBUART2_midi_out_ep);
        #else
            outLength = (uint8)USBUART2_GetEPCount(USBUART2_midi_out_ep);
        #endif /* (USBUART2_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!USBUART2_EP_MANAGEMENT_DMA_AUTO)
            #if (USBUART2_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = USBUART2_ReadOutEP(USBUART2_midi_out_ep,
                                                       USBUART2_midiOutBuffer, outLength);
            #else
                outLength = (uint8)USBUART2_ReadOutEP(USBUART2_midi_out_ep,
                                                              USBUART2_midiOutBuffer, (uint16) outLength);
            #endif /* (USBUART2_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (USBUART2_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (USBUART2_OUT_BUFFER_FULL == USBUART2_GetEPState(USBUART2_midi_out_ep))
                {
                }
            ...
            if (outLength >= USBUART2_EVENT_LENGTH)
            {
                ...
                #if (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF)
                    if ((USBUART2_midiOutBuffer[outPointer] & USBUART2_CABLE_MASK) ==
                         USBUART2_MIDI_CABLE_00)
                    {
                        USBUART2_MIDI1_ProcessUsbOut(&USBUART2_midiOutBuffer[outPointer]);
                    }
                    else if ((USBUART2_midiOutBuffer[outPointer] & USBUART2_CABLE_MASK) ==
                             USBUART2_MIDI_CABLE_01)
                    {
                    #if (USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF)
                         USBUART2_MIDI2_ProcessUsbOut(&USBUART2_midiOutBuffer[outPointer]);
                    #endif /*  USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef USBUART2_MIDI_OUT_EP_SERVICE_CALLBACK
                            USBUART2_MIDI_OUT_EP_Service_Callback();
                        #endif /* USBUART2_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    USBUART2_callbackLocalMidiEvent(USBUART2_midiOutBuffer[outPointer] & USBUART2_CABLE_MASK,
                                                            &USBUART2_midiOutBuffer[outPointer + USBUART2_EVENT_BYTE1]);
                    outPointer += USBUART2_EVENT_LENGTH;
                }
            }
...
#if (USBUART2_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBUART2_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  USBUART2_MIDI_IN_Service() and from the
    *  USBUART2_PutUsbMidiIn() function.
    *
    * \globalvars
    *  USBUART2_midi_in_ep: Used as an IN endpoint number.
    *  USBUART2_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   USBUART2_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBUART2_MIDI_IN_EP_Service(void) 
    {
        ...
        if (USBUART2_midiInPointer != 0u)
        {
            if(USBUART2_GetEPState(USBUART2_midi_in_ep) == USBUART2_EVENT_PENDING)
            {
            #if (USBUART2_EP_MANAGEMENT_DMA_AUTO)
                USBUART2_LoadInEP(USBUART2_midi_in_ep, NULL, (uint16)USBUART2_midiInPointer);
            #else
                USBUART2_LoadInEP(USBUART2_midi_in_ep, USBUART2_midiInBuffer,
                                                              (uint16) USBUART2_midiInPointer);
            #endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *
    *******************************************************************************/
    void USBUART2_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == USBUART2_midiInPointer)
        {
            do
            {
                if (USBUART2_midiInPointer <= (USBUART2_MIDI_IN_BUFF_SIZE - USBUART2_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = USBUART2_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        USBUART2_PrepareInBuffer(m1, (uint8 *)&USBUART2_MIDI1_Event.msgBuff[0],
                                                                       USBUART2_MIDI1_Event.size, USBUART2_MIDI_CABLE_00);
                    }
                }
                ...
                if (USBUART2_midiInPointer <= (USBUART2_MIDI_IN_BUFF_SIZE - USBUART2_EVENT_LENGTH))
                {
                    ...
                    m2 = USBUART2_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        USBUART2_PrepareInBuffer(m2, (uint8 *)&USBUART2_MIDI2_Event.msgBuff[0],
                                                                       USBUART2_MIDI2_Event.size, USBUART2_MIDI_CABLE_01);
                    }
                }
            #endif /*  USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF */
            }
            while((USBUART2_midiInPointer <= (USBUART2_MIDI_IN_BUFF_SIZE - USBUART2_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        USBUART2_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: USBUART2_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The USBUART2_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  USBUART2_TRUE  | Host is not ready to receive this message
    *  USBUART2_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref USBUART2_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref USBUART2_midiInPointer: Checked this variable to see if
    ...
    *
    ***************************************************************************/
    uint8 USBUART2_PutUsbMidiIn(uint8 ic, const uint8 midiMsg[], uint8 cable)
                                                                
    {
        uint8 retError = USBUART2_FALSE;
        ...
        if (USBUART2_midiInPointer >
                    (USBUART2_EP[USBUART2_midi_in_ep].bufferSize - USBUART2_EVENT_LENGTH))
        {
            USBUART2_MIDI_IN_EP_Service();
        }
        ...
        if (USBUART2_midiInPointer <=
                    (USBUART2_EP[USBUART2_midi_in_ep].bufferSize - USBUART2_EVENT_LENGTH))
        {
            if((ic < USBUART2_EVENT_LENGTH) || (ic >= USBUART2_MIDI_STATUS_MASK))
            {
                USBUART2_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else /* Only SysEx message is greater than 4 bytes */
            {
                ...
                do
                {
                    ...
                    USBUART2_PrepareInBuffer(USBUART2_MIDI_SYSEX, &midiMsg[idx],
                                                     USBUART2_EVENT_BYTE3, cable);

                    /* Move to next 3 bytes of message */
                    ic  -= USBUART2_EVENT_BYTE3;
                    idx += USBUART2_EVENT_BYTE3;
                        ...
                        (USBUART2_EP[USBUART2_midi_in_ep].bufferSize - USBUART2_EVENT_LENGTH))
                    {
                           ...
                           (USBUART2_EP[USBUART2_midi_in_ep].bufferSize - USBUART2_EVENT_LENGTH))
                        {
                            ...
                            retError = USBUART2_TRUE;
                            break;
                        }
                    }
                }
                while (ic >= USBUART2_EVENT_BYTE3);

                if (retError == USBUART2_FALSE)
                {
                    /* Handle end of message: valid size of messages is 0, 1 and 2 */
                    USBUART2_PrepareInBuffer(USBUART2_MIDI_EOSEX, &midiMsg[idx], ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: USBUART2_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBUART2_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    USBUART2_PutUsbMidiIn() function messages.
    *  USBUART2_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBUART2_PrepareInBuffer(uint8 ic, const uint8 srcBuff[], uint8 eventLen, uint8 cable)
                                                                 
    {
        ...
        if (ic >= (USBUART2_MIDI_STATUS_MASK | USBUART2_MIDI_SINGLE_BYTE_MASK))
        {
            USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_SINGLE_BYTE | cable;
            USBUART2_midiInPointer++;
            USBUART2_midiInBuffer[USBUART2_midiInPointer] = ic;
            USBUART2_midiInPointer++;
            USBUART2_midiInBuffer[USBUART2_midiInPointer] = 0u;
            USBUART2_midiInPointer++;
            USBUART2_midiInBuffer[USBUART2_midiInPointer] = 0u;
            USBUART2_midiInPointer++;
        }
        else if((ic < USBUART2_EVENT_LENGTH) || (ic == USBUART2_MIDI_SYSEX))
        {
            if(ic == USBUART2_MIDI_SYSEX)
            {
                USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_SYSEX | cable;
                USBUART2_midiInPointer++;
            }
            else if (srcBuffZero < USBUART2_MIDI_SYSEX)
            {
                USBUART2_midiInBuffer[USBUART2_midiInPointer] = (srcBuffZero >> 4u) | cable;
                USBUART2_midiInPointer++;
            }
            else if (srcBuffZero == USBUART2_MIDI_TUNEREQ)
            {
                USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_1BYTE_COMMON | cable;
                USBUART2_midiInPointer++;
            }
            else if ((srcBuffZero == USBUART2_MIDI_QFM) || (srcBuffZero == USBUART2_MIDI_SONGSEL))
            {
                USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_2BYTE_COMMON | cable;
                USBUART2_midiInPointer++;
            }
            else if (srcBuffZero == USBUART2_MIDI_SPP)
            {
                USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_3BYTE_COMMON | cable;
                USBUART2_midiInPointer++;
            }
            else
            {
            ...
            USBUART2_midiInBuffer[USBUART2_midiInPointer] = srcBuffZero;
            USBUART2_midiInPointer++;
            USBUART2_midiInBuffer[USBUART2_midiInPointer] = srcBuffOne;
            USBUART2_midiInPointer++;
            USBUART2_midiInBuffer[USBUART2_midiInPointer] = srcBuff[2u];
            USBUART2_midiInPointer++;
        }
        else if (ic == USBUART2_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_SYSEX_ENDS_WITH1 | cable;
                    USBUART2_midiInPointer++;
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_MIDI_EOSEX;
                    USBUART2_midiInPointer++;
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = 0u;
                    USBUART2_midiInPointer++;
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = 0u;
                    ...
                    break;
                case 1u:
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_SYSEX_ENDS_WITH2 | cable;
                    USBUART2_midiInPointer++;
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = srcBuffZero;
                    USBUART2_midiInPointer++;
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_MIDI_EOSEX;
                    ...
                    break;
                case 2u:
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = USBUART2_SYSEX_ENDS_WITH3 | cable;
                    USBUART2_midiInPointer++;
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = srcBuffZero;
                    USBUART2_midiInPointer++;
                    USBUART2_midiInBuffer[USBUART2_midiInPointer] = srcBuffOne;
...
*/
#if (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBUART2_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   USBUART2_MIDI_Event: initialized to zero.
    *   USBUART2_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void USBUART2_MIDI_InitInterface(void) 
    {
        USBUART2_MIDI1_Event.length  = 0u;
        USBUART2_MIDI1_Event.count   = 0u;
        USBUART2_MIDI1_Event.size    = 0u;
        USBUART2_MIDI1_Event.runstat = 0u;
        USBUART2_MIDI1_TxRunStat     = 0u;
        USBUART2_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, USBUART2_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, USBUART2_CUSTOM_UART_RX_PRIOR_NUM);

    #if (USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF)
        USBUART2_MIDI2_Event.length  = 0u;
        USBUART2_MIDI2_Event.count   = 0u;
        USBUART2_MIDI2_Event.size    = 0u;
        USBUART2_MIDI2_Event.runstat = 0u;
        USBUART2_MIDI2_TxRunStat     = 0u;
        USBUART2_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, USBUART2_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, USBUART2_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF) */
    ...
    #ifdef USBUART2_MIDI_INIT_CALLBACK
        USBUART2_MIDI_Init_Callback();
    #endif /* (USBUART2_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: USBUART2_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    uint8 USBUART2_ProcessMidiIn(uint8 mData, USBUART2_MIDI_RX_STATUS *rxStat)
                                                                
    {
        ...
        */
        if ((mData & USBUART2_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & USBUART2_MIDI_STATUS_MASK) == USBUART2_MIDI_STATUS_MASK)
            {
                if ((mData & USBUART2_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case USBUART2_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = USBUART2_MIDI_SYSEX;
                            rxStat->runstat = USBUART2_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBUART2_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = USBUART2_MIDI_EOSEX;
                            break;
                        case USBUART2_MIDI_SPP:
                            rxStat->msgBuff[0u] = USBUART2_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBUART2_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = USBUART2_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBUART2_MIDI_QFM:
                            rxStat->msgBuff[0u] = USBUART2_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBUART2_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = USBUART2_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & USBUART2_MIDI_STATUS_MASK)
                {
                    case USBUART2_MIDI_NOTE_OFF:
                    case USBUART2_MIDI_NOTE_ON:
                    case USBUART2_MIDI_POLY_KEY_PRESSURE:
                    case USBUART2_MIDI_CONTROL_CHANGE:
                    case USBUART2_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case USBUART2_MIDI_PROGRAM_CHANGE:
                    case USBUART2_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == USBUART2_MIDI_SYSEX)
            {
                ...
                if (rxStat->count >= rxStat->length)
                {
                    rxStat->size = rxStat->count;
                    rxStat->count = 0u;
                    midiReturn = USBUART2_MIDI_SYSEX;
                }
            }
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & USBUART2_MIDI_STATUS_MASK)
                {
                    case USBUART2_MIDI_NOTE_OFF:
                    case USBUART2_MIDI_NOTE_ON:
                    case USBUART2_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: USBUART2_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBUART2_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
    uint8 USBUART2_MIDI1_GetEvent(void) 
    {
        uint8 msgRtn = 0u;
        uint8 rxData;
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBUART2_ProcessMidiIn(rxData,
                                                    (USBUART2_MIDI_RX_STATUS *)&USBUART2_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: USBUART2_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBUART2_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBUART2_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    USBUART2_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    USBUART2_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *      This bit should be cleared by user when Identity Reply message generated.
    *
    *******************************************************************************/
    void USBUART2_MIDI1_ProcessUsbOut(const uint8 epBuf[])
    ...
    #ifdef USBUART2_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBUART2_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (USBUART2_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[USBUART2_EVENT_BYTE0] & USBUART2_CIN_MASK;

        if ((cmd != USBUART2_RESERVED0) && (cmd != USBUART2_RESERVED1))
        {
            len = USBUART2_MIDI_SIZE[cmd];
            i = USBUART2_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == USBUART2_SYSEX)
            {
                if ((epBuf[USBUART2_EVENT_BYTE1] == USBUART2_MIDI_SYSEX) &&
                    (epBuf[USBUART2_EVENT_BYTE2] == USBUART2_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    USBUART2_MIDI1_InqFlags |= USBUART2_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBUART2_MIDI1_InqFlags &= (uint8)~USBUART2_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == USBUART2_SYSEX_ENDS_WITH1)
            {
                USBUART2_MIDI1_InqFlags &= (uint8)~USBUART2_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBUART2_SYSEX_ENDS_WITH2)
            {
                USBUART2_MIDI1_InqFlags &= (uint8)~USBUART2_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBUART2_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((USBUART2_MIDI1_InqFlags & USBUART2_INQ_SYSEX_FLAG) != 0u)
                {
                    USBUART2_MIDI1_InqFlags &= (uint8)~USBUART2_INQ_SYSEX_FLAG;
                    if ((epBuf[USBUART2_EVENT_BYTE1] == USBUART2_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[USBUART2_EVENT_BYTE2] == USBUART2_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBUART2_MIDI1_InqFlags |= USBUART2_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= USBUART2_NOTE_OFF) && (cmd <= USBUART2_PITCH_BEND_CHANGE))
            {
                if (USBUART2_MIDI1_TxRunStat == epBuf[USBUART2_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    USBUART2_MIDI1_TxRunStat = epBuf[USBUART2_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef USBUART2_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        USBUART2_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (USBUART2_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBUART2_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  USBUART2_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
    uint8 USBUART2_MIDI2_GetEvent(void) 
    {
        uint8 msgRtn = 0u;
        uint8 rxData;
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBUART2_ProcessMidiIn(rxData,
                                                    (USBUART2_MIDI_RX_STATUS *)&USBUART2_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: USBUART2_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  USBUART2_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBUART2_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  USBUART2_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  USBUART2_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    *
    *******************************************************************************/
    void USBUART2_MIDI2_ProcessUsbOut(const uint8 epBuf[])
                                                            
    {
    ...
    #ifdef USBUART2_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBUART2_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (USBUART2_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != USBUART2_RESERVED0) && (cmd != USBUART2_RESERVED1))
        {
            ...
            if(cmd == USBUART2_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    USBUART2_MIDI2_InqFlags |= USBUART2_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBUART2_MIDI2_InqFlags &= (uint8)~USBUART2_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == USBUART2_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((USBUART2_MIDI2_InqFlags & USBUART2_INQ_SYSEX_FLAG) != 0u)
                {
                    USBUART2_MIDI2_InqFlags &= (uint8)~USBUART2_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBUART2_MIDI2_InqFlags |= USBUART2_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= USBUART2_NOTE_OFF) && ( cmd <= USBUART2_PITCH_BEND_CHANGE))
            {
                if (USBUART2_MIDI2_TxRunStat == epBuf[USBUART2_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    USBUART2_MIDI2_TxRunStat = epBuf[USBUART2_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef USBUART2_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        USBUART2_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (USBUART2_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_TWO_EXT_INTRF) */
#endif /* (USBUART2_MIDI_EXT_MODE >= USBUART2_ONE_EXT_INTRF) */

#endif  /*  (USBUART2_ENABLE_MIDI_API != 0u) */
...
#endif  /* defined(USBUART2_ENABLE_MIDI_STREAMING) */
