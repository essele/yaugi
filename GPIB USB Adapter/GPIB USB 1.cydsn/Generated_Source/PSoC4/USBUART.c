...
void USBUART_Stop(void) 
{
    ...
    USBUART_CR0_REG &= (uint32) ~USBUART_CR0_ENABLE;

    /* Disable D+ pull-up. */
    USBUART_USBIO_CR1_REG &= (uint32) ~ USBUART_USBIO_CR1_USBPUEN;
...
{
    uint32 cr1Reg = USBUART_CR1_REG;

    /* Clear bus activity. */
    USBUART_CR1_REG = (cr1Reg & (uint32) ~USBUART_CR1_BUS_ACTIVITY);
