...
***************************************/
    ...
    static const uint8 CYCODE USBCOMP_MIDI_SIZE[] = {
...
***************************************/
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 USBCOMP_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 USBCOMP_midiInPointer;                             /* Input endpoint buffer pointer */
    ...
    volatile uint8 USBCOMP_midi_in_ep;
    ...
    uint8 USBCOMP_midiInBuffer[USBCOMP_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
    ...
    volatile uint8 USBCOMP_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 USBCOMP_midiOutBuffer[USBCOMP_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (USBCOMP_MIDI_OUT_BUFF_SIZE > 0) */

#if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF)

    static USBCOMP_MIDI_RX_STATUS USBCOMP_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 USBCOMP_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 USBCOMP_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF)
        static USBCOMP_MIDI_RX_STATUS USBCOMP_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 USBCOMP_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref USBCOMP_MIDI1_InqFlags*/
        volatile uint8 USBCOMP_MIDI2_InqFlags;                             /* Device inquiry flag */
...
#if (USBCOMP_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: USBCOMP_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBCOMP_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. USBCOMP_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref USBCOMP_midiOutBuffer: This buffer is used by the
*    USBCOMP_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref USBCOMP_callbackLocalMidiEvent function.
*
*  \ref USBCOMP_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref USBCOMP_midi_in_ep: Used as an IN endpoint number.
*
*   \ref USBCOMP_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void USBCOMP_MIDI_Init(void) 
{
   ...
   USBCOMP_midiInPointer = 0u;
        ...
        USBCOMP_LoadInEP(USBCOMP_midi_in_ep, USBCOMP_midiInBuffer,
                                                               USBCOMP_MIDI_IN_BUFF_SIZE);
    #endif  /* (USBCOMP_MIDI_IN_BUFF_SIZE > 0) */

    #if (USBCOMP_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)USBCOMP_ReadOutEP(USBCOMP_midi_out_ep, USBCOMP_midiOutBuffer,
                                                                       USBCOMP_MIDI_OUT_BUFF_SIZE);
    #endif /* (USBCOMP_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO) */

#if (USBCOMP_MIDI_OUT_BUFF_SIZE > 0)
    USBCOMP_EnableOutEP(USBCOMP_midi_out_ep);
#endif /* (USBCOMP_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF)
    USBCOMP_MIDI_InitInterface();
#endif /* (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF) */
}
...
#if (USBCOMP_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBCOMP_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref USBCOMP_midiOutBuffer: Used as temporary buffer between USB
    ...
    *******************************************************************************/
    ...
    {
    #if (USBCOMP_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (USBCOMP_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (USBCOMP_OUT_BUFFER_FULL == USBCOMP_GetEPState(USBCOMP_midi_out_ep))
        {
        #if (USBCOMP_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = USBCOMP_GetEPCount(USBCOMP_midi_out_ep);
        #else
            outLength = (uint8)USBCOMP_GetEPCount(USBCOMP_midi_out_ep);
        #endif /* (USBCOMP_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!USBCOMP_EP_MANAGEMENT_DMA_AUTO)
            #if (USBCOMP_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = USBCOMP_ReadOutEP(USBCOMP_midi_out_ep,
                                                       USBCOMP_midiOutBuffer, outLength);
            #else
                outLength = (uint8)USBCOMP_ReadOutEP(USBCOMP_midi_out_ep,
                                                              USBCOMP_midiOutBuffer, (uint16) outLength);
            #endif /* (USBCOMP_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (USBCOMP_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (USBCOMP_OUT_BUFFER_FULL == USBCOMP_GetEPState(USBCOMP_midi_out_ep))
                {
                }
            ...
            if (outLength >= USBCOMP_EVENT_LENGTH)
            {
                ...
                #if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF)
                    if ((USBCOMP_midiOutBuffer[outPointer] & USBCOMP_CABLE_MASK) ==
                         USBCOMP_MIDI_CABLE_00)
                    {
                        USBCOMP_MIDI1_ProcessUsbOut(&USBCOMP_midiOutBuffer[outPointer]);
                    }
                    else if ((USBCOMP_midiOutBuffer[outPointer] & USBCOMP_CABLE_MASK) ==
                             USBCOMP_MIDI_CABLE_01)
                    {
                    #if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF)
                         USBCOMP_MIDI2_ProcessUsbOut(&USBCOMP_midiOutBuffer[outPointer]);
                    #endif /*  USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef USBCOMP_MIDI_OUT_EP_SERVICE_CALLBACK
                            USBCOMP_MIDI_OUT_EP_Service_Callback();
                        #endif /* USBCOMP_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    USBCOMP_callbackLocalMidiEvent(USBCOMP_midiOutBuffer[outPointer] & USBCOMP_CABLE_MASK,
                                                            &USBCOMP_midiOutBuffer[outPointer + USBCOMP_EVENT_BYTE1]);
                    outPointer += USBCOMP_EVENT_LENGTH;
                }
            }
...
#if (USBCOMP_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBCOMP_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  USBCOMP_MIDI_IN_Service() and from the
    *  USBCOMP_PutUsbMidiIn() function.
    *
    * \globalvars
    *  USBCOMP_midi_in_ep: Used as an IN endpoint number.
    *  USBCOMP_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   USBCOMP_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBCOMP_MIDI_IN_EP_Service(void) 
    {
        ...
        if (USBCOMP_midiInPointer != 0u)
        {
            if(USBCOMP_GetEPState(USBCOMP_midi_in_ep) == USBCOMP_EVENT_PENDING)
            {
            #if (USBCOMP_EP_MANAGEMENT_DMA_AUTO)
                USBCOMP_LoadInEP(USBCOMP_midi_in_ep, NULL, (uint16)USBCOMP_midiInPointer);
            #else
                USBCOMP_LoadInEP(USBCOMP_midi_in_ep, USBCOMP_midiInBuffer,
                                                              (uint16) USBCOMP_midiInPointer);
            #endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *******************************************************************************/
    void USBCOMP_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == USBCOMP_midiInPointer)
        {
            do
            {
                if (USBCOMP_midiInPointer <= (USBCOMP_MIDI_IN_BUFF_SIZE - USBCOMP_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = USBCOMP_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        USBCOMP_PrepareInBuffer(m1, (uint8 *)&USBCOMP_MIDI1_Event.msgBuff[0],
                                                                       USBCOMP_MIDI1_Event.size, USBCOMP_MIDI_CABLE_00);
                    }
                }
                ...
                if (USBCOMP_midiInPointer <= (USBCOMP_MIDI_IN_BUFF_SIZE - USBCOMP_EVENT_LENGTH))
                {
                    ...
                    m2 = USBCOMP_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        USBCOMP_PrepareInBuffer(m2, (uint8 *)&USBCOMP_MIDI2_Event.msgBuff[0],
                                                                       USBCOMP_MIDI2_Event.size, USBCOMP_MIDI_CABLE_01);
                    }
                }
            #endif /*  USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF */
            }
            while((USBCOMP_midiInPointer <= (USBCOMP_MIDI_IN_BUFF_SIZE - USBCOMP_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        USBCOMP_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: USBCOMP_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The USBCOMP_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  USBCOMP_TRUE  | Host is not ready to receive this message
    *  USBCOMP_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref USBCOMP_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref USBCOMP_midiInPointer: Checked this variable to see if
    ...
    ***************************************************************************/
                                                                ...
                                                                
    {
        ...
        if (USBCOMP_midiInPointer >
                    (USBCOMP_EP[USBCOMP_midi_in_ep].bufferSize - USBCOMP_EVENT_LENGTH))
        {
            USBCOMP_MIDI_IN_EP_Service();
        }
        ...
        if (USBCOMP_midiInPointer <=
                    (USBCOMP_EP[USBCOMP_midi_in_ep].bufferSize - USBCOMP_EVENT_LENGTH))
        {
            if((ic < USBCOMP_EVENT_LENGTH) || (ic >= USBCOMP_MIDI_STATUS_MASK))
            {
                USBCOMP_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else /* Only SysEx message is greater than 4 bytes */
            {
                ...
                do
                {
                    ...
                    USBCOMP_PrepareInBuffer(USBCOMP_MIDI_SYSEX, &midiMsg[idx],
                                                     USBCOMP_EVENT_BYTE3, cable);

                    /* Move to next 3 bytes of message */
                    ic  -= USBCOMP_EVENT_BYTE3;
                    idx += USBCOMP_EVENT_BYTE3;
                        ...
                        (USBCOMP_EP[USBCOMP_midi_in_ep].bufferSize - USBCOMP_EVENT_LENGTH))
                    {
                           ...
                           (USBCOMP_EP[USBCOMP_midi_in_ep].bufferSize - USBCOMP_EVENT_LENGTH))
                        {
                            ...
                            retError = USBCOMP_TRUE;
                            break;
                        }
                    }
                }
                while (ic >= USBCOMP_EVENT_BYTE3);

                if (retError == USBCOMP_FALSE)
                {
                    /* Handle end of message: valid size of messages is 0, 1 and 2 */
                    USBCOMP_PrepareInBuffer(USBCOMP_MIDI_EOSEX, &midiMsg[idx], ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: USBCOMP_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBCOMP_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    USBCOMP_PutUsbMidiIn() function messages.
    *  USBCOMP_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    ...
    *******************************************************************************/
                                                                 ...
                                                                 
    {
        ...
        if (ic >= (USBCOMP_MIDI_STATUS_MASK | USBCOMP_MIDI_SINGLE_BYTE_MASK))
        {
            USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_SINGLE_BYTE | cable;
            USBCOMP_midiInPointer++;
            USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = ic;
            USBCOMP_midiInPointer++;
            USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = 0u;
            USBCOMP_midiInPointer++;
            USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = 0u;
            USBCOMP_midiInPointer++;
        }
        else if((ic < USBCOMP_EVENT_LENGTH) || (ic == USBCOMP_MIDI_SYSEX))
        {
            if(ic == USBCOMP_MIDI_SYSEX)
            {
                USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_SYSEX | cable;
                USBCOMP_midiInPointer++;
            }
            else if (srcBuffZero < USBCOMP_MIDI_SYSEX)
            {
                USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = (srcBuffZero >> 4u) | cable;
                USBCOMP_midiInPointer++;
            }
            else if (srcBuffZero == USBCOMP_MIDI_TUNEREQ)
            {
                USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_1BYTE_COMMON | cable;
                USBCOMP_midiInPointer++;
            }
            else if ((srcBuffZero == USBCOMP_MIDI_QFM) || (srcBuffZero == USBCOMP_MIDI_SONGSEL))
            {
                USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_2BYTE_COMMON | cable;
                USBCOMP_midiInPointer++;
            }
            else if (srcBuffZero == USBCOMP_MIDI_SPP)
            {
                USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_3BYTE_COMMON | cable;
                USBCOMP_midiInPointer++;
            }
            else
            {
            ...
            USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = srcBuffZero;
            USBCOMP_midiInPointer++;
            USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = srcBuffOne;
            USBCOMP_midiInPointer++;
            USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = srcBuff[2u];
            USBCOMP_midiInPointer++;
        }
        else if (ic == USBCOMP_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_SYSEX_ENDS_WITH1 | cable;
                    USBCOMP_midiInPointer++;
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_MIDI_EOSEX;
                    USBCOMP_midiInPointer++;
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = 0u;
                    USBCOMP_midiInPointer++;
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = 0u;
                    ...
                    break;
                case 1u:
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_SYSEX_ENDS_WITH2 | cable;
                    USBCOMP_midiInPointer++;
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = srcBuffZero;
                    USBCOMP_midiInPointer++;
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_MIDI_EOSEX;
                    ...
                    break;
                case 2u:
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = USBCOMP_SYSEX_ENDS_WITH3 | cable;
                    USBCOMP_midiInPointer++;
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = srcBuffZero;
                    USBCOMP_midiInPointer++;
                    USBCOMP_midiInBuffer[USBCOMP_midiInPointer] = srcBuffOne;
...
*/
#if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBCOMP_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   USBCOMP_MIDI_Event: initialized to zero.
    *   USBCOMP_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void USBCOMP_MIDI_InitInterface(void) 
    {
        USBCOMP_MIDI1_Event.length  = 0u;
        USBCOMP_MIDI1_Event.count   = 0u;
        USBCOMP_MIDI1_Event.size    = 0u;
        USBCOMP_MIDI1_Event.runstat = 0u;
        USBCOMP_MIDI1_TxRunStat     = 0u;
        USBCOMP_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, USBCOMP_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, USBCOMP_CUSTOM_UART_RX_PRIOR_NUM);

    #if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF)
        USBCOMP_MIDI2_Event.length  = 0u;
        USBCOMP_MIDI2_Event.count   = 0u;
        USBCOMP_MIDI2_Event.size    = 0u;
        USBCOMP_MIDI2_Event.runstat = 0u;
        USBCOMP_MIDI2_TxRunStat     = 0u;
        USBCOMP_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, USBCOMP_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, USBCOMP_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF) */
    ...
    #ifdef USBCOMP_MIDI_INIT_CALLBACK
        USBCOMP_MIDI_Init_Callback();
    #endif /* (USBCOMP_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: USBCOMP_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBCOMP_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & USBCOMP_MIDI_STATUS_MASK) == USBCOMP_MIDI_STATUS_MASK)
            {
                if ((mData & USBCOMP_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case USBCOMP_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = USBCOMP_MIDI_SYSEX;
                            rxStat->runstat = USBCOMP_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBCOMP_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = USBCOMP_MIDI_EOSEX;
                            break;
                        case USBCOMP_MIDI_SPP:
                            rxStat->msgBuff[0u] = USBCOMP_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBCOMP_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = USBCOMP_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBCOMP_MIDI_QFM:
                            rxStat->msgBuff[0u] = USBCOMP_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBCOMP_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = USBCOMP_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & USBCOMP_MIDI_STATUS_MASK)
                {
                    case USBCOMP_MIDI_NOTE_OFF:
                    case USBCOMP_MIDI_NOTE_ON:
                    case USBCOMP_MIDI_POLY_KEY_PRESSURE:
                    case USBCOMP_MIDI_CONTROL_CHANGE:
                    case USBCOMP_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case USBCOMP_MIDI_PROGRAM_CHANGE:
                    case USBCOMP_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == USBCOMP_MIDI_SYSEX)
            {
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & USBCOMP_MIDI_STATUS_MASK)
                {
                    case USBCOMP_MIDI_NOTE_OFF:
                    case USBCOMP_MIDI_NOTE_ON:
                    case USBCOMP_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: USBCOMP_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBCOMP_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBCOMP_ProcessMidiIn(rxData,
                                                    (USBCOMP_MIDI_RX_STATUS *)&USBCOMP_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: USBCOMP_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBCOMP_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBCOMP_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    USBCOMP_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    USBCOMP_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    ...
    *******************************************************************************/
    ...
    #ifdef USBCOMP_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBCOMP_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (USBCOMP_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[USBCOMP_EVENT_BYTE0] & USBCOMP_CIN_MASK;

        if ((cmd != USBCOMP_RESERVED0) && (cmd != USBCOMP_RESERVED1))
        {
            len = USBCOMP_MIDI_SIZE[cmd];
            i = USBCOMP_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == USBCOMP_SYSEX)
            {
                if ((epBuf[USBCOMP_EVENT_BYTE1] == USBCOMP_MIDI_SYSEX) &&
                    (epBuf[USBCOMP_EVENT_BYTE2] == USBCOMP_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    USBCOMP_MIDI1_InqFlags |= USBCOMP_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBCOMP_MIDI1_InqFlags &= (uint8)~USBCOMP_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == USBCOMP_SYSEX_ENDS_WITH1)
            {
                USBCOMP_MIDI1_InqFlags &= (uint8)~USBCOMP_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBCOMP_SYSEX_ENDS_WITH2)
            {
                USBCOMP_MIDI1_InqFlags &= (uint8)~USBCOMP_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBCOMP_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((USBCOMP_MIDI1_InqFlags & USBCOMP_INQ_SYSEX_FLAG) != 0u)
                {
                    USBCOMP_MIDI1_InqFlags &= (uint8)~USBCOMP_INQ_SYSEX_FLAG;
                    if ((epBuf[USBCOMP_EVENT_BYTE1] == USBCOMP_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[USBCOMP_EVENT_BYTE2] == USBCOMP_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBCOMP_MIDI1_InqFlags |= USBCOMP_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= USBCOMP_NOTE_OFF) && (cmd <= USBCOMP_PITCH_BEND_CHANGE))
            {
                if (USBCOMP_MIDI1_TxRunStat == epBuf[USBCOMP_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    USBCOMP_MIDI1_TxRunStat = epBuf[USBCOMP_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef USBCOMP_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        USBCOMP_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (USBCOMP_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBCOMP_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  USBCOMP_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBCOMP_ProcessMidiIn(rxData,
                                                    (USBCOMP_MIDI_RX_STATUS *)&USBCOMP_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: USBCOMP_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  USBCOMP_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBCOMP_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  USBCOMP_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  USBCOMP_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
    ...
    #ifdef USBCOMP_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBCOMP_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (USBCOMP_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != USBCOMP_RESERVED0) && (cmd != USBCOMP_RESERVED1))
        {
            ...
            if(cmd == USBCOMP_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    USBCOMP_MIDI2_InqFlags |= USBCOMP_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBCOMP_MIDI2_InqFlags &= (uint8)~USBCOMP_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == USBCOMP_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((USBCOMP_MIDI2_InqFlags & USBCOMP_INQ_SYSEX_FLAG) != 0u)
                {
                    USBCOMP_MIDI2_InqFlags &= (uint8)~USBCOMP_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBCOMP_MIDI2_InqFlags |= USBCOMP_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= USBCOMP_NOTE_OFF) && ( cmd <= USBCOMP_PITCH_BEND_CHANGE))
            {
                if (USBCOMP_MIDI2_TxRunStat == epBuf[USBCOMP_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    USBCOMP_MIDI2_TxRunStat = epBuf[USBCOMP_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef USBCOMP_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        USBCOMP_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (USBCOMP_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (USBCOMP_MIDI_EXT_MODE >= USBCOMP_TWO_EXT_INTRF) */
#endif /* (USBCOMP_MIDI_EXT_MODE >= USBCOMP_ONE_EXT_INTRF) */
...
#endif  /* defined(USBCOMP_ENABLE_MIDI_STREAMING) */
