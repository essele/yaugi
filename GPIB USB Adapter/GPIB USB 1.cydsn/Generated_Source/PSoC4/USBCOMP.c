...
* functions.
*/
uint8 USBCOMP_initVar = 0u;

#if (USBCOMP_EP_MANAGEMENT_DMA)
    #if (CY_PSOC4)
        static void USBCOMP_InitEpDma(void);

        /* DMA chanels assigend for endpoints. */
        const uint8 USBCOMP_DmaChan[USBCOMP_MAX_EP] =
        {
            0u,
            0u,
            0u,
    ...
    #else
        /* DMA chanels assigend for endpoints. */
        uint8 USBCOMP_DmaChan[USBCOMP_MAX_EP];
        
        /* DMA TDs require for PSoC 3/5LP operation. */
        uint8 USBCOMP_DmaTd[USBCOMP_MAX_EP];
    #endif /* (CY_PSOC4) */
    ...
    uint8  USBCOMP_DmaEpBurstCnt   [USBCOMP_MAX_EP];
    
    /* Number of bytes to transfer in last DMA burst. */
    uint8  USBCOMP_DmaEpLastBurstEl[USBCOMP_MAX_EP];

    /* Storage for arrays above. */
    uint8  USBCOMP_DmaEpBurstCntBackup  [USBCOMP_MAX_EP];
    uint32 USBCOMP_DmaEpBufferAddrBackup[USBCOMP_MAX_EP];
     
    /* DMA trigger mux output for usb.dma_req[0-7]. */
    const uint8 USBCOMP_DmaReqOut[USBCOMP_MAX_EP] =
    {
        0u,
        USBCOMP_ep1_dma__TR_OUTPUT,
        USBCOMP_ep2_dma__TR_OUTPUT,
        USBCOMP_ep3_dma__TR_OUTPUT,
        0u,
        0u,
        0u,
        0u,
    ...
    const uint8 USBCOMP_DmaBurstEndOut[USBCOMP_MAX_EP] =
    {
        0u,
        USBCOMP_BURSTEND_0_TR_OUTPUT,
        USBCOMP_BURSTEND_1_TR_OUTPUT,
        USBCOMP_BURSTEND_2_TR_OUTPUT,
        USBCOMP_BURSTEND_3_TR_OUTPUT,
        USBCOMP_BURSTEND_4_TR_OUTPUT,
        USBCOMP_BURSTEND_5_TR_OUTPUT,
        USBCOMP_BURSTEND_6_TR_OUTPUT,
        USBCOMP_BURSTEND_7_TR_OUTPUT
    };
    
#else
    #if (USBCOMP_EP_DMA_AUTO_OPT == 0u)
        static uint8 clearInDataRdyStatus = USBCOMP_ARB_EPX_CFG_DEFAULT;
        uint8 USBCOMP_DmaNextTd[USBCOMP_MAX_EP];
        const uint8 USBCOMP_epX_TD_TERMOUT_EN[USBCOMP_MAX_EP] =
        {
            0u,
            0u,
            0u,
        ...
        volatile uint16 USBCOMP_inLength[USBCOMP_MAX_EP];
        const uint8 *USBCOMP_inDataPointer[USBCOMP_MAX_EP];
        volatile uint8 USBCOMP_inBufFull[USBCOMP_MAX_EP];
...
*   following list.
*
*       *USBCOMP_3V_OPERATION* - Disable voltage regulator and pass-
*                                      through Vcc for pull-up
*
*       *USBCOMP_5V_OPERATION* - Enable voltage regulator and use
*                                      regulator for pull-up
*
*       *USBCOMP_DWR_POWER_OPERATION* - Enable or disable the voltage
...
{
    if (0u == USBCOMP_initVar)
    {
        USBCOMP_Init();
        USBCOMP_initVar = 1u;
    }

    USBCOMP_InitComponent(device, mode);
}


/*******************************************************************************
* Function Name: USBCOMP_Init
****************************************************************************//**
*
...
* USBCOMP_Init() because the USBCOMP_Start() routine calls
...
void USBCOMP_Init(void) 
{
    ...
    * USBCOMP_InitComponent() if it is required.
    */

    /* Enable USBIO control on drive mode of D+ and D- pins. */
    USBCOMP_USBIO_CR1_REG &= ~ (uint32) USBCOMP_USBIO_CR1_IOMODE;

    /* Set number of LF CLK to detect UBS bus reset. */
    USBCOMP_BUS_RST_CNT_REG = USBCOMP_DEFUALT_BUS_RST_CNT;
    ...
    */
    USBCOMP_POWER_CTRL_REG = USBCOMP_DEFAULT_POWER_CTRL_VBUS;

    /* Enable PHY detector and single-ended and differential receivers. */
    USBCOMP_POWER_CTRL_REG |= USBCOMP_DEFAULT_POWER_CTRL_PHY;
    ...
    USBCOMP_CR1_REG = USBCOMP_DEFUALT_CR1;
    ...
    USBCOMP_INTR_SIE_MASK_REG = USBCOMP_DEFAULT_INTR_SIE_MASK;
    ...
    USBCOMP_CR1_REG = USBCOMP_CR1_ENABLE_LOCK;
    ...
    */
    ...
    USBCOMP_USBIO_CR1_REG &= (uint8) ~USBCOMP_USBIO_CR1_IOMODE;
    ...
    CyDelayUs(USBCOMP_WAIT_REG_STABILITY_1US);
    ...
    USBCOMP_DM_INP_DIS_REG &= (uint8) ~USBCOMP_DM_MASK;
    USBCOMP_DP_INP_DIS_REG &= (uint8) ~USBCOMP_DP_MASK;
    ...
    CY_SET_REG8(USBCOMP_ARB_RW1_WA_PTR,     0u);
    CY_SET_REG8(USBCOMP_ARB_RW1_WA_MSB_PTR, 0u);

#if (USBCOMP_EP_MANAGEMENT_DMA)
    /* Initialize transfer descriptor. This will be used to detect DMA state - initialized or not. */
    for (i = 0u; i < USBCOMP_MAX_EP; ++i)
    {
        USBCOMP_DmaTd[i] = DMA_INVALID_TD;

    #if (USBCOMP_EP_MANAGEMENT_DMA_AUTO && (USBCOMP_EP_DMA_AUTO_OPT == 0u))
        USBCOMP_DmaNextTd[i] = DMA_INVALID_TD;
    ...
    CyIntSetPriority     (USBCOMP_INTR_HI_VECT_NUM,  USBCOMP_INTR_HI_PRIORITY);
    (void) CyIntSetVector(USBCOMP_INTR_HI_VECT_NUM, &USBCOMP_INTR_HI_ISR);

    /* Configure lo_int: set handler and priority. */
    CyIntSetPriority     (USBCOMP_INTR_LO_VECT_NUM,  USBCOMP_INTR_LO_PRIORITY);
    (void) CyIntSetVector(USBCOMP_INTR_LO_VECT_NUM, &USBCOMP_INTR_LO_ISR);

    /* Configure med_int: set handler and priority (routed through DSI). */
    CyIntSetPriority     (USBCOMP_INTR_MED_VECT_NUM,  USBCOMP_INTR_MED_PRIORITY);
    (void) CyIntSetVector(USBCOMP_INTR_MED_VECT_NUM, &USBCOMP_INTR_MED_ISR);

#else
    /* Set bus reset interrupt. */
    CyIntSetPriority(USBCOMP_BUS_RESET_VECT_NUM, USBCOMP_BUS_RESET_PRIOR);
    (void) CyIntSetVector(USBCOMP_BUS_RESET_VECT_NUM,   &USBCOMP_BUS_RESET_ISR);

    /* Set Control Endpoint Interrupt. */
    CyIntSetPriority(USBCOMP_EP_0_VECT_NUM, USBCOMP_EP_0_PRIOR);
    (void) CyIntSetVector(USBCOMP_EP_0_VECT_NUM,   &USBCOMP_EP_0_ISR);

    /* Set SOF interrupt. */
    #if (USBCOMP_SOF_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_SOF_VECT_NUM,  USBCOMP_SOF_PRIOR);
        (void) CyIntSetVector(USBCOMP_SOF_VECT_NUM, &USBCOMP_SOF_ISR);
    #endif /* (USBCOMP_SOF_ISR_ACTIVE) */

    /* Set Data Endpoint 1 Interrupt. */
    #if (USBCOMP_EP1_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_1_VECT_NUM,  USBCOMP_EP_1_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_1_VECT_NUM,  &USBCOMP_EP_1_ISR);
    #endif /* (USBCOMP_EP1_ISR_ACTIVE) */

    /* Set Data Endpoint 2 Interrupt. */
    #if (USBCOMP_EP2_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_2_VECT_NUM,  USBCOMP_EP_2_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_2_VECT_NUM, &USBCOMP_EP_2_ISR);
    #endif /* (USBCOMP_EP2_ISR_ACTIVE) */

    /* Set Data Endpoint 3 Interrupt. */
    #if (USBCOMP_EP3_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_3_VECT_NUM,  USBCOMP_EP_3_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_3_VECT_NUM, &USBCOMP_EP_3_ISR);
    #endif /* (USBCOMP_EP3_ISR_ACTIVE) */

    /* Set Data Endpoint 4 Interrupt. */
    #if (USBCOMP_EP4_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_4_VECT_NUM,  USBCOMP_EP_4_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_4_VECT_NUM, &USBCOMP_EP_4_ISR);
    #endif /* (USBCOMP_EP4_ISR_ACTIVE) */

    /* Set Data Endpoint 5 Interrupt. */
    #if (USBCOMP_EP5_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_5_VECT_NUM,  USBCOMP_EP_5_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_5_VECT_NUM, &USBCOMP_EP_5_ISR);
    #endif /* (USBCOMP_EP5_ISR_ACTIVE) */

    /* Set Data Endpoint 6 Interrupt. */
    #if (USBCOMP_EP6_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_6_VECT_NUM,  USBCOMP_EP_6_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_6_VECT_NUM, &USBCOMP_EP_6_ISR);
    #endif /* (USBCOMP_EP6_ISR_ACTIVE) */

     /* Set Data Endpoint 7 Interrupt. */
    #if (USBCOMP_EP7_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_7_VECT_NUM,  USBCOMP_EP_7_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_7_VECT_NUM, &USBCOMP_EP_7_ISR);
    #endif /* (USBCOMP_EP7_ISR_ACTIVE) */

    /* Set Data Endpoint 8 Interrupt. */
    #if (USBCOMP_EP8_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_EP_8_VECT_NUM,  USBCOMP_EP_8_PRIOR);
        (void) CyIntSetVector(USBCOMP_EP_8_VECT_NUM, &USBCOMP_EP_8_ISR);
    #endif /* (USBCOMP_EP8_ISR_ACTIVE) */

    /* Set ARB Interrupt. */
    #if (USBCOMP_EP_MANAGEMENT_DMA && USBCOMP_ARB_ISR_ACTIVE)
        CyIntSetPriority     (USBCOMP_ARB_VECT_NUM,  USBCOMP_ARB_PRIOR);
        (void) CyIntSetVector(USBCOMP_ARB_VECT_NUM, &USBCOMP_ARB_ISR);
    #endif /* (USBCOMP_EP_MANAGEMENT_DMA && USBCOMP_ARB_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

    /* Common: Configure GPIO interrupt for wakeup. */
#if (USBCOMP_DP_ISR_ACTIVE)
    CyIntSetPriority     (USBCOMP_DP_INTC_VECT_NUM,  USBCOMP_DP_INTC_PRIORITY);
    (void) CyIntSetVector(USBCOMP_DP_INTC_VECT_NUM, &USBCOMP_DP_ISR);
#endif /* (USBCOMP_DP_ISR_ACTIVE) */

#if (USBCOMP_EP_MANAGEMENT_DMA && CY_PSOC4)
    /* Initialize DMA channels. */
    USBCOMP_InitEpDma();
#endif /* (USBCOMP_EP_MANAGEMENT_DMA && CY_PSOC4) */
}


/*******************************************************************************
* Function Name: USBCOMP_InitComponent
****************************************************************************//**
*
...
* \globalvars
*   \ref USBCOMP_device
*   \ref USBCOMP_transferState
*   \ref USBCOMP_configuration
*   \ref USBCOMP_deviceStatus
*
*   \ref USBCOMP_deviceAddress - Contains the current device address. This
...
*      SET_ADDRESS request.
*
*   \ref USBCOMP_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
void USBCOMP_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
    ...
    {
        USBCOMP_hidProtocol[i] = USBCOMP_PROTOCOL_REPORT;
    }
#endif /* USBCOMP_ENABLE_HID_CLASS */

    /* Store device number to access descriptor. */
    USBCOMP_device = device;

    /* Reset component internal variables. */
    USBCOMP_transferState   = USBCOMP_TRANS_STATE_IDLE;
    USBCOMP_configurationChanged = 0u;
    USBCOMP_configuration   = 0u;
    USBCOMP_interfaceNumber = 0u;
    USBCOMP_deviceAddress   = 0u;
    USBCOMP_deviceStatus    = 0u;
    USBCOMP_lastPacketSize  = 0u;

    /* Enable component interrupts. */
#if (CY_PSOC4)
    CyIntEnable(USBCOMP_INTR_HI_VECT_NUM);
    CyIntEnable(USBCOMP_INTR_MED_VECT_NUM);
    CyIntEnable(USBCOMP_INTR_LO_VECT_NUM);
#else
    CyIntEnable(USBCOMP_BUS_RESET_VECT_NUM);
    CyIntEnable(USBCOMP_EP_0_VECT_NUM);

    #if (USBCOMP_SOF_ISR_ACTIVE)
        CyIntEnable(USBCOMP_SOF_VECT_NUM);
    #endif /* (USBCOMP_SOF_ISR_ACTIVE) */

    #if (USBCOMP_EP1_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_1_VECT_NUM);
    #endif /* (USBCOMP_EP1_ISR_ACTIVE) */

    #if (USBCOMP_EP2_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_2_VECT_NUM);
    #endif /* (USBCOMP_EP5_ISR_ACTIVE) */

    #if (USBCOMP_EP3_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_3_VECT_NUM);
    #endif /* (USBCOMP_EP5_ISR_ACTIVE) */

    #if (USBCOMP_EP4_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_4_VECT_NUM);
    #endif /* (USBCOMP_EP5_ISR_ACTIVE) */

    #if (USBCOMP_EP5_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_5_VECT_NUM);
    #endif /* (USBCOMP_EP5_ISR_ACTIVE) */

    #if (USBCOMP_EP6_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_6_VECT_NUM);
    #endif /* USBCOMP_EP6_ISR_REMOVE */

    #if (USBCOMP_EP7_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_7_VECT_NUM);
    #endif /* (USBCOMP_EP7_ISR_ACTIVE) */

    #if (USBCOMP_EP8_ISR_ACTIVE)
        CyIntEnable(USBCOMP_EP_8_VECT_NUM);
    #endif /* (USBCOMP_EP8_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */
    ...
    USBCOMP_ARB_INT_EN_REG = USBCOMP_DEFAULT_ARB_INT_EN;
        ...
        CyIntEnable(USBCOMP_ARB_VECT_NUM);
    ...
    USBCOMP_ARB_CFG_REG = USBCOMP_DEFAULT_ARB_CFG;
        ...
        CyDmaEnable();
            ...
            /* Change DMA priority to be highest. */
             CyIntSetPriority(CYDMA_INTR_NUMBER, USBCOMP_DMA_AUTO_INTR_PRIO);
        #endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (CY_PSOC4) */
            ...
            CyIntEnable(CYDMA_INTR_NUMBER);
                ...
                USBCOMP_EP_DMA_Done_isr_StartEx(&USBCOMP_EP_DMA_DONE_ISR);
                USBCOMP_EP17_DMA_Done_SR_InterruptEnable();
                USBCOMP_EP8_DMA_Done_SR_InterruptEnable();
            #endif /* (USBCOMP_EP_DMA_AUTO_OPT == 0u) */
        #endif /* (CY_PSOC4) */
    #endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO) */
#endif /* (USBCOMP_EP_MANAGEMENT_DMA) */
...
/*******************************************************************************
* Function Name: USBCOMP_ReInitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   USBCOMP_device - Contains the device number of the desired Device
...
*   USBCOMP_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
*   \ref USBCOMP_deviceStatus
*
*   \ref USBCOMP_configurationChanged
*
*   USBCOMP_intiVar -  This variable is set to zero
*
*******************************************************************************/
void USBCOMP_Stop(void) 
{
    ...
    USBCOMP_CR0_REG &= (uint32) ~USBCOMP_CR0_ENABLE;

    /* Disable D+ pull-up. */
    USBCOMP_USBIO_CR1_REG &= (uint32) ~ USBCOMP_USBIO_CR1_USBPUEN;

#if (CY_PSOC4)
    /* Disable USBFS block. */
    USBCOMP_POWER_CTRL_REG &= (uint32) ~USBCOMP_POWER_CTRL_ENABLE;
#else
...
#if (CY_PSOC4)
    CyIntDisable(USBCOMP_INTR_HI_VECT_NUM);
    CyIntDisable(USBCOMP_INTR_LO_VECT_NUM);
    CyIntDisable(USBCOMP_INTR_MED_VECT_NUM);
#else

    CyIntDisable(USBCOMP_BUS_RESET_VECT_NUM);
    CyIntDisable(USBCOMP_EP_0_VECT_NUM);

    #if (USBCOMP_SOF_ISR_ACTIVE)
        CyIntDisable(USBCOMP_SOF_VECT_NUM);
    #endif /* (USBCOMP_SOF_ISR_ACTIVE) */

    #if (USBCOMP_EP1_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_1_VECT_NUM);
    #endif /* (USBCOMP_EP1_ISR_ACTIVE) */

    #if (USBCOMP_EP2_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_2_VECT_NUM);
    #endif /* (USBCOMP_EP2_ISR_ACTIVE) */

    #if (USBCOMP_EP3_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_3_VECT_NUM);
    #endif /* (USBCOMP_EP3_ISR_ACTIVE) */

    #if (USBCOMP_EP4_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_4_VECT_NUM);
    #endif /* (USBCOMP_EP4_ISR_ACTIVE) */

    #if (USBCOMP_EP5_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_5_VECT_NUM);
    #endif /* (USBCOMP_EP5_ISR_ACTIVE) */

    #if (USBCOMP_EP6_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_6_VECT_NUM);
    #endif /* USBCOMP_EP6_ISR_REMOVE */

    #if (USBCOMP_EP7_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_7_VECT_NUM);
    #endif /* (USBCOMP_EP7_ISR_ACTIVE) */

    #if (USBCOMP_EP8_ISR_ACTIVE)
        CyIntDisable(USBCOMP_EP_8_VECT_NUM);
    #endif /* (USBCOMP_EP8_ISR_ACTIVE) */
...
/*******************************************************************************
* Function Name: USBCOMP_CheckActivity
****************************************************************************//**
*
...
{
    uint32 cr1Reg = USBCOMP_CR1_REG;

    /* Clear bus activity. */
    USBCOMP_CR1_REG = (cr1Reg & (uint32) ~USBCOMP_CR1_BUS_ACTIVITY);

    /* Get bus activity. */
    return ((0u != (cr1Reg & USBCOMP_CR1_BUS_ACTIVITY)) ? (1u) : (0u));
}


/*******************************************************************************
* Function Name: USBCOMP_GetConfiguration
****************************************************************************//**
*
...
{
    return (USBCOMP_configuration);
}


/*******************************************************************************
* Function Name: USBCOMP_IsConfigurationChanged
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBCOMP_configurationChanged - This variable is set to 1 after
...
uint8 USBCOMP_IsConfigurationChanged(void) 
{
    ...
    if (USBCOMP_configurationChanged != 0u)
    {
        res = USBCOMP_configurationChanged;
        USBCOMP_configurationChanged = 0u;
    }
...
/*******************************************************************************
* Function Name: USBCOMP_GetInterfaceSetting
****************************************************************************//**
*
...
{
    return (USBCOMP_interfaceSetting[interfaceNumber]);
}


/*******************************************************************************
* Function Name: USBCOMP_GetEPState
****************************************************************************//**
*
...
{
    return (USBCOMP_EP[epNumber].apiEpState);
}


/*******************************************************************************
* Function Name: USBCOMP_GetEPCount
****************************************************************************//**
*
...
uint16 USBCOMP_GetEPCount(uint8 epNumber) 
{
    ...
    if ((epNumber > USBCOMP_EP0) && (epNumber < USBCOMP_MAX_EP))
    {
        ...
        cntr  = ((uint16) USBCOMP_SIE_EP_BASE.sieEp[epNumber].epCnt0) & USBCOMP_EPX_CNT0_MASK;
        cntr  = ((uint16) (cntr << 8u)) | ((uint16) USBCOMP_SIE_EP_BASE.sieEp[epNumber].epCnt1);
        cntr -= USBCOMP_EPX_CNTX_CRC_COUNT;
    }
...
#if (CY_PSOC4)
    /*******************************************************************************
    * Function Name: USBCOMP_InitEpDma
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static void USBCOMP_InitEpDma(void)
    {
        ...
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep1_dma_CHANNEL] = USBCOMP_ep1_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA1_ACTIVE) */

    #if (USBCOMP_DMA2_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep2_dma_CHANNEL] = USBCOMP_ep2_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA2_ACTIVE) */

    #if (USBCOMP_DMA3_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep3_dma_CHANNEL] = USBCOMP_ep3_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA3_ACTIVE) */

    #if (USBCOMP_DMA4_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep4_dma_CHANNEL] = USBCOMP_ep4_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA4_ACTIVE) */

    #if (USBCOMP_DMA5_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep5_dma_CHANNEL] = USBCOMP_ep5_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA5_ACTIVE) */

    #if (USBCOMP_DMA6_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep6_dma_CHANNEL] = USBCOMP_ep6_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA6_ACTIVE) */

    #if (USBCOMP_DMA7_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep7_dma_CHANNEL] = USBCOMP_ep7_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA7_ACTIVE) */

    #if (USBCOMP_DMA8_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBCOMP_ep8_dma_CHANNEL] = USBCOMP_ep8_dma_CHANNEL_CFG;
    #endif /* (USBCOMP_DMA8_ACTIVE) */

    #if (USBCOMP_EP_MANAGEMENT_DMA_AUTO)
        /* Initialize DMA channel callbacks. */
        #if (USBCOMP_DMA1_ACTIVE)
            (void) USBCOMP_ep1_dma_SetInterruptCallback(&USBCOMP_EP1_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA1_ACTIVE) */

        #if (USBCOMP_DMA2_ACTIVE)
            (void) USBCOMP_ep2_dma_SetInterruptCallback(&USBCOMP_EP2_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA2_ACTIVE) */

        #if (USBCOMP_DMA3_ACTIVE)
            (void) USBCOMP_ep3_dma_SetInterruptCallback(&USBCOMP_EP3_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA3_ACTIVE) */

        #if (USBCOMP_DMA4_ACTIVE)
            (void) USBCOMP_ep4_dma_SetInterruptCallback(&USBCOMP_EP4_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA4_ACTIVE) */

        #if (USBCOMP_DMA5_ACTIVE)
            (void) USBCOMP_ep5_dma_SetInterruptCallback(&USBCOMP_EP5_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA5_ACTIVE) */

        #if (USBCOMP_DMA6_ACTIVE)
            (void) USBCOMP_ep6_dma_SetInterruptCallback(&USBCOMP_EP6_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA6_ACTIVE) */

        #if (USBCOMP_DMA7_ACTIVE)
            (void) USBCOMP_ep7_dma_SetInterruptCallback(&USBCOMP_EP7_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA7_ACTIVE) */

        #if (USBCOMP_DMA8_ACTIVE)
            (void) USBCOMP_ep8_dma_SetInterruptCallback(&USBCOMP_EP8_DMA_DONE_ISR);
        #endif /* (USBCOMP_DMA8_ACTIVE) */
    #endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO) */
    }
#else
    ...
    /***************************************************************************
    * Function Name: USBCOMP_InitEP_DMA
    ************************************************************************//**
    *
    *  This function allocates and initializes a DMA channel to be used by the
    *  USBCOMP_LoadInEP() or USBCOMP_ReadOutEP() APIs for data
    ...
    ***************************************************************************/
                                                                    ...
                                                                    
    {
    ...
    #if (USBCOMP_DMA1_ACTIVE)
        case USBCOMP_EP1:
            USBCOMP_DmaChan[epNumber] = USBCOMP_ep1_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                    USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA1_ACTIVE) */

    #if (USBCOMP_DMA2_ACTIVE)
        case USBCOMP_EP2:
            USBCOMP_DmaChan[epNumber] = USBCOMP_ep2_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                    USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA2_ACTIVE) */
    ...
    #if (USBCOMP_DMA3_ACTIVE)
            case USBCOMP_EP3:
                USBCOMP_DmaChan[epNumber] = USBCOMP_ep3_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                        USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA3_ACTIVE) */
    ...
    #if (USBCOMP_DMA4_ACTIVE)
            case USBCOMP_EP4:
                USBCOMP_DmaChan[epNumber] = USBCOMP_ep4_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                        USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA4_ACTIVE) */
    ...
    #if (USBCOMP_DMA5_ACTIVE)
            case USBCOMP_EP5:
                USBCOMP_DmaChan[epNumber] = USBCOMP_ep5_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                        USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA5_ACTIVE) */
    ...
    #if (USBCOMP_DMA6_ACTIVE)
        case USBCOMP_EP6:
            USBCOMP_DmaChan[epNumber] = USBCOMP_ep6_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                    USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA6_ACTIVE) */
    ...
    #if (USBCOMP_DMA7_ACTIVE)
        case USBCOMP_EP7:
                USBCOMP_DmaChan[epNumber] = USBCOMP_ep7_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                        USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA7_ACTIVE) */
    ...
    #if (USBCOMP_DMA8_ACTIVE)
        case USBCOMP_EP8:
                USBCOMP_DmaChan[epNumber] = USBCOMP_ep8_DmaInitialize(USBCOMP_DMA_BYTES_PER_BURST,
                                                                                        USBCOMP_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBCOMP_DMA8_ACTIVE) */
        ...
        if ((epNumber > USBCOMP_EP0) && (epNumber < USBCOMP_MAX_EP))
        {
            USBCOMP_DmaTd[epNumber] = CyDmaTdAllocate();

        #if (USBCOMP_EP_MANAGEMENT_DMA_AUTO && (USBCOMP_EP_DMA_AUTO_OPT == 0u))
            USBCOMP_DmaNextTd[epNumber] = CyDmaTdAllocate();
        #endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO && (USBCOMP_EP_DMA_AUTO_OPT == 0u)) */
        }
    }
#endif /* (CY_PSOC4) */
    ...
    /***************************************************************************
    * Function Name: USBCOMP_Stop_DMA
    ************************************************************************//**
    *  
    ...
    *  to trigger DMA re-configuration when USBCOMP_LoadInEP() or 
    *  USBCOMP_ReadOutEP() functions are called the first time. 
    *  
    ...
    ***************************************************************************/
    void USBCOMP_Stop_DMA(uint8 epNumber) 
    {
        ...
        i = (epNumber < USBCOMP_MAX_EP) ? epNumber : USBCOMP_EP1;

        do
        {
        #if (CY_PSOC4)
            if (0u != USBCOMP_DmaChan[i])
            {
                USBCOMP_CyDmaChDisable(USBCOMP_DmaChan[i]);
            }
        #else
            if(USBCOMP_DmaTd[i] != DMA_INVALID_TD)
            {
                (void) CyDmaChDisable(USBCOMP_DmaChan[i]);
                CyDmaTdFree(USBCOMP_DmaTd[i]);
                USBCOMP_DmaTd[i] = DMA_INVALID_TD;
            }

            #if (USBCOMP_EP_MANAGEMENT_DMA_AUTO && (USBCOMP_EP_DMA_AUTO_OPT == 0u))
                if(USBCOMP_DmaNextTd[i] != DMA_INVALID_TD)
                {
                    CyDmaTdFree(USBCOMP_DmaNextTd[i]);
                    USBCOMP_DmaNextTd[i] = DMA_INVALID_TD;
                }
            #endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO && (USBCOMP_EP_DMA_AUTO_OPT == 0u)) */
        ...
        while ((i < USBCOMP_MAX_EP) && (epNumber == USBCOMP_MAX_EP));
    }
#endif /* (USBCOMP_EP_MANAGEMENT_DMA) */
...
#if (USBCOMP_EP_MANAGEMENT_DMA_AUTO && (USBCOMP_EP_DMA_AUTO_OPT == 0u))
    /***************************************************************************
    * Function Name: USBCOMP_LoadNextInEP
    ************************************************************************//**
    *
    * Summary:
    ...
    ***************************************************************************/
    void USBCOMP_LoadNextInEP(uint8 epNumber, uint8 mode) 
    {
        ...
        if (mode == 0u)
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBCOMP_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBCOMP_inLength[epNumber] - USBCOMP_DMA_BYTES_PER_BURST);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBCOMP_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert, LO16((uint32)USBCOMP_inDataPointer[epNumber] +
                                            USBCOMP_DMA_BYTES_PER_BURST));
            USBCOMP_inBufFull[epNumber] = 1u;
        }
        else
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBCOMP_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBCOMP_DMA_BYTES_REPEAT);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBCOMP_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert,  LO16((uint32)USBCOMP_inDataPointer[epNumber] +
                                   (USBCOMP_DMA_BYTES_PER_BURST - USBCOMP_DMA_BYTES_REPEAT)));
        }

        /* CyDmaChSetInitialTd API is optimized to initialize TD. */
        CY_DMA_CH_STRUCT_PTR[USBCOMP_DmaChan[epNumber]].basic_status[1u] = USBCOMP_DmaTd[epNumber];
    }
#endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO && (USBCOMP_EP_DMA_AUTO_OPT == 0u)) */
#endif /* (CY_PSOC3 || CY_PSOC5) */
...
/*******************************************************************************
* Function Name: USBCOMP_LoadInEP
****************************************************************************//**
*
                                                                        ...
                                                                        
{
    if ((epNumber > USBCOMP_EP0) && (epNumber < USBCOMP_MAX_EP))
    {
        ...
        if (length > (USBCOMP_EPX_DATA_BUF_MAX - USBCOMP_EP[epNumber].buffOffset))
        {
            length = USBCOMP_EPX_DATA_BUF_MAX - USBCOMP_EP[epNumber].buffOffset;
        }
    #endif /* (!USBCOMP_EP_MANAGEMENT_DMA_AUTO) */

        /* Set count and data toggle. */
        USBCOMP_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint32) HI8(length) | USBCOMP_EP[epNumber].epToggle;
        USBCOMP_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint32) LO8(length);

    #if (USBCOMP_EP_MANAGEMENT_MANUAL)
        if (NULL != pData)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr = pData[i];
            }
        }

        /* IN endpoint buffer is full - read to be read. */
        USBCOMP_EP[epNumber].apiEpState = USBCOMP_NO_EVENT_PENDING;

        /* Arm IN endpoint. */
        USBCOMP_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBCOMP_EP[epNumber].epMode;

    #else

        #if (CY_PSOC3 || CY_PSOC5LP)
            /* Initialize DMA if it was not initialized. */
            if (DMA_INVALID_TD == USBCOMP_DmaTd[epNumber])
            {
                USBCOMP_InitEP_DMA(epNumber, pData);
            }
            ...
            {
            #if (CY_PSOC4)
                uint32 channelNum = (uint32) USBCOMP_DmaChan[epNumber];

                /* Disable DMA channel: start configuration. */
                USBCOMP_CyDmaChDisable(channelNum);

                /* Configure source and destination. */
                USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) pData);
                USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                --length;
                USBCOMP_CyDmaSetConfiguration(channelNum, USBCOMP_DMA_DESCR0, USBCOMP_DMA_COMMON_CFG | length |
                ...
                (void) CyDmaChDisable(USBCOMP_DmaChan[epNumber]);
                (void) CyDmaTdSetConfiguration(USBCOMP_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USBCOMP_DmaTd[epNumber], LO16((uint32) pData), LO16((uint32) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr));

                /* Enable DMA channel. */
                (void) CyDmaChSetInitialTd(USBCOMP_DmaChan[epNumber], USBCOMP_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBCOMP_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */

                /* Generate DMA request. */
                USBCOMP_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint32)  USBCOMP_ARB_EPX_CFG_DMA_REQ;
                USBCOMP_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint32) ~USBCOMP_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                USBCOMP_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
                ...
                USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr);
                USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR1, (void*) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                USBCOMP_CyDmaSetConfiguration(channelNum, USBCOMP_DMA_DESCR0, USBCOMP_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Configure DMA descriptor. */
                USBCOMP_CyDmaSetConfiguration(channelNum, USBCOMP_DMA_DESCR1, USBCOMP_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Enable interrupt from DMA channel. */
                USBCOMP_CyDmaSetInterruptMask(channelNum);
                ...
                (void) CyDmaChDisable(USBCOMP_DmaChan[epNumber]);
                ...
                #if (USBCOMP_EP_DMA_AUTO_OPT == 0u)
                    USBCOMP_inLength[epNumber] = length;
                    USBCOMP_inDataPointer[epNumber] = pData;

                    /* Configure DMA to send data only for first burst */
                    (void) CyDmaTdSetConfiguration(USBCOMP_DmaTd[epNumber],
                        (length > USBCOMP_DMA_BYTES_PER_BURST) ? USBCOMP_DMA_BYTES_PER_BURST : length,
                        USBCOMP_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBCOMP_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    ...
                    */
                    (void) CyDmaTdSetConfiguration(USBCOMP_DmaNextTd[epNumber], 1u,
                                                   USBCOMP_DmaNextTd[epNumber],
                                                   USBCOMP_epX_TD_TERMOUT_EN[epNumber]);

                    /* Configure DmaNextTd to clear Data Ready status. */
                    (void) CyDmaTdSetAddress(USBCOMP_DmaNextTd[epNumber], LO16((uint32) &clearInDataRdyStatus),
                                                                                   LO16((uint32) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].epCfg));
                #else
                    /* Configure DMA to send all data. */
                    (void) CyDmaTdSetConfiguration(USBCOMP_DmaTd[epNumber], length,
                                                   USBCOMP_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBCOMP_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr));
                #endif /* (USBCOMP_EP_DMA_AUTO_OPT == 0u) */

                /* Clear any potential pending DMA requests before starting DMA channel to transfer data. */
                (void) CyDmaClearPendingDrq(USBCOMP_DmaChan[epNumber]);
                /* Enable DMA. */
                (void) CyDmaChSetInitialTd(USBCOMP_DmaChan[epNumber], USBCOMP_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBCOMP_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    USBCOMP_DmaEpBurstCnt[epNumber] = (uint8) (length / USBCOMP_DMA_BYTES_PER_BURST);

                    /* Get number of elements in the last burst. */
                    USBCOMP_DmaEpLastBurstEl[epNumber] = (uint8) (length % USBCOMP_DMA_BYTES_PER_BURST);

                    /* Get total number of bursts. */
                    USBCOMP_DmaEpBurstCnt[epNumber] += (0u != USBCOMP_DmaEpLastBurstEl[epNumber]) ? 1u : 0u;

                    /* Adjust number of data elements transferred in last burst. */
                    USBCOMP_DmaEpLastBurstEl[epNumber] = (0u != USBCOMP_DmaEpLastBurstEl[epNumber]) ?
                                                                          (USBCOMP_DmaEpLastBurstEl[epNumber] - 1u) :
                                                                          (USBCOMP_DMA_BYTES_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBCOMP_DmaEpBurstCnt[epNumber]) ? USBCOMP_DmaEpLastBurstEl[epNumber] : (USBCOMP_DMA_BYTES_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBCOMP_DmaEpBurstCnt[epNumber]) ? USBCOMP_DmaEpLastBurstEl[epNumber] : (USBCOMP_DMA_BYTES_PER_BURST - 1u);


                    /* Mark which descriptor is last one to execute. */
                    USBCOMP_DmaEpLastBurstEl[epNumber] |= (0u != (USBCOMP_DmaEpBurstCnt[epNumber] & 0x1u)) ?
                                                                            USBCOMP_DMA_DESCR0_MASK : USBCOMP_DMA_DESCR1_MASK;

                    /* Restore DMA settings for current transfer. */
                    USBCOMP_CyDmaChDisable(channelNum);

                    /* Restore destination address for input endpoint. */
                    USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) ((uint32) USBCOMP_DmaEpBufferAddrBackup[epNumber]));
                    USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR1, (void*) ((uint32) USBCOMP_DmaEpBufferAddrBackup[epNumber] +
                                                                                                                   USBCOMP_DMA_BYTES_PER_BURST));

                    /* Set number of elements to transfer. */
                    USBCOMP_CyDmaSetNumDataElements(channelNum, USBCOMP_DMA_DESCR0, lengthDescr0);
                    USBCOMP_CyDmaSetNumDataElements(channelNum, USBCOMP_DMA_DESCR1, lengthDescr1);
                    ...
                    if (USBCOMP_DmaEpBurstCnt[epNumber] > 1u)
                    {
                    ...
                    USBCOMP_DmaEpBurstCnt[epNumber] = USBCOMP_DMA_GET_BURST_CNT(USBCOMP_DmaEpBurstCnt[epNumber]);

                    /* Enable DMA channel: configuration complete. */
                    USBCOMP_CyDmaChEnable(channelNum);

                #elif (USBCOMP_EP_DMA_AUTO_OPT == 0u)
                    USBCOMP_inLength[epNumber]  = length;
                    USBCOMP_inBufFull[epNumber] = 0u;

                    (void) CyDmaChDisable(USBCOMP_DmaChan[epNumber]);
                    /* Configure DMA to send data only for first burst. */
                    (void) CyDmaTdSetConfiguration(
                        USBCOMP_DmaTd[epNumber], (length > USBCOMP_DMA_BYTES_PER_BURST) ?
                        USBCOMP_DMA_BYTES_PER_BURST : length,
                        USBCOMP_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(USBCOMP_DmaTd[epNumber], LO16((uint32)  USBCOMP_inDataPointer[epNumber]),
                                                                               LO16((uint32) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    /* Clear Any potential pending DMA requests before starting DMA channel to transfer data. */
                    (void) CyDmaClearPendingDrq(USBCOMP_DmaChan[epNumber]);
                    /* Enable DMA. */
                ...
                #if !defined (USBCOMP_MANUAL_IN_EP_ARM)
                    /* Set IN data ready to generate DMA request to load data into endpoint buffer. */
                    USBCOMP_ARB_EP_BASE.arbEp[epNumber].epCfg |= USBCOMP_ARB_EPX_CFG_IN_DATA_RDY;
                #endif  /* (USBCOMP_MANUAL_IN_EP_ARM) */
        ...
        #endif /* (USBCOMP_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (USBCOMP_EP_MANAGEMENT_MANUAL) */
    }
}


/*******************************************************************************
* Function Name: USBCOMP_ReadOutEP
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
...
*   transfer after calling the USBCOMP_ReadOutEP() API. For example,
*   by checking EPstate:
*
...
{
    if ((pData != NULL) && (epNumber > USBCOMP_EP0) && (epNumber < USBCOMP_MAX_EP))
    {
    #if (!USBCOMP_EP_MANAGEMENT_DMA_AUTO)
        /* Adjust requested length to available data. */
        length = (length > USBCOMP_GetEPCount(epNumber)) ? USBCOMP_GetEPCount(epNumber) : length;
    #endif /* (!USBCOMP_EP_MANAGEMENT_DMA_AUTO) */
    ...
    #if (USBCOMP_EP_MANAGEMENT_MANUAL)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                pData[i] = (uint8) USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr;
            }
        }
        ...
        #if (CY_PSOC4)
        {
            ...
            /* Configure source and destination. */
            USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBCOMP_CyDmaSetConfiguration(channelNum, USBCOMP_DMA_DESCR0, USBCOMP_DMA_COMMON_CFG | (uint16)(length - 1u) |
            ...
            /* Configure DMA to transfer data. */
            (void) CyDmaChDisable(USBCOMP_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBCOMP_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBCOMP_DmaTd[epNumber], LO16((uint32) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32)pData));
        ...
        #if (CY_PSOC4)
        {
            ...
            USBCOMP_DmaEpLastBurstEl[epNumber] |= (USBCOMP_DmaEpBurstCnt[epNumber] > 2u) ? USBCOMP_DMA_DESCR_REVERT : 0u;
            ...
            USBCOMP_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
            USBCOMP_DmaEpBurstCntBackup[epNumber]   = USBCOMP_DmaEpBurstCnt[epNumber];
            ...
            /* Set destination address. */
            USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR1, (void*) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr);

            USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) ((uint32) pData));
            USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR1, (void*) ((uint32) pData + USBCOMP_DMA_BYTES_PER_BURST));

            /* Configure DMA descriptor. */
            USBCOMP_CyDmaSetConfiguration(channelNum, USBCOMP_DMA_DESCR0, USBCOMP_DMA_COMMON_CFG  | lengthDescr0 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

            /* Configure DMA descriptor. */
            USBCOMP_CyDmaSetConfiguration(channelNum, USBCOMP_DMA_DESCR1, USBCOMP_DMA_COMMON_CFG  | lengthDescr1 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);
            ...
            if (USBCOMP_DmaEpBurstCntBackup[epNumber] > 1u)
            {
                USBCOMP_CyDmaValidateDescriptor(channelNum, USBCOMP_DMA_DESCR1);
            }
        ...
        #else
            (void) CyDmaChDisable(USBCOMP_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBCOMP_DmaTd[epNumber], length,  USBCOMP_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBCOMP_DmaTd[epNumber], LO16((uint32) &USBCOMP_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32) pData));
...
/*******************************************************************************
* Function Name: USBCOMP_LoadInEP16
****************************************************************************//**
*
*  This function performs different functionality depending on the Component’s
*  configured Endpoint Buffer Management. This parameter is defined in
...
void USBCOMP_LoadInEP16(uint8 epNumber, const uint8 pData[], uint16 length)
{
    ...
    if ((epNumber > USBCOMP_EP0) && (epNumber < USBCOMP_MAX_EP))
    {
        ...
        if (NULL != pData)
        {
            ...
            {
                USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16 = dataBuf[i];
            }
        }
        ...
        USBCOMP_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBCOMP_EP[epNumber].epMode;
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                /* Configure source and destination. */
                USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) pData);
                USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            if (pData != NULL)
            {
                ...
                /* Set destination address. */
                USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR1, (void*) &USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    /* Adjust number of data elements transferred in last burst. */
                    USBCOMP_DmaEpLastBurstEl[epNumber] = (0u != USBCOMP_DmaEpLastBurstEl[epNumber]) ?
                                                                          ((USBCOMP_DmaEpLastBurstEl[epNumber] >> 1u) - 1u) :
                                                                           (USBCOMP_DMA_HALFWORDS_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBCOMP_DmaEpBurstCnt[epNumber]) ? USBCOMP_DmaEpLastBurstEl[epNumber] : (USBCOMP_DMA_HALFWORDS_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBCOMP_DmaEpBurstCnt[epNumber]) ? USBCOMP_DmaEpLastBurstEl[epNumber] : (USBCOMP_DMA_HALFWORDS_PER_BURST - 1u);
...
/*******************************************************************************
* Function Name: USBCOMP_ReadOutEP16
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
*   configured Endpoint Buffer Management. This parameter is defined in the
...
uint16 USBCOMP_ReadOutEP16(uint8 epNumber, uint8 pData[], uint16 length)
{
    ...
    adjLength =  length + ((uint32)length & 1u);
    ...
    #if (USBCOMP_EP_MANAGEMENT_MANUAL)
        {
            ...
            {
                dataBuf[i] = (uint16) USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16;
            }
        }
        ...
        #if (USBCOMP_EP_MANAGEMENT_DMA_MANUAL)
        {
            ...
            /* Configure source and destination. */
            USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBCOMP_CyDmaSetDstAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBCOMP_CyDmaSetConfiguration(channelNum, USBCOMP_DMA_DESCR0, USBCOMP_DMA_COMMON_CFG | (uint16)((adjLength >> 1u) - 1u) |
        ...
        #if (USBCOMP_EP_MANAGEMENT_DMA_AUTO)
        {
            ...
            /* Get number of full bursts. */
            USBCOMP_DmaEpBurstCnt[epNumber] = (uint8) (adjLength / USBCOMP_DMA_BYTES_PER_BURST);

            /* Get number of elements in last burst. */
            USBCOMP_DmaEpLastBurstEl[epNumber] = (uint8) (adjLength % USBCOMP_DMA_BYTES_PER_BURST);
            ...
            USBCOMP_DmaEpLastBurstEl[epNumber] |= USBCOMP_DMA_DESCR_16BITS;
            ...
            /* Set destination address. */
            USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR0, (void*) &USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBCOMP_CyDmaSetSrcAddress(channelNum, USBCOMP_DMA_DESCR1, (void*) &USBCOMP_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
...
/*******************************************************************************
* Function Name: USBCOMP_EnableOutEP
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBCOMP_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* \reentrant
*  No.
*
...
/*******************************************************************************
* Function Name: USBCOMP_DisableOutEP
****************************************************************************//**
*
...
*    State                      |Description
*    ---------------------------|----------------------------------------------
*    USBCOMP_FORCE_J   | Force a J State onto the D+/D– lines
*    USBCOMP_FORCE_K   | Force a K State onto the D+/D– lines
*    USBCOMP_FORCE_SE0 | Force a Single Ended 0 onto the D+/D– lines
*    USBCOMP_FORCE_NONE| Return bus to SIE control
*
*
*******************************************************************************/
void USBCOMP_Force(uint8 bState) 
{
    ...
    */
    USBCOMP_USBIO_CR0_REG = bState;
}


/*******************************************************************************
* Function Name: USBCOMP_GetEPAckState
****************************************************************************//**
*
...
uint8 USBCOMP_GetEPAckState(uint8 epNumber) 
{
    ...
    if ((epNumber > USBCOMP_EP0) && (epNumber < USBCOMP_MAX_EP))
    {
        cr = USBCOMP_SIE_EP_BASE.sieEp[epNumber].epCr0 & USBCOMP_MODE_ACKD;
    }
...
/*******************************************************************************
* Function Name: USBCOMP_SetPowerStatus
****************************************************************************//**
*
...
*  --------------------------------------------|---------------------------
*  USBCOMP_DEVICE_STATUS_BUS_POWERED  | Set the device to bus powered
*  USBCOMP_DEVICE_STATUS_SELF_POWERED | Set the device to self powered
*
* \globalvars
*
*  \ref USBCOMP_deviceStatus - set power status
*
* \reentrant
*  No.
*
...
{
    if (powerStatus != USBCOMP_DEVICE_STATUS_BUS_POWERED)
    {
    ...
    /***************************************************************************
    * Function Name: USBCOMP_VBusPresent
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return ((0u != (USBCOMP_VBUS_STATUS_REG & USBCOMP_VBUS_VALID)) ? (uint8) 1u : (uint8) 0u);
    }
#endif /* (USBCOMP_VBUS_MONITORING_ENABLE) */


/*******************************************************************************
* Function Name: USBCOMP_RWUEnabled
****************************************************************************//**
*
...
*
* \globalvars
*  USBCOMP_deviceStatus - checked to determine remote status
*
*******************************************************************************/
uint8 USBCOMP_RWUEnabled(void) 
{
    uint8 result = USBCOMP_FALSE;

    if (0u != (USBCOMP_deviceStatus & USBCOMP_DEVICE_STATUS_REMOTE_WAKEUP))
    {
        result = USBCOMP_TRUE;
    }
...
/*******************************************************************************
* Function Name: USBCOMP_GetDeviceAddress
****************************************************************************//**
*
...
{
    return (uint8)(USBCOMP_CR0_REG & USBCOMP_CR0_DEVICE_ADDRESS_MASK);
}


/*******************************************************************************
* Function Name: USBCOMP_EnableSofInt
****************************************************************************//**
*
...
void USBCOMP_EnableSofInt(void) 
{
    ...
    USBCOMP_INTR_SIE_MASK_REG |= (uint32) USBCOMP_INTR_SIE_SOF_INTR;
#else
...
/*******************************************************************************
* Function Name: USBCOMP_DisableSofInt
****************************************************************************//**
*
...
void USBCOMP_DisableSofInt(void) 
{
    ...
    /***************************************************************************
    * Function Name: USBCOMP_DetectPortType
    ************************************************************************//**
    *
    ...
    *   Return Value                      |Description
    *   ----------------------------------|-------------------------------------
    *   USBCOMP_BCD_PORT_SDP     | Standard downstream port detected
    *   USBCOMP_BCD_PORT_CDP     | Charging downstream port detected
    *   USBCOMP_BCD_PORT_DCP     | Dedicated charging port detected
    *   USBCOMP_BCD_PORT_UNKNOWN | Unable to detect charging port type (proprietary charger type)
    *   USBCOMP_BCD_PORT_ERR     | Error condition in detection process
    *
    *
    * \sideeffects
    *
    ...
    ***************************************************************************/
    uint8 USBCOMP_Bcd_DetectPortType(void)
    {
        ...
        uint8 result = USBCOMP_BCD_PORT_UNKNOWN;
        ...
        */
        bkPwrCtrl = USBCOMP_POWER_CTRL_REG;
        USBCOMP_POWER_CTRL_REG = USBCOMP_DEFAULT_POWER_CTRL_VBUS\
                            & (~USBCOMP_POWER_CTRL_ENABLE_VBUS_PULLDOWN)\
                            & (~USBCOMP_POWER_CTRL_ENABLE_DM_PULLDOWN);


        /* Enable PHY detector and single-ended and differential receivers.
         * Enable charger detection.  */
        USBCOMP_POWER_CTRL_REG |= USBCOMP_DEFAULT_POWER_CTRL_PHY\
                                         | USBCOMP_POWER_CTRL_ENABLE_CHGDET;
        ...
        */
        #if defined (USBCOMP_NO_DCD)
        CyDelay(USBCOMP_BCD_TIMEOUT);
        #else
        /* DCD implementation:*/

        {
            uint16 timeout = USBCOMP_BCD_TIMEOUT;
            uint8 connectionApproved = 0u;
            uint8 connected = 0u;

            /*   BCD spec 1.2: Turns on Idp_src and D- pull-down resistor */
            USBCOMP_POWER_CTRL_REG |= USBCOMP_POWER_CTRL_ENABLE_DM_PULLDOWN;
            USBCOMP_CHGDET_CTRL_REG |= USBCOMP_CHGDET_CTRL_DCD_SRC_EN;

            /* BCD spec 1.2: Waits for D+ to be low for a time of Tdcd_dbnc*/
            while ((0u != timeout) && (0u == connectionApproved))
            {
                if (0u == (USBCOMP_USBIO_CR1_REG & USBCOMP_USBIO_CR1_DP0))
                {
                    connected++;
                }
                else
                ...
                connectionApproved = (USBCOMP_BCD_TDCD_DBNC < connected) ? 1u:0u;
                CyDelay(1u);
                timeout--;
            }
        ...
        #endif /*(USBCOMP_NO_DCD)*/

        /* Primary detection: enable VDP_SRC on D+ and IDM_SINK on D-. */
        USBCOMP_CHGDET_CTRL_REG = USBCOMP_CHGDET_CTRL_PRIMARY;
        CyDelay(USBCOMP_BCD_PRIMARY_WAIT);
        cr1RegVal = USBCOMP_USBIO_CR1_REG;

        /* Check is it SDP or DCP/CDP, read comparator 2 output. */
        if (0u == (USBCOMP_CHGDET_CTRL_REG & USBCOMP_CHGDET_CTRL_COMP_OUT))
        {
            /* Check status of D- line. */
            if (0u == (cr1RegVal & USBCOMP_USBIO_CR1_DM0))
            {
                result = USBCOMP_BCD_PORT_SDP;
            }
            else
            {
        ...
        {
            USBCOMP_CHGDET_CTRL_REG = USBCOMP_CHGDET_CTRL_DEFAULT \
                                                | USBCOMP_CHGDET_CTRL_SECONDARY;
            CyDelay(USBCOMP_BCD_SECONDARY_WAIT);
            cr1RegVal = USBCOMP_USBIO_CR1_REG;
            ...
            {
                /* Check status of D+ line. */
                if (0u == (cr1RegVal & USBCOMP_USBIO_CR1_DP0))
                {
                    result = USBCOMP_BCD_PORT_CDP;
                }
                else
                {
                    /* ERROR: such combination is impossible. Abort charger
            ...
            else
            {
                ...
                else
                {
                    ...
                    */
                    result = USBCOMP_BCD_PORT_UNKNOWN;
                }
            }
        }

        /* Restore CHGDET_CTRL. */
        USBCOMP_CHGDET_CTRL_REG = 0u;

        /*Revert registers back*/
        USBCOMP_POWER_CTRL_REG = bkPwrCtrl;
        USBCOMP_USBIO_CR1_REG |= (uint32) USBCOMP_USBIO_CR1_IOMODE;
        USBCOMP_USB_CLK_EN_REG = ~USBCOMP_USB_CLK_CSR_CLK_EN;
...
#if (USBCOMP_LPM_ACTIVE)
    /***************************************************************************
    * Function Name: USBCOMP_Lpm_GetBeslValue
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBCOMP_LPM_STAT_REG & USBCOMP_LPM_STAT_LPM_BESL_MASK);
    }


    /***************************************************************************
    * Function Name: USBCOMP_Lpm_RemoteWakeUpAllowed
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBCOMP_LPM_STAT_REG & USBCOMP_LPM_STAT_LPM_REMOTE_WAKE);
    }


    /***************************************************************************
    * Function Name: USBCOMP_Lpm_SetResponse
    ************************************************************************//**
    *
    ...
    *       - USBCOMP_LPM_REQ_ACK - next LPM request will be
    *                                           responded with ACK
    *       - USBCOMP_LPM_REQ_NACK - next LPM request will be
    *                                           responded with NACK
    *       - USBCOMP_LPM_REQ_NYET - next LPM request will be
    *                                           responded with NYET
    *
    ***************************************************************************/
    void USBCOMP_Lpm_SetResponse(uint32 response)
    {
        uint32 lpmCtrl = USBCOMP_LPM_CTRL_REG & (uint32) ~USBCOMP_LPM_CTRL_ACK_NYET_MASK;

        USBCOMP_LPM_CTRL_REG = lpmCtrl | ((uint32) response & USBCOMP_LPM_CTRL_ACK_NYET_MASK);
    }


    /***************************************************************************
    * Function Name: USBCOMP_Lpm_GetResponse
    ************************************************************************//**
    *
    ...
    *       - USBCOMP_LPM_REQ_ACK - next LPM request will be responded
    *                                        with ACK
    *       - USBCOMP_LPM_REQ_NACK - next LPM request will be responded
    *                                        with NACK
    *       - USBCOMP_LPM_REQ_NYET - next LPM request will be responded
    *                                        with NYET
    *
    ***************************************************************************/
    uint32 USBCOMP_Lpm_GetResponse(void)
    {
        ...
        return  ((uint32) USBCOMP_LPM_CTRL_REG & (uint32)USBCOMP_LPM_CTRL_ACK_NYET_MASK);
    }
