...
#if (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF)
    /* The Size of the MIDI messages (MIDI Table 4-1) */
    static const uint8 CYCODE USBTMC_MIDI_SIZE[] = {
...
#endif  /* USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF */
...
***************************************/
...
#if (USBTMC_MIDI_IN_BUFF_SIZE > 0)
    #if (USBTMC_MIDI_IN_BUFF_SIZE >= 256)
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 USBTMC_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 USBTMC_midiInPointer;                             /* Input endpoint buffer pointer */
    #endif /* (USBTMC_MIDI_IN_BUFF_SIZE >= 256) */
    ...
    volatile uint8 USBTMC_midi_in_ep;
    ...
    uint8 USBTMC_midiInBuffer[USBTMC_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
#endif /* (USBTMC_MIDI_IN_BUFF_SIZE > 0) */

#if (USBTMC_MIDI_OUT_BUFF_SIZE > 0)
    ...
    volatile uint8 USBTMC_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 USBTMC_midiOutBuffer[USBTMC_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (USBTMC_MIDI_OUT_BUFF_SIZE > 0) */

#if (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF)

    static USBTMC_MIDI_RX_STATUS USBTMC_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 USBTMC_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 USBTMC_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF)
        static USBTMC_MIDI_RX_STATUS USBTMC_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 USBTMC_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref USBTMC_MIDI1_InqFlags*/
        volatile uint8 USBTMC_MIDI2_InqFlags;                             /* Device inquiry flag */
    #endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF) */
#endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF) */
...
#if (USBTMC_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: USBTMC_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBTMC_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. USBTMC_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref USBTMC_midiOutBuffer: This buffer is used by the
*    USBTMC_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref USBTMC_callbackLocalMidiEvent function.
*
*  \ref USBTMC_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref USBTMC_midi_in_ep: Used as an IN endpoint number.
*
*   \ref USBTMC_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void USBTMC_MIDI_Init(void) 
{
   ...
   USBTMC_midiInPointer = 0u;
        ...
        USBTMC_LoadInEP(USBTMC_midi_in_ep, USBTMC_midiInBuffer,
                                                               USBTMC_MIDI_IN_BUFF_SIZE);
    #endif  /* (USBTMC_MIDI_IN_BUFF_SIZE > 0) */

    #if (USBTMC_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)USBTMC_ReadOutEP(USBTMC_midi_out_ep, USBTMC_midiOutBuffer,
                                                                       USBTMC_MIDI_OUT_BUFF_SIZE);
    #endif /* (USBTMC_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO) */

#if (USBTMC_MIDI_OUT_BUFF_SIZE > 0)
    USBTMC_EnableOutEP(USBTMC_midi_out_ep);
#endif /* (USBTMC_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF)
    USBTMC_MIDI_InitInterface();
#endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF) */
}
...
#if (USBTMC_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBTMC_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref USBTMC_midiOutBuffer: Used as temporary buffer between USB
    ...
    *
    *******************************************************************************/
    void USBTMC_MIDI_OUT_Service(void) 
    {
    #if (USBTMC_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (USBTMC_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (USBTMC_OUT_BUFFER_FULL == USBTMC_GetEPState(USBTMC_midi_out_ep))
        {
        #if (USBTMC_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = USBTMC_GetEPCount(USBTMC_midi_out_ep);
        #else
            outLength = (uint8)USBTMC_GetEPCount(USBTMC_midi_out_ep);
        #endif /* (USBTMC_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!USBTMC_EP_MANAGEMENT_DMA_AUTO)
            #if (USBTMC_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = USBTMC_ReadOutEP(USBTMC_midi_out_ep,
                                                       USBTMC_midiOutBuffer, outLength);
            #else
                outLength = (uint8)USBTMC_ReadOutEP(USBTMC_midi_out_ep,
                                                              USBTMC_midiOutBuffer, (uint16) outLength);
            #endif /* (USBTMC_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (USBTMC_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (USBTMC_OUT_BUFFER_FULL == USBTMC_GetEPState(USBTMC_midi_out_ep))
                {
                }
            ...
            if (outLength >= USBTMC_EVENT_LENGTH)
            {
                ...
                #if (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF)
                    if ((USBTMC_midiOutBuffer[outPointer] & USBTMC_CABLE_MASK) ==
                         USBTMC_MIDI_CABLE_00)
                    {
                        USBTMC_MIDI1_ProcessUsbOut(&USBTMC_midiOutBuffer[outPointer]);
                    }
                    else if ((USBTMC_midiOutBuffer[outPointer] & USBTMC_CABLE_MASK) ==
                             USBTMC_MIDI_CABLE_01)
                    {
                    #if (USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF)
                         USBTMC_MIDI2_ProcessUsbOut(&USBTMC_midiOutBuffer[outPointer]);
                    #endif /*  USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef USBTMC_MIDI_OUT_EP_SERVICE_CALLBACK
                            USBTMC_MIDI_OUT_EP_Service_Callback();
                        #endif /* USBTMC_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    USBTMC_callbackLocalMidiEvent(USBTMC_midiOutBuffer[outPointer] & USBTMC_CABLE_MASK,
                                                            &USBTMC_midiOutBuffer[outPointer + USBTMC_EVENT_BYTE1]);
                    outPointer += USBTMC_EVENT_LENGTH;
                }
            }
...
#if (USBTMC_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBTMC_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  USBTMC_MIDI_IN_Service() and from the
    *  USBTMC_PutUsbMidiIn() function.
    *
    * \globalvars
    *  USBTMC_midi_in_ep: Used as an IN endpoint number.
    *  USBTMC_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   USBTMC_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBTMC_MIDI_IN_EP_Service(void) 
    {
        ...
        if (USBTMC_midiInPointer != 0u)
        {
            if(USBTMC_GetEPState(USBTMC_midi_in_ep) == USBTMC_EVENT_PENDING)
            {
            #if (USBTMC_EP_MANAGEMENT_DMA_AUTO)
                USBTMC_LoadInEP(USBTMC_midi_in_ep, NULL, (uint16)USBTMC_midiInPointer);
            #else
                USBTMC_LoadInEP(USBTMC_midi_in_ep, USBTMC_midiInBuffer,
                                                              (uint16) USBTMC_midiInPointer);
            #endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *
    *******************************************************************************/
    void USBTMC_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == USBTMC_midiInPointer)
        {
            do
            {
                if (USBTMC_midiInPointer <= (USBTMC_MIDI_IN_BUFF_SIZE - USBTMC_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = USBTMC_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        USBTMC_PrepareInBuffer(m1, (uint8 *)&USBTMC_MIDI1_Event.msgBuff[0],
                                                                       USBTMC_MIDI1_Event.size, USBTMC_MIDI_CABLE_00);
                    }
                }
                ...
                if (USBTMC_midiInPointer <= (USBTMC_MIDI_IN_BUFF_SIZE - USBTMC_EVENT_LENGTH))
                {
                    ...
                    m2 = USBTMC_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        USBTMC_PrepareInBuffer(m2, (uint8 *)&USBTMC_MIDI2_Event.msgBuff[0],
                                                                       USBTMC_MIDI2_Event.size, USBTMC_MIDI_CABLE_01);
                    }
                }
            #endif /*  USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF */
            }
            while((USBTMC_midiInPointer <= (USBTMC_MIDI_IN_BUFF_SIZE - USBTMC_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        USBTMC_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: USBTMC_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The USBTMC_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  USBTMC_TRUE  | Host is not ready to receive this message
    *  USBTMC_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref USBTMC_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref USBTMC_midiInPointer: Checked this variable to see if
    ...
    *
    ***************************************************************************/
    uint8 USBTMC_PutUsbMidiIn(uint8 ic, const uint8 midiMsg[], uint8 cable)
                                                                
    {
        uint8 retError = USBTMC_FALSE;
        ...
        if (USBTMC_midiInPointer >
                    (USBTMC_EP[USBTMC_midi_in_ep].bufferSize - USBTMC_EVENT_LENGTH))
        {
            USBTMC_MIDI_IN_EP_Service();
        }
        ...
        if (USBTMC_midiInPointer <=
                    (USBTMC_EP[USBTMC_midi_in_ep].bufferSize - USBTMC_EVENT_LENGTH))
        {
            if((ic < USBTMC_EVENT_LENGTH) || (ic >= USBTMC_MIDI_STATUS_MASK))
            {
                USBTMC_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else /* Only SysEx message is greater than 4 bytes */
            {
                ...
                do
                {
                    ...
                    USBTMC_PrepareInBuffer(USBTMC_MIDI_SYSEX, &midiMsg[idx],
                                                     USBTMC_EVENT_BYTE3, cable);

                    /* Move to next 3 bytes of message */
                    ic  -= USBTMC_EVENT_BYTE3;
                    idx += USBTMC_EVENT_BYTE3;
                        ...
                        (USBTMC_EP[USBTMC_midi_in_ep].bufferSize - USBTMC_EVENT_LENGTH))
                    {
                           ...
                           (USBTMC_EP[USBTMC_midi_in_ep].bufferSize - USBTMC_EVENT_LENGTH))
                        {
                            ...
                            retError = USBTMC_TRUE;
                            break;
                        }
                    }
                }
                while (ic >= USBTMC_EVENT_BYTE3);

                if (retError == USBTMC_FALSE)
                {
                    /* Handle end of message: valid size of messages is 0, 1 and 2 */
                    USBTMC_PrepareInBuffer(USBTMC_MIDI_EOSEX, &midiMsg[idx], ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: USBTMC_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBTMC_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    USBTMC_PutUsbMidiIn() function messages.
    *  USBTMC_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBTMC_PrepareInBuffer(uint8 ic, const uint8 srcBuff[], uint8 eventLen, uint8 cable)
                                                                 
    {
        ...
        if (ic >= (USBTMC_MIDI_STATUS_MASK | USBTMC_MIDI_SINGLE_BYTE_MASK))
        {
            USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_SINGLE_BYTE | cable;
            USBTMC_midiInPointer++;
            USBTMC_midiInBuffer[USBTMC_midiInPointer] = ic;
            USBTMC_midiInPointer++;
            USBTMC_midiInBuffer[USBTMC_midiInPointer] = 0u;
            USBTMC_midiInPointer++;
            USBTMC_midiInBuffer[USBTMC_midiInPointer] = 0u;
            USBTMC_midiInPointer++;
        }
        else if((ic < USBTMC_EVENT_LENGTH) || (ic == USBTMC_MIDI_SYSEX))
        {
            if(ic == USBTMC_MIDI_SYSEX)
            {
                USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_SYSEX | cable;
                USBTMC_midiInPointer++;
            }
            else if (srcBuffZero < USBTMC_MIDI_SYSEX)
            {
                USBTMC_midiInBuffer[USBTMC_midiInPointer] = (srcBuffZero >> 4u) | cable;
                USBTMC_midiInPointer++;
            }
            else if (srcBuffZero == USBTMC_MIDI_TUNEREQ)
            {
                USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_1BYTE_COMMON | cable;
                USBTMC_midiInPointer++;
            }
            else if ((srcBuffZero == USBTMC_MIDI_QFM) || (srcBuffZero == USBTMC_MIDI_SONGSEL))
            {
                USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_2BYTE_COMMON | cable;
                USBTMC_midiInPointer++;
            }
            else if (srcBuffZero == USBTMC_MIDI_SPP)
            {
                USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_3BYTE_COMMON | cable;
                USBTMC_midiInPointer++;
            }
            else
            {
            ...
            USBTMC_midiInBuffer[USBTMC_midiInPointer] = srcBuffZero;
            USBTMC_midiInPointer++;
            USBTMC_midiInBuffer[USBTMC_midiInPointer] = srcBuffOne;
            USBTMC_midiInPointer++;
            USBTMC_midiInBuffer[USBTMC_midiInPointer] = srcBuff[2u];
            USBTMC_midiInPointer++;
        }
        else if (ic == USBTMC_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_SYSEX_ENDS_WITH1 | cable;
                    USBTMC_midiInPointer++;
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_MIDI_EOSEX;
                    USBTMC_midiInPointer++;
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = 0u;
                    USBTMC_midiInPointer++;
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = 0u;
                    ...
                    break;
                case 1u:
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_SYSEX_ENDS_WITH2 | cable;
                    USBTMC_midiInPointer++;
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = srcBuffZero;
                    USBTMC_midiInPointer++;
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_MIDI_EOSEX;
                    ...
                    break;
                case 2u:
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = USBTMC_SYSEX_ENDS_WITH3 | cable;
                    USBTMC_midiInPointer++;
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = srcBuffZero;
                    USBTMC_midiInPointer++;
                    USBTMC_midiInBuffer[USBTMC_midiInPointer] = srcBuffOne;
...
*/
#if (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBTMC_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   USBTMC_MIDI_Event: initialized to zero.
    *   USBTMC_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void USBTMC_MIDI_InitInterface(void) 
    {
        USBTMC_MIDI1_Event.length  = 0u;
        USBTMC_MIDI1_Event.count   = 0u;
        USBTMC_MIDI1_Event.size    = 0u;
        USBTMC_MIDI1_Event.runstat = 0u;
        USBTMC_MIDI1_TxRunStat     = 0u;
        USBTMC_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, USBTMC_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, USBTMC_CUSTOM_UART_RX_PRIOR_NUM);

    #if (USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF)
        USBTMC_MIDI2_Event.length  = 0u;
        USBTMC_MIDI2_Event.count   = 0u;
        USBTMC_MIDI2_Event.size    = 0u;
        USBTMC_MIDI2_Event.runstat = 0u;
        USBTMC_MIDI2_TxRunStat     = 0u;
        USBTMC_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, USBTMC_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, USBTMC_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF) */
    ...
    #ifdef USBTMC_MIDI_INIT_CALLBACK
        USBTMC_MIDI_Init_Callback();
    #endif /* (USBTMC_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: USBTMC_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    uint8 USBTMC_ProcessMidiIn(uint8 mData, USBTMC_MIDI_RX_STATUS *rxStat)
                                                                
    {
        ...
        */
        if ((mData & USBTMC_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & USBTMC_MIDI_STATUS_MASK) == USBTMC_MIDI_STATUS_MASK)
            {
                if ((mData & USBTMC_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case USBTMC_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = USBTMC_MIDI_SYSEX;
                            rxStat->runstat = USBTMC_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBTMC_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = USBTMC_MIDI_EOSEX;
                            break;
                        case USBTMC_MIDI_SPP:
                            rxStat->msgBuff[0u] = USBTMC_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBTMC_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = USBTMC_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBTMC_MIDI_QFM:
                            rxStat->msgBuff[0u] = USBTMC_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBTMC_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = USBTMC_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & USBTMC_MIDI_STATUS_MASK)
                {
                    case USBTMC_MIDI_NOTE_OFF:
                    case USBTMC_MIDI_NOTE_ON:
                    case USBTMC_MIDI_POLY_KEY_PRESSURE:
                    case USBTMC_MIDI_CONTROL_CHANGE:
                    case USBTMC_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case USBTMC_MIDI_PROGRAM_CHANGE:
                    case USBTMC_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == USBTMC_MIDI_SYSEX)
            {
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & USBTMC_MIDI_STATUS_MASK)
                {
                    case USBTMC_MIDI_NOTE_OFF:
                    case USBTMC_MIDI_NOTE_ON:
                    case USBTMC_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: USBTMC_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBTMC_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
    uint8 USBTMC_MIDI1_GetEvent(void) 
    {
        uint8 msgRtn = 0u;
        uint8 rxData;
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBTMC_ProcessMidiIn(rxData,
                                                    (USBTMC_MIDI_RX_STATUS *)&USBTMC_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: USBTMC_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBTMC_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBTMC_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    USBTMC_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    USBTMC_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *      This bit should be cleared by user when Identity Reply message generated.
    *
    *******************************************************************************/
    void USBTMC_MIDI1_ProcessUsbOut(const uint8 epBuf[])
    ...
    #ifdef USBTMC_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBTMC_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (USBTMC_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[USBTMC_EVENT_BYTE0] & USBTMC_CIN_MASK;

        if ((cmd != USBTMC_RESERVED0) && (cmd != USBTMC_RESERVED1))
        {
            len = USBTMC_MIDI_SIZE[cmd];
            i = USBTMC_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == USBTMC_SYSEX)
            {
                if ((epBuf[USBTMC_EVENT_BYTE1] == USBTMC_MIDI_SYSEX) &&
                    (epBuf[USBTMC_EVENT_BYTE2] == USBTMC_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    USBTMC_MIDI1_InqFlags |= USBTMC_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBTMC_MIDI1_InqFlags &= (uint8)~USBTMC_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == USBTMC_SYSEX_ENDS_WITH1)
            {
                USBTMC_MIDI1_InqFlags &= (uint8)~USBTMC_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBTMC_SYSEX_ENDS_WITH2)
            {
                USBTMC_MIDI1_InqFlags &= (uint8)~USBTMC_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBTMC_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((USBTMC_MIDI1_InqFlags & USBTMC_INQ_SYSEX_FLAG) != 0u)
                {
                    USBTMC_MIDI1_InqFlags &= (uint8)~USBTMC_INQ_SYSEX_FLAG;
                    if ((epBuf[USBTMC_EVENT_BYTE1] == USBTMC_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[USBTMC_EVENT_BYTE2] == USBTMC_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBTMC_MIDI1_InqFlags |= USBTMC_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= USBTMC_NOTE_OFF) && (cmd <= USBTMC_PITCH_BEND_CHANGE))
            {
                if (USBTMC_MIDI1_TxRunStat == epBuf[USBTMC_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    USBTMC_MIDI1_TxRunStat = epBuf[USBTMC_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef USBTMC_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        USBTMC_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (USBTMC_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBTMC_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  USBTMC_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
    uint8 USBTMC_MIDI2_GetEvent(void) 
    {
        uint8 msgRtn = 0u;
        uint8 rxData;
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBTMC_ProcessMidiIn(rxData,
                                                    (USBTMC_MIDI_RX_STATUS *)&USBTMC_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: USBTMC_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  USBTMC_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBTMC_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  USBTMC_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  USBTMC_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    *
    *******************************************************************************/
    void USBTMC_MIDI2_ProcessUsbOut(const uint8 epBuf[])
                                                            
    {
    ...
    #ifdef USBTMC_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBTMC_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (USBTMC_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != USBTMC_RESERVED0) && (cmd != USBTMC_RESERVED1))
        {
            ...
            if(cmd == USBTMC_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    USBTMC_MIDI2_InqFlags |= USBTMC_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBTMC_MIDI2_InqFlags &= (uint8)~USBTMC_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == USBTMC_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((USBTMC_MIDI2_InqFlags & USBTMC_INQ_SYSEX_FLAG) != 0u)
                {
                    USBTMC_MIDI2_InqFlags &= (uint8)~USBTMC_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBTMC_MIDI2_InqFlags |= USBTMC_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= USBTMC_NOTE_OFF) && ( cmd <= USBTMC_PITCH_BEND_CHANGE))
            {
                if (USBTMC_MIDI2_TxRunStat == epBuf[USBTMC_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    USBTMC_MIDI2_TxRunStat = epBuf[USBTMC_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef USBTMC_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        USBTMC_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (USBTMC_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_TWO_EXT_INTRF) */
#endif /* (USBTMC_MIDI_EXT_MODE >= USBTMC_ONE_EXT_INTRF) */

#endif  /*  (USBTMC_ENABLE_MIDI_API != 0u) */
...
#endif  /* defined(USBTMC_ENABLE_MIDI_STREAMING) */
