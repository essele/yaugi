...
static USBUART2_BACKUP_STRUCT  USBUART2_backup;

#if (USBUART2_DP_ISR_ACTIVE)
    /*******************************************************************************
    * Function Name: USBUART2_DP_ISR
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    CY_ISR(USBUART2_DP_ISR)
    {
    #ifdef USBUART2_DP_ISR_ENTRY_CALLBACK
        USBUART2_DP_ISR_EntryCallback();
    #endif /* (USBUART2_DP_ISR_ENTRY_CALLBACK) */
	...
	#ifdef USBUART2_DP_ISR_EXIT_CALLBACK
        USBUART2_DP_ISR_ExitCallback();
    #endif /* (USBUART2_DP_ISR_EXIT_CALLBACK) */
    }
#endif /* (USBUART2_DP_ISR_ACTIVE) */


/*******************************************************************************
* Function Name: USBUART2_SaveConfig
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: USBUART2_RestoreConfig
****************************************************************************//**
*
...
void USBUART2_RestoreConfig(void) 
{
    ...
    {
        USBUART2_ConfigReg();
        USBUART2_EpStateInit();
    }
}


/*******************************************************************************
* Function Name: USBUART2_Suspend
****************************************************************************//**
*
...
*
* \globalvars
*  USBUART2_backup.enable:  modified.
*
* \reentrant
*  No.
*
...
void USBUART2_Suspend(void) 
{
    ...
    if (0u != (USBUART2_CR0_REG & USBUART2_CR0_ENABLE))
    {
        /* USB block is enabled. */
        USBUART2_backup.enableState = 1u;
        ...
        USBUART2_POWER_CTRL_REG |= (USBUART2_POWER_CTRL_SUSPEND |
                                            USBUART2_POWER_CTRL_SUSPEND_DEL);

        /* Store state of USB regulator and disable it. */
        USBUART2_backup.mode = (uint8)  (USBUART2_CR1_REG & USBUART2_CR1_REG_ENABLE);
        USBUART2_CR1_REG    &= (uint32) ~USBUART2_CR1_REG_ENABLE;
        
        /* Store SIE interrupt sources. Valid bits are 0 - 4. */
        USBUART2_backup.intrSeiMask = (uint8) USBUART2_INTR_SIE_MASK_REG;
        ...
        /* Disable the SIE. */
        USBUART2_CR0_REG &= (uint8) ~USBUART2_CR0_ENABLE;

        CyDelayUs(USBUART2_WAIT_REG_STABILITY_50NS);  /* ~50ns delay. */
        /* Store mode and disable VRegulator. */
        USBUART2_backup.mode = (uint8) (USBUART2_CR1_REG & USBUART2_CR1_REG_ENABLE);
        USBUART2_CR1_REG    &= (uint8) ~USBUART2_CR1_REG_ENABLE;

        CyDelayUs(USBUART2_WAIT_REG_STABILITY_1US); /* min 0.5us delay required. */

        /* Disable the USBIO reference by setting PM.USB_CR0.fsusbio_ref_en.*/
        USBUART2_PM_USB_CR0_REG &= (uint8) ~USBUART2_PM_USB_CR0_REF_EN;

        /* Switch DP and DM terminals to GPIO mode and disconnect 1.5k pull-up. */
        USBUART2_USBIO_CR1_REG |= USBUART2_USBIO_CR1_IOMODE;
    ...
    {
        USBUART2_backup.enableState = 0u;
    }
    ...
    CyIntEnable      (USBUART2_DP_INTC_VECT_NUM);
#endif /* (USBUART2_DP_ISR_ACTIVE). */
}


/*******************************************************************************
* Function Name: USBUART2_Resume
****************************************************************************//**
*
...
void USBUART2_Resume(void) 
{
    ...
    if (0u != USBUART2_backup.enableState)
    {
    #if (USBUART2_DP_ISR_ACTIVE)
        CyIntDisable(USBUART2_DP_INTC_VECT_NUM);
    #endif /* (USBUART2_DP_ISR_ACTIVE) */
        ...
        /* Restore state of USB regulator and wait until it supples stable power. */
        USBUART2_CR1_REG |= USBUART2_backup.mode;
        CyDelayUs(USBUART2_WAIT_VREF_STABILITY);   /*~50ns delay. */
        ...
        USBUART2_CR0_REG |= USBUART2_CR0_ENABLE;
