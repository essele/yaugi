...
* functions.
*/
uint8 USBUART2_initVar = 0u;

#if (USBUART2_EP_MANAGEMENT_DMA)
    #if (CY_PSOC4)
        static void USBUART2_InitEpDma(void);

        /* DMA chanels assigend for endpoints. */
        const uint8 USBUART2_DmaChan[USBUART2_MAX_EP] =
        {
            0u,
            0u,
            0u,
    ...
    #else
        /* DMA chanels assigend for endpoints. */
        uint8 USBUART2_DmaChan[USBUART2_MAX_EP];
        
        /* DMA TDs require for PSoC 3/5LP operation. */
        uint8 USBUART2_DmaTd[USBUART2_MAX_EP];
    #endif /* (CY_PSOC4) */
    ...
    uint8  USBUART2_DmaEpBurstCnt   [USBUART2_MAX_EP];
    
    /* Number of bytes to transfer in last DMA burst. */
    uint8  USBUART2_DmaEpLastBurstEl[USBUART2_MAX_EP];

    /* Storage for arrays above. */
    uint8  USBUART2_DmaEpBurstCntBackup  [USBUART2_MAX_EP];
    uint32 USBUART2_DmaEpBufferAddrBackup[USBUART2_MAX_EP];
     
    /* DMA trigger mux output for usb.dma_req[0-7]. */
    const uint8 USBUART2_DmaReqOut[USBUART2_MAX_EP] =
    {
        0u,
        USBUART2_ep1_dma__TR_OUTPUT,
        USBUART2_ep2_dma__TR_OUTPUT,
        USBUART2_ep3_dma__TR_OUTPUT,
        0u,
        0u,
        0u,
        0u,
    ...
    const uint8 USBUART2_DmaBurstEndOut[USBUART2_MAX_EP] =
    {
        0u,
        USBUART2_BURSTEND_0_TR_OUTPUT,
        USBUART2_BURSTEND_1_TR_OUTPUT,
        USBUART2_BURSTEND_2_TR_OUTPUT,
        USBUART2_BURSTEND_3_TR_OUTPUT,
        USBUART2_BURSTEND_4_TR_OUTPUT,
        USBUART2_BURSTEND_5_TR_OUTPUT,
        USBUART2_BURSTEND_6_TR_OUTPUT,
        USBUART2_BURSTEND_7_TR_OUTPUT
    };
    
#else
    #if (USBUART2_EP_DMA_AUTO_OPT == 0u)
        static uint8 clearInDataRdyStatus = USBUART2_ARB_EPX_CFG_DEFAULT;
        uint8 USBUART2_DmaNextTd[USBUART2_MAX_EP];
        const uint8 USBUART2_epX_TD_TERMOUT_EN[USBUART2_MAX_EP] =
        {
            0u,
            0u,
            0u,
        ...
        volatile uint16 USBUART2_inLength[USBUART2_MAX_EP];
        const uint8 *USBUART2_inDataPointer[USBUART2_MAX_EP];
        volatile uint8 USBUART2_inBufFull[USBUART2_MAX_EP];
...
*   following list.
*
*       *USBUART2_3V_OPERATION* - Disable voltage regulator and pass-
*                                      through Vcc for pull-up
*
*       *USBUART2_5V_OPERATION* - Enable voltage regulator and use
*                                      regulator for pull-up
*
*       *USBUART2_DWR_POWER_OPERATION* - Enable or disable the voltage
...
{
    if (0u == USBUART2_initVar)
    {
        USBUART2_Init();
        USBUART2_initVar = 1u;
    }

    USBUART2_InitComponent(device, mode);
}


/*******************************************************************************
* Function Name: USBUART2_Init
****************************************************************************//**
*
...
* USBUART2_Init() because the USBUART2_Start() routine calls
...
void USBUART2_Init(void) 
{
    ...
    USBUART2_USB_CLK_EN_REG = USBUART2_USB_CLK_CSR_CLK_EN;

    /* The internal regulator (CR1.REG_ENABLE) is enabled in
    * USBUART2_InitComponent() if it is required.
    */

    /* Enable USBIO control on drive mode of D+ and D- pins. */
    USBUART2_USBIO_CR1_REG &= ~ (uint32) USBUART2_USBIO_CR1_IOMODE;

    /* Set number of LF CLK to detect UBS bus reset. */
    USBUART2_BUS_RST_CNT_REG = USBUART2_DEFUALT_BUS_RST_CNT;
    ...
    */
    USBUART2_POWER_CTRL_REG = USBUART2_DEFAULT_POWER_CTRL_VBUS;

    /* Enable PHY detector and single-ended and differential receivers. */
    USBUART2_POWER_CTRL_REG |= USBUART2_DEFAULT_POWER_CTRL_PHY;

    /* Suspend clear sequence. */
    USBUART2_POWER_CTRL_REG &= (uint32) ~USBUART2_POWER_CTRL_SUSPEND;
    CyDelayUs(USBUART2_WAIT_SUSPEND_DEL_DISABLE);
    USBUART2_POWER_CTRL_REG &= (uint32) ~USBUART2_POWER_CTRL_SUSPEND_DEL;

    /* Sets IMO lock options and clear all other bits. */
    USBUART2_CR1_REG = USBUART2_DEFUALT_CR1;

    /* Configure level (hi, lo, med) for each interrupt source. */
    USBUART2_INTR_LVL_SEL_REG = USBUART2_DEFAULT_INTR_LVL_SEL;

    /* Configure interrupt sources from: SOF, Bus Reset and EP0. */
    USBUART2_INTR_SIE_MASK_REG = USBUART2_DEFAULT_INTR_SIE_MASK;
    ...
    /* Enable USB block. */
    USBUART2_PM_ACT_CFG_REG |= USBUART2_PM_ACT_EN_FSUSB;
    /* Enable USB block for Standby Power Mode. */
    USBUART2_PM_STBY_CFG_REG |= USBUART2_PM_STBY_EN_FSUSB;

    /* Enable core clock. */
    USBUART2_USB_CLK_EN_REG = USBUART2_USB_CLK_ENABLE;

    USBUART2_CR1_REG = USBUART2_CR1_ENABLE_LOCK;

    /* ENABLING USBIO PADS IN USB MODE FROM I/O MODE. */
    /* Ensure USB transmit enable is low (USB_USBIO_CR0.ten). - Manual Transmission - Disabled. */
    USBUART2_USBIO_CR0_REG &= (uint8) ~USBUART2_USBIO_CR0_TEN;
    CyDelayUs(USBUART2_WAIT_REG_STABILITY_50NS);  /* ~50ns delay. */
    ...
    */
    USBUART2_PM_USB_CR0_REG &= (uint8) ~(USBUART2_PM_USB_CR0_PD_N |
                                                 USBUART2_PM_USB_CR0_PD_PULLUP_N);

    /* Select IOMODE to USB mode. */
    USBUART2_USBIO_CR1_REG &= (uint8) ~USBUART2_USBIO_CR1_IOMODE;

    /* Enable USBIO reference by setting PM.USB_CR0.fsusbio_ref_en. */
    USBUART2_PM_USB_CR0_REG |= USBUART2_PM_USB_CR0_REF_EN;
    /* Reference is available for 1us after regulator is enabled. */
    CyDelayUs(USBUART2_WAIT_REG_STABILITY_1US);
    /* OR 40us after power is restored. */
    CyDelayUs(USBUART2_WAIT_VREF_RESTORE);
    /* Ensure single-ended disable bits are low (PRT15.INP_DIS[7:6])(input receiver enabled). */
    USBUART2_DM_INP_DIS_REG &= (uint8) ~USBUART2_DM_MASK;
    USBUART2_DP_INP_DIS_REG &= (uint8) ~USBUART2_DP_MASK;

    /* Enable USBIO. */
    USBUART2_PM_USB_CR0_REG |= USBUART2_PM_USB_CR0_PD_N;
    CyDelayUs(USBUART2_WAIT_PD_PULLUP_N_ENABLE);
    /* Set USBIO pull-up enable. */
    USBUART2_PM_USB_CR0_REG |= USBUART2_PM_USB_CR0_PD_PULLUP_N;

    /* Reset Arbiter Write Address register for endpoint 1. */
    CY_SET_REG8(USBUART2_ARB_RW1_WA_PTR,     0u);
    CY_SET_REG8(USBUART2_ARB_RW1_WA_MSB_PTR, 0u);

#if (USBUART2_EP_MANAGEMENT_DMA)
    /* Initialize transfer descriptor. This will be used to detect DMA state - initialized or not. */
    for (i = 0u; i < USBUART2_MAX_EP; ++i)
    {
        USBUART2_DmaTd[i] = DMA_INVALID_TD;

    #if (USBUART2_EP_MANAGEMENT_DMA_AUTO && (USBUART2_EP_DMA_AUTO_OPT == 0u))
        USBUART2_DmaNextTd[i] = DMA_INVALID_TD;
    ...
    CyIntSetPriority     (USBUART2_INTR_HI_VECT_NUM,  USBUART2_INTR_HI_PRIORITY);
    (void) CyIntSetVector(USBUART2_INTR_HI_VECT_NUM, &USBUART2_INTR_HI_ISR);

    /* Configure lo_int: set handler and priority. */
    CyIntSetPriority     (USBUART2_INTR_LO_VECT_NUM,  USBUART2_INTR_LO_PRIORITY);
    (void) CyIntSetVector(USBUART2_INTR_LO_VECT_NUM, &USBUART2_INTR_LO_ISR);

    /* Configure med_int: set handler and priority (routed through DSI). */
    CyIntSetPriority     (USBUART2_INTR_MED_VECT_NUM,  USBUART2_INTR_MED_PRIORITY);
    (void) CyIntSetVector(USBUART2_INTR_MED_VECT_NUM, &USBUART2_INTR_MED_ISR);

#else
    /* Set bus reset interrupt. */
    CyIntSetPriority(USBUART2_BUS_RESET_VECT_NUM, USBUART2_BUS_RESET_PRIOR);
    (void) CyIntSetVector(USBUART2_BUS_RESET_VECT_NUM,   &USBUART2_BUS_RESET_ISR);

    /* Set Control Endpoint Interrupt. */
    CyIntSetPriority(USBUART2_EP_0_VECT_NUM, USBUART2_EP_0_PRIOR);
    (void) CyIntSetVector(USBUART2_EP_0_VECT_NUM,   &USBUART2_EP_0_ISR);

    /* Set SOF interrupt. */
    #if (USBUART2_SOF_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_SOF_VECT_NUM,  USBUART2_SOF_PRIOR);
        (void) CyIntSetVector(USBUART2_SOF_VECT_NUM, &USBUART2_SOF_ISR);
    #endif /* (USBUART2_SOF_ISR_ACTIVE) */

    /* Set Data Endpoint 1 Interrupt. */
    #if (USBUART2_EP1_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_1_VECT_NUM,  USBUART2_EP_1_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_1_VECT_NUM,  &USBUART2_EP_1_ISR);
    #endif /* (USBUART2_EP1_ISR_ACTIVE) */

    /* Set Data Endpoint 2 Interrupt. */
    #if (USBUART2_EP2_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_2_VECT_NUM,  USBUART2_EP_2_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_2_VECT_NUM, &USBUART2_EP_2_ISR);
    #endif /* (USBUART2_EP2_ISR_ACTIVE) */

    /* Set Data Endpoint 3 Interrupt. */
    #if (USBUART2_EP3_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_3_VECT_NUM,  USBUART2_EP_3_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_3_VECT_NUM, &USBUART2_EP_3_ISR);
    #endif /* (USBUART2_EP3_ISR_ACTIVE) */

    /* Set Data Endpoint 4 Interrupt. */
    #if (USBUART2_EP4_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_4_VECT_NUM,  USBUART2_EP_4_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_4_VECT_NUM, &USBUART2_EP_4_ISR);
    #endif /* (USBUART2_EP4_ISR_ACTIVE) */

    /* Set Data Endpoint 5 Interrupt. */
    #if (USBUART2_EP5_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_5_VECT_NUM,  USBUART2_EP_5_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_5_VECT_NUM, &USBUART2_EP_5_ISR);
    #endif /* (USBUART2_EP5_ISR_ACTIVE) */

    /* Set Data Endpoint 6 Interrupt. */
    #if (USBUART2_EP6_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_6_VECT_NUM,  USBUART2_EP_6_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_6_VECT_NUM, &USBUART2_EP_6_ISR);
    #endif /* (USBUART2_EP6_ISR_ACTIVE) */

     /* Set Data Endpoint 7 Interrupt. */
    #if (USBUART2_EP7_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_7_VECT_NUM,  USBUART2_EP_7_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_7_VECT_NUM, &USBUART2_EP_7_ISR);
    #endif /* (USBUART2_EP7_ISR_ACTIVE) */

    /* Set Data Endpoint 8 Interrupt. */
    #if (USBUART2_EP8_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_EP_8_VECT_NUM,  USBUART2_EP_8_PRIOR);
        (void) CyIntSetVector(USBUART2_EP_8_VECT_NUM, &USBUART2_EP_8_ISR);
    #endif /* (USBUART2_EP8_ISR_ACTIVE) */

    /* Set ARB Interrupt. */
    #if (USBUART2_EP_MANAGEMENT_DMA && USBUART2_ARB_ISR_ACTIVE)
        CyIntSetPriority     (USBUART2_ARB_VECT_NUM,  USBUART2_ARB_PRIOR);
        (void) CyIntSetVector(USBUART2_ARB_VECT_NUM, &USBUART2_ARB_ISR);
    #endif /* (USBUART2_EP_MANAGEMENT_DMA && USBUART2_ARB_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

    /* Common: Configure GPIO interrupt for wakeup. */
#if (USBUART2_DP_ISR_ACTIVE)
    CyIntSetPriority     (USBUART2_DP_INTC_VECT_NUM,  USBUART2_DP_INTC_PRIORITY);
    (void) CyIntSetVector(USBUART2_DP_INTC_VECT_NUM, &USBUART2_DP_ISR);
#endif /* (USBUART2_DP_ISR_ACTIVE) */

#if (USBUART2_EP_MANAGEMENT_DMA && CY_PSOC4)
    /* Initialize DMA channels. */
    USBUART2_InitEpDma();
#endif /* (USBUART2_EP_MANAGEMENT_DMA && CY_PSOC4) */
}


/*******************************************************************************
* Function Name: USBUART2_InitComponent
****************************************************************************//**
*
...
* \globalvars
*   \ref USBUART2_device
*   \ref USBUART2_transferState
*   \ref USBUART2_configuration
*   \ref USBUART2_deviceStatus
*
*   \ref USBUART2_deviceAddress - Contains the current device address. This
...
*      SET_ADDRESS request.
*
*   \ref USBUART2_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
void USBUART2_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
    ...
    {
        USBUART2_hidProtocol[i] = USBUART2_PROTOCOL_REPORT;
    }
#endif /* USBUART2_ENABLE_HID_CLASS */

    /* Store device number to access descriptor. */
    USBUART2_device = device;

    /* Reset component internal variables. */
    USBUART2_transferState   = USBUART2_TRANS_STATE_IDLE;
    USBUART2_configurationChanged = 0u;
    USBUART2_configuration   = 0u;
    USBUART2_interfaceNumber = 0u;
    USBUART2_deviceAddress   = 0u;
    USBUART2_deviceStatus    = 0u;
    USBUART2_lastPacketSize  = 0u;

    /* Enable component interrupts. */
#if (CY_PSOC4)
    CyIntEnable(USBUART2_INTR_HI_VECT_NUM);
    CyIntEnable(USBUART2_INTR_MED_VECT_NUM);
    CyIntEnable(USBUART2_INTR_LO_VECT_NUM);
#else
    CyIntEnable(USBUART2_BUS_RESET_VECT_NUM);
    CyIntEnable(USBUART2_EP_0_VECT_NUM);

    #if (USBUART2_SOF_ISR_ACTIVE)
        CyIntEnable(USBUART2_SOF_VECT_NUM);
    #endif /* (USBUART2_SOF_ISR_ACTIVE) */

    #if (USBUART2_EP1_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_1_VECT_NUM);
    #endif /* (USBUART2_EP1_ISR_ACTIVE) */

    #if (USBUART2_EP2_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_2_VECT_NUM);
    #endif /* (USBUART2_EP5_ISR_ACTIVE) */

    #if (USBUART2_EP3_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_3_VECT_NUM);
    #endif /* (USBUART2_EP5_ISR_ACTIVE) */

    #if (USBUART2_EP4_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_4_VECT_NUM);
    #endif /* (USBUART2_EP5_ISR_ACTIVE) */

    #if (USBUART2_EP5_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_5_VECT_NUM);
    #endif /* (USBUART2_EP5_ISR_ACTIVE) */

    #if (USBUART2_EP6_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_6_VECT_NUM);
    #endif /* USBUART2_EP6_ISR_REMOVE */

    #if (USBUART2_EP7_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_7_VECT_NUM);
    #endif /* (USBUART2_EP7_ISR_ACTIVE) */

    #if (USBUART2_EP8_ISR_ACTIVE)
        CyIntEnable(USBUART2_EP_8_VECT_NUM);
    #endif /* (USBUART2_EP8_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

#if (USBUART2_EP_MANAGEMENT_DMA && USBUART2_ARB_ISR_ACTIVE)
    /* Enable ARB EP interrupt sources. */
    USBUART2_ARB_INT_EN_REG = USBUART2_DEFAULT_ARB_INT_EN;

    #if (CY_PSOC3 || CY_PSOC5)
        CyIntEnable(USBUART2_ARB_VECT_NUM);
...
/* Arbiter configuration for DMA transfers. */
    ...
    USBUART2_ARB_CFG_REG = USBUART2_DEFAULT_ARB_CFG;
        ...
        CyDmaEnable();
            ...
            /* Change DMA priority to be highest. */
             CyIntSetPriority(CYDMA_INTR_NUMBER, USBUART2_DMA_AUTO_INTR_PRIO);
        #endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (CY_PSOC4) */
            ...
            CyIntEnable(CYDMA_INTR_NUMBER);
                ...
                USBUART2_EP_DMA_Done_isr_StartEx(&USBUART2_EP_DMA_DONE_ISR);
                USBUART2_EP17_DMA_Done_SR_InterruptEnable();
                USBUART2_EP8_DMA_Done_SR_InterruptEnable();
            #endif /* (USBUART2_EP_DMA_AUTO_OPT == 0u) */
        #endif /* (CY_PSOC4) */
    #endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO) */
#endif /* (USBUART2_EP_MANAGEMENT_DMA) */
    ...
    switch(mode)
    {
    case USBUART2_3V_OPERATION:
        /* Disable regulator for 3V operation. */
        USBUART2_CR1_REG &= (uint32) ~USBUART2_CR1_REG_ENABLE;
        break;

    case USBUART2_5V_OPERATION:
        /* Enable regulator for 5V operation. */
        USBUART2_CR1_REG |= (uint32)  USBUART2_CR1_REG_ENABLE;
        break;

    default: /* Check DWR settings of USB power supply. */
    #if (USBUART2_VDDD_MV < USBUART2_3500MV)
        /* Disable regulator for 3V operation. */
        USBUART2_CR1_REG &= (uint32) ~USBUART2_CR1_REG_ENABLE;
    #else
        /* Enable regulator for 5V operation. */
        USBUART2_CR1_REG |= (uint32)  USBUART2_CR1_REG_ENABLE;
    #endif /* (USBUART2_VDDD_MV < USBUART2_3500MV) */
        break;
    }

#if (CY_PSOC4)
    /* Clear bus activity. */
    USBUART2_CR1_REG &= (uint32) ~USBUART2_CR1_BUS_ACTIVITY;

    /* Clear EP0 count register. */
    USBUART2_EP0_CNT_REG = USBUART2_CLEAR_REG;

    /* Set EP0.CR: ACK Setup, NAK IN/OUT. */
    USBUART2_EP0_CR_REG = USBUART2_MODE_NAK_IN_OUT;

    #if (USBUART2_LPM_ACTIVE)
        if (NULL != USBUART2_GetBOSPtr())
        {
            /* Enable LPM and acknowledge LPM packets for active device.
            * Reset NYET_EN and SUB_RESP bits in the LPM_CTRL register.
            */
            USBUART2_LPM_CTRL_REG = (USBUART2_LPM_CTRL_LPM_EN | \
                                             USBUART2_LPM_CTRL_LPM_ACK_RESP);
        }
        else
        {
            /* Disable LPM for active device. */
            USBUART2_LPM_CTRL_REG &= (uint32) ~USBUART2_LPM_CTRL_LPM_EN;
        }
    #endif /* (USBUART2_LPM_ACTIVE) */

    /* Enable device to responds to USB traffic with address 0. */
    USBUART2_CR0_REG = USBUART2_DEFUALT_CR0;

#else
    /* Set EP0.CR: ACK Setup, STALL IN/OUT. */
    USBUART2_EP0_CR_REG = USBUART2_MODE_STALL_IN_OUT;

    /* Enable device to respond to USB traffic with address 0. */
    USBUART2_CR0_REG = USBUART2_DEFUALT_CR0;
    CyDelayCycles(USBUART2_WAIT_CR0_REG_STABILITY);
#endif /* (CY_PSOC4) */

    /* Enable D+ pull-up and keep USB control on IO. */
    USBUART2_USBIO_CR1_REG = USBUART2_USBIO_CR1_USBPUEN;
}


/*******************************************************************************
* Function Name: USBUART2_ReInitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   USBUART2_device - Contains the device number of the desired Device
...
*   USBUART2_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
*   \ref USBUART2_deviceStatus
*
*   \ref USBUART2_configurationChanged
*
*   USBUART2_intiVar -  This variable is set to zero
*
*******************************************************************************/
void USBUART2_Stop(void) 
{
    ...
    USBUART2_Stop_DMA(USBUART2_MAX_EP);
#endif /* (USBUART2_EP_MANAGEMENT_DMA) */

    enableInterrupts = CyEnterCriticalSection();

    /* Disable USB IP to respond to USB traffic. */
    USBUART2_CR0_REG &= (uint32) ~USBUART2_CR0_ENABLE;

    /* Disable D+ pull-up. */
    USBUART2_USBIO_CR1_REG &= (uint32) ~ USBUART2_USBIO_CR1_USBPUEN;

#if (CY_PSOC4)
    /* Disable USBFS block. */
    USBUART2_POWER_CTRL_REG &= (uint32) ~USBUART2_POWER_CTRL_ENABLE;
#else
    /* Clear power active and standby mode templates. */
    USBUART2_PM_ACT_CFG_REG  &= (uint8) ~USBUART2_PM_ACT_EN_FSUSB;
    USBUART2_PM_STBY_CFG_REG &= (uint8) ~USBUART2_PM_STBY_EN_FSUSB;
...
#if (CY_PSOC4)
    CyIntDisable(USBUART2_INTR_HI_VECT_NUM);
    CyIntDisable(USBUART2_INTR_LO_VECT_NUM);
    CyIntDisable(USBUART2_INTR_MED_VECT_NUM);
#else

    CyIntDisable(USBUART2_BUS_RESET_VECT_NUM);
    CyIntDisable(USBUART2_EP_0_VECT_NUM);

    #if (USBUART2_SOF_ISR_ACTIVE)
        CyIntDisable(USBUART2_SOF_VECT_NUM);
    #endif /* (USBUART2_SOF_ISR_ACTIVE) */

    #if (USBUART2_EP1_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_1_VECT_NUM);
    #endif /* (USBUART2_EP1_ISR_ACTIVE) */

    #if (USBUART2_EP2_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_2_VECT_NUM);
    #endif /* (USBUART2_EP2_ISR_ACTIVE) */

    #if (USBUART2_EP3_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_3_VECT_NUM);
    #endif /* (USBUART2_EP3_ISR_ACTIVE) */

    #if (USBUART2_EP4_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_4_VECT_NUM);
    #endif /* (USBUART2_EP4_ISR_ACTIVE) */

    #if (USBUART2_EP5_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_5_VECT_NUM);
    #endif /* (USBUART2_EP5_ISR_ACTIVE) */

    #if (USBUART2_EP6_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_6_VECT_NUM);
    #endif /* USBUART2_EP6_ISR_REMOVE */

    #if (USBUART2_EP7_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_7_VECT_NUM);
    #endif /* (USBUART2_EP7_ISR_ACTIVE) */

    #if (USBUART2_EP8_ISR_ACTIVE)
        CyIntDisable(USBUART2_EP_8_VECT_NUM);
    #endif /* (USBUART2_EP8_ISR_ACTIVE) */

    #if (USBUART2_DP_ISR_ACTIVE)
        /* Clear active mode Dp interrupt source history. */
        (void) USBUART2_Dp_ClearInterrupt();
        CyIntClearPending(USBUART2_DP_INTC_VECT_NUM);
    #endif /* (USBUART2_DP_ISR_ACTIVE). */
...
/*******************************************************************************
* Function Name: USBUART2_CheckActivity
****************************************************************************//**
*
...
{
    uint32 cr1Reg = USBUART2_CR1_REG;

    /* Clear bus activity. */
    USBUART2_CR1_REG = (cr1Reg & (uint32) ~USBUART2_CR1_BUS_ACTIVITY);

    /* Get bus activity. */
    return ((0u != (cr1Reg & USBUART2_CR1_BUS_ACTIVITY)) ? (1u) : (0u));
}


/*******************************************************************************
* Function Name: USBUART2_GetConfiguration
****************************************************************************//**
*
...
{
    return (USBUART2_configuration);
}


/*******************************************************************************
* Function Name: USBUART2_IsConfigurationChanged
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBUART2_configurationChanged - This variable is set to 1 after
...
uint8 USBUART2_IsConfigurationChanged(void) 
{
    ...
    if (USBUART2_configurationChanged != 0u)
    {
        res = USBUART2_configurationChanged;
        USBUART2_configurationChanged = 0u;
    }
...
/*******************************************************************************
* Function Name: USBUART2_GetInterfaceSetting
****************************************************************************//**
*
...
{
    return (USBUART2_interfaceSetting[interfaceNumber]);
}


/*******************************************************************************
* Function Name: USBUART2_GetEPState
****************************************************************************//**
*
...
{
    return (USBUART2_EP[epNumber].apiEpState);
}


/*******************************************************************************
* Function Name: USBUART2_GetEPCount
****************************************************************************//**
*
...
uint16 USBUART2_GetEPCount(uint8 epNumber) 
{
    ...
    if ((epNumber > USBUART2_EP0) && (epNumber < USBUART2_MAX_EP))
    {
        ...
        cntr  = ((uint16) USBUART2_SIE_EP_BASE.sieEp[epNumber].epCnt0) & USBUART2_EPX_CNT0_MASK;
        cntr  = ((uint16) (cntr << 8u)) | ((uint16) USBUART2_SIE_EP_BASE.sieEp[epNumber].epCnt1);
        cntr -= USBUART2_EPX_CNTX_CRC_COUNT;
    }
...
#if (CY_PSOC4)
    /*******************************************************************************
    * Function Name: USBUART2_InitEpDma
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static void USBUART2_InitEpDma(void)
    {
        ...
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep1_dma_CHANNEL] = USBUART2_ep1_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA1_ACTIVE) */

    #if (USBUART2_DMA2_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep2_dma_CHANNEL] = USBUART2_ep2_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA2_ACTIVE) */

    #if (USBUART2_DMA3_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep3_dma_CHANNEL] = USBUART2_ep3_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA3_ACTIVE) */

    #if (USBUART2_DMA4_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep4_dma_CHANNEL] = USBUART2_ep4_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA4_ACTIVE) */

    #if (USBUART2_DMA5_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep5_dma_CHANNEL] = USBUART2_ep5_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA5_ACTIVE) */

    #if (USBUART2_DMA6_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep6_dma_CHANNEL] = USBUART2_ep6_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA6_ACTIVE) */

    #if (USBUART2_DMA7_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep7_dma_CHANNEL] = USBUART2_ep7_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA7_ACTIVE) */

    #if (USBUART2_DMA8_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART2_ep8_dma_CHANNEL] = USBUART2_ep8_dma_CHANNEL_CFG;
    #endif /* (USBUART2_DMA8_ACTIVE) */

    #if (USBUART2_EP_MANAGEMENT_DMA_AUTO)
        /* Initialize DMA channel callbacks. */
        #if (USBUART2_DMA1_ACTIVE)
            (void) USBUART2_ep1_dma_SetInterruptCallback(&USBUART2_EP1_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA1_ACTIVE) */

        #if (USBUART2_DMA2_ACTIVE)
            (void) USBUART2_ep2_dma_SetInterruptCallback(&USBUART2_EP2_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA2_ACTIVE) */

        #if (USBUART2_DMA3_ACTIVE)
            (void) USBUART2_ep3_dma_SetInterruptCallback(&USBUART2_EP3_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA3_ACTIVE) */

        #if (USBUART2_DMA4_ACTIVE)
            (void) USBUART2_ep4_dma_SetInterruptCallback(&USBUART2_EP4_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA4_ACTIVE) */

        #if (USBUART2_DMA5_ACTIVE)
            (void) USBUART2_ep5_dma_SetInterruptCallback(&USBUART2_EP5_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA5_ACTIVE) */

        #if (USBUART2_DMA6_ACTIVE)
            (void) USBUART2_ep6_dma_SetInterruptCallback(&USBUART2_EP6_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA6_ACTIVE) */

        #if (USBUART2_DMA7_ACTIVE)
            (void) USBUART2_ep7_dma_SetInterruptCallback(&USBUART2_EP7_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA7_ACTIVE) */

        #if (USBUART2_DMA8_ACTIVE)
            (void) USBUART2_ep8_dma_SetInterruptCallback(&USBUART2_EP8_DMA_DONE_ISR);
        #endif /* (USBUART2_DMA8_ACTIVE) */
    #endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO) */
    }
#else
    ...
    /***************************************************************************
    * Function Name: USBUART2_InitEP_DMA
    ************************************************************************//**
    *
    *  This function allocates and initializes a DMA channel to be used by the
    *  USBUART2_LoadInEP() or USBUART2_ReadOutEP() APIs for data
    ...
    ***************************************************************************/
                                                                    ...
                                                                    
    {
    ...
    #if (USBUART2_DMA1_ACTIVE)
        case USBUART2_EP1:
            USBUART2_DmaChan[epNumber] = USBUART2_ep1_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                    USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA1_ACTIVE) */

    #if (USBUART2_DMA2_ACTIVE)
        case USBUART2_EP2:
            USBUART2_DmaChan[epNumber] = USBUART2_ep2_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                    USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA2_ACTIVE) */
    ...
    #if (USBUART2_DMA3_ACTIVE)
            case USBUART2_EP3:
                USBUART2_DmaChan[epNumber] = USBUART2_ep3_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                        USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA3_ACTIVE) */
    ...
    #if (USBUART2_DMA4_ACTIVE)
            case USBUART2_EP4:
                USBUART2_DmaChan[epNumber] = USBUART2_ep4_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                        USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA4_ACTIVE) */
    ...
    #if (USBUART2_DMA5_ACTIVE)
            case USBUART2_EP5:
                USBUART2_DmaChan[epNumber] = USBUART2_ep5_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                        USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA5_ACTIVE) */
    ...
    #if (USBUART2_DMA6_ACTIVE)
        case USBUART2_EP6:
            USBUART2_DmaChan[epNumber] = USBUART2_ep6_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                    USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA6_ACTIVE) */
    ...
    #if (USBUART2_DMA7_ACTIVE)
        case USBUART2_EP7:
                USBUART2_DmaChan[epNumber] = USBUART2_ep7_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                        USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA7_ACTIVE) */
    ...
    #if (USBUART2_DMA8_ACTIVE)
        case USBUART2_EP8:
                USBUART2_DmaChan[epNumber] = USBUART2_ep8_DmaInitialize(USBUART2_DMA_BYTES_PER_BURST,
                                                                                        USBUART2_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART2_DMA8_ACTIVE) */
        ...
        if ((epNumber > USBUART2_EP0) && (epNumber < USBUART2_MAX_EP))
        {
            USBUART2_DmaTd[epNumber] = CyDmaTdAllocate();

        #if (USBUART2_EP_MANAGEMENT_DMA_AUTO && (USBUART2_EP_DMA_AUTO_OPT == 0u))
            USBUART2_DmaNextTd[epNumber] = CyDmaTdAllocate();
        #endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO && (USBUART2_EP_DMA_AUTO_OPT == 0u)) */
        }
    }
#endif /* (CY_PSOC4) */
    ...
    /***************************************************************************
    * Function Name: USBUART2_Stop_DMA
    ************************************************************************//**
    *  
    ...
    *  to trigger DMA re-configuration when USBUART2_LoadInEP() or 
    *  USBUART2_ReadOutEP() functions are called the first time. 
    *  
    ...
    ***************************************************************************/
    void USBUART2_Stop_DMA(uint8 epNumber) 
    {
        ...
        i = (epNumber < USBUART2_MAX_EP) ? epNumber : USBUART2_EP1;

        do
        {
        #if (CY_PSOC4)
            if (0u != USBUART2_DmaChan[i])
            {
                USBUART2_CyDmaChDisable(USBUART2_DmaChan[i]);
            }
        #else
            if(USBUART2_DmaTd[i] != DMA_INVALID_TD)
            {
                (void) CyDmaChDisable(USBUART2_DmaChan[i]);
                CyDmaTdFree(USBUART2_DmaTd[i]);
                USBUART2_DmaTd[i] = DMA_INVALID_TD;
            }

            #if (USBUART2_EP_MANAGEMENT_DMA_AUTO && (USBUART2_EP_DMA_AUTO_OPT == 0u))
                if(USBUART2_DmaNextTd[i] != DMA_INVALID_TD)
                {
                    CyDmaTdFree(USBUART2_DmaNextTd[i]);
                    USBUART2_DmaNextTd[i] = DMA_INVALID_TD;
                }
            #endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO && (USBUART2_EP_DMA_AUTO_OPT == 0u)) */
        ...
        while ((i < USBUART2_MAX_EP) && (epNumber == USBUART2_MAX_EP));
    }
#endif /* (USBUART2_EP_MANAGEMENT_DMA) */
...
#if (USBUART2_EP_MANAGEMENT_DMA_AUTO && (USBUART2_EP_DMA_AUTO_OPT == 0u))
    /***************************************************************************
    * Function Name: USBUART2_LoadNextInEP
    ************************************************************************//**
    *
    * Summary:
    ...
    ***************************************************************************/
    void USBUART2_LoadNextInEP(uint8 epNumber, uint8 mode) 
    {
        ...
        if (mode == 0u)
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART2_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBUART2_inLength[epNumber] - USBUART2_DMA_BYTES_PER_BURST);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART2_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert, LO16((uint32)USBUART2_inDataPointer[epNumber] +
                                            USBUART2_DMA_BYTES_PER_BURST));
            USBUART2_inBufFull[epNumber] = 1u;
        }
        else
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART2_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBUART2_DMA_BYTES_REPEAT);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART2_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert,  LO16((uint32)USBUART2_inDataPointer[epNumber] +
                                   (USBUART2_DMA_BYTES_PER_BURST - USBUART2_DMA_BYTES_REPEAT)));
        }

        /* CyDmaChSetInitialTd API is optimized to initialize TD. */
        CY_DMA_CH_STRUCT_PTR[USBUART2_DmaChan[epNumber]].basic_status[1u] = USBUART2_DmaTd[epNumber];
    }
#endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO && (USBUART2_EP_DMA_AUTO_OPT == 0u)) */
#endif /* (CY_PSOC3 || CY_PSOC5) */
...
/*******************************************************************************
* Function Name: USBUART2_LoadInEP
****************************************************************************//**
*
                                                                        ...
                                                                        
{
    if ((epNumber > USBUART2_EP0) && (epNumber < USBUART2_MAX_EP))
    {
        ...
        if (length > (USBUART2_EPX_DATA_BUF_MAX - USBUART2_EP[epNumber].buffOffset))
        {
            length = USBUART2_EPX_DATA_BUF_MAX - USBUART2_EP[epNumber].buffOffset;
        }
    #endif /* (!USBUART2_EP_MANAGEMENT_DMA_AUTO) */

        /* Set count and data toggle. */
        USBUART2_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint32) HI8(length) | USBUART2_EP[epNumber].epToggle;
        USBUART2_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint32) LO8(length);

    #if (USBUART2_EP_MANAGEMENT_MANUAL)
        if (NULL != pData)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr = pData[i];
            }
        }

        /* IN endpoint buffer is full - read to be read. */
        USBUART2_EP[epNumber].apiEpState = USBUART2_NO_EVENT_PENDING;

        /* Arm IN endpoint. */
        USBUART2_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBUART2_EP[epNumber].epMode;

    #else

        #if (CY_PSOC3 || CY_PSOC5LP)
            /* Initialize DMA if it was not initialized. */
            if (DMA_INVALID_TD == USBUART2_DmaTd[epNumber])
            {
                USBUART2_InitEP_DMA(epNumber, pData);
            }
            ...
            {
            #if (CY_PSOC4)
                uint32 channelNum = (uint32) USBUART2_DmaChan[epNumber];

                /* Disable DMA channel: start configuration. */
                USBUART2_CyDmaChDisable(channelNum);

                /* Configure source and destination. */
                USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR0, (void*) pData);
                USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                --length;
                USBUART2_CyDmaSetConfiguration(channelNum, USBUART2_DMA_DESCR0, USBUART2_DMA_COMMON_CFG | length |
                ...
                USBUART2_CyDmaValidateDescriptor(channelNum, USBUART2_DMA_DESCR0);

                /* Enable DMA channel: configuration complete. */
                USBUART2_CyDmaChEnable(channelNum);
            #else
                /* Configure DMA to transfer data. */
                (void) CyDmaChDisable(USBUART2_DmaChan[epNumber]);
                (void) CyDmaTdSetConfiguration(USBUART2_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USBUART2_DmaTd[epNumber], LO16((uint32) pData), LO16((uint32) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr));

                /* Enable DMA channel. */
                (void) CyDmaChSetInitialTd(USBUART2_DmaChan[epNumber], USBUART2_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBUART2_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */

                /* Generate DMA request. */
                USBUART2_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint32)  USBUART2_ARB_EPX_CFG_DMA_REQ;
                USBUART2_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint32) ~USBUART2_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                USBUART2_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
                ...
                USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr);
                USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR1, (void*) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                USBUART2_CyDmaSetConfiguration(channelNum, USBUART2_DMA_DESCR0, USBUART2_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Configure DMA descriptor. */
                USBUART2_CyDmaSetConfiguration(channelNum, USBUART2_DMA_DESCR1, USBUART2_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Enable interrupt from DMA channel. */
                USBUART2_CyDmaSetInterruptMask(channelNum);
                ...
                (void) CyDmaChDisable(USBUART2_DmaChan[epNumber]);
                ...
                #if (USBUART2_EP_DMA_AUTO_OPT == 0u)
                    USBUART2_inLength[epNumber] = length;
                    USBUART2_inDataPointer[epNumber] = pData;

                    /* Configure DMA to send data only for first burst */
                    (void) CyDmaTdSetConfiguration(USBUART2_DmaTd[epNumber],
                        (length > USBUART2_DMA_BYTES_PER_BURST) ? USBUART2_DMA_BYTES_PER_BURST : length,
                        USBUART2_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBUART2_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    ...
                    */
                    (void) CyDmaTdSetConfiguration(USBUART2_DmaNextTd[epNumber], 1u,
                                                   USBUART2_DmaNextTd[epNumber],
                                                   USBUART2_epX_TD_TERMOUT_EN[epNumber]);

                    /* Configure DmaNextTd to clear Data Ready status. */
                    (void) CyDmaTdSetAddress(USBUART2_DmaNextTd[epNumber], LO16((uint32) &clearInDataRdyStatus),
                                                                                   LO16((uint32) &USBUART2_ARB_EP_BASE.arbEp[epNumber].epCfg));
                #else
                    /* Configure DMA to send all data. */
                    (void) CyDmaTdSetConfiguration(USBUART2_DmaTd[epNumber], length,
                                                   USBUART2_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBUART2_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr));
                #endif /* (USBUART2_EP_DMA_AUTO_OPT == 0u) */

                /* Clear any potential pending DMA requests before starting DMA channel to transfer data. */
                (void) CyDmaClearPendingDrq(USBUART2_DmaChan[epNumber]);
                /* Enable DMA. */
                (void) CyDmaChSetInitialTd(USBUART2_DmaChan[epNumber], USBUART2_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBUART2_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    USBUART2_DmaEpBurstCnt[epNumber] = (uint8) (length / USBUART2_DMA_BYTES_PER_BURST);

                    /* Get number of elements in the last burst. */
                    USBUART2_DmaEpLastBurstEl[epNumber] = (uint8) (length % USBUART2_DMA_BYTES_PER_BURST);

                    /* Get total number of bursts. */
                    USBUART2_DmaEpBurstCnt[epNumber] += (0u != USBUART2_DmaEpLastBurstEl[epNumber]) ? 1u : 0u;

                    /* Adjust number of data elements transferred in last burst. */
                    USBUART2_DmaEpLastBurstEl[epNumber] = (0u != USBUART2_DmaEpLastBurstEl[epNumber]) ?
                                                                          (USBUART2_DmaEpLastBurstEl[epNumber] - 1u) :
                                                                          (USBUART2_DMA_BYTES_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBUART2_DmaEpBurstCnt[epNumber]) ? USBUART2_DmaEpLastBurstEl[epNumber] : (USBUART2_DMA_BYTES_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBUART2_DmaEpBurstCnt[epNumber]) ? USBUART2_DmaEpLastBurstEl[epNumber] : (USBUART2_DMA_BYTES_PER_BURST - 1u);


                    /* Mark which descriptor is last one to execute. */
                    USBUART2_DmaEpLastBurstEl[epNumber] |= (0u != (USBUART2_DmaEpBurstCnt[epNumber] & 0x1u)) ?
                                                                            USBUART2_DMA_DESCR0_MASK : USBUART2_DMA_DESCR1_MASK;

                    /* Restore DMA settings for current transfer. */
                    USBUART2_CyDmaChDisable(channelNum);

                    /* Restore destination address for input endpoint. */
                    USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR0, (void*) ((uint32) USBUART2_DmaEpBufferAddrBackup[epNumber]));
                    USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR1, (void*) ((uint32) USBUART2_DmaEpBufferAddrBackup[epNumber] +
                                                                                                                   USBUART2_DMA_BYTES_PER_BURST));

                    /* Set number of elements to transfer. */
                    USBUART2_CyDmaSetNumDataElements(channelNum, USBUART2_DMA_DESCR0, lengthDescr0);
                    USBUART2_CyDmaSetNumDataElements(channelNum, USBUART2_DMA_DESCR1, lengthDescr1);

                    /* Validate descriptor 0 and command to start with it. */
                    USBUART2_CyDmaValidateDescriptor(channelNum, USBUART2_DMA_DESCR0);
                    USBUART2_CyDmaSetDescriptor0Next(channelNum);

                    /* Validate descriptor 1. */
                    if (USBUART2_DmaEpBurstCnt[epNumber] > 1u)
                    {
                        USBUART2_CyDmaValidateDescriptor(channelNum, USBUART2_DMA_DESCR1); 
                    }                   

                    /* Adjust burst counter taking to account: 2 valid descriptors and interrupt trigger after valid descriptor were executed. */
                    USBUART2_DmaEpBurstCnt[epNumber] = USBUART2_DMA_GET_BURST_CNT(USBUART2_DmaEpBurstCnt[epNumber]);

                    /* Enable DMA channel: configuration complete. */
                    USBUART2_CyDmaChEnable(channelNum);

                #elif (USBUART2_EP_DMA_AUTO_OPT == 0u)
                    USBUART2_inLength[epNumber]  = length;
                    USBUART2_inBufFull[epNumber] = 0u;

                    (void) CyDmaChDisable(USBUART2_DmaChan[epNumber]);
                    /* Configure DMA to send data only for first burst. */
                    (void) CyDmaTdSetConfiguration(
                        USBUART2_DmaTd[epNumber], (length > USBUART2_DMA_BYTES_PER_BURST) ?
                        USBUART2_DMA_BYTES_PER_BURST : length,
                        USBUART2_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(USBUART2_DmaTd[epNumber], LO16((uint32)  USBUART2_inDataPointer[epNumber]),
                                                                               LO16((uint32) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    /* Clear Any potential pending DMA requests before starting DMA channel to transfer data. */
                    (void) CyDmaClearPendingDrq(USBUART2_DmaChan[epNumber]);
                    /* Enable DMA. */
                ...
                #if !defined (USBUART2_MANUAL_IN_EP_ARM)
                    /* Set IN data ready to generate DMA request to load data into endpoint buffer. */
                    USBUART2_ARB_EP_BASE.arbEp[epNumber].epCfg |= USBUART2_ARB_EPX_CFG_IN_DATA_RDY;
                #endif  /* (USBUART2_MANUAL_IN_EP_ARM) */
        ...
        #endif /* (USBUART2_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (USBUART2_EP_MANAGEMENT_MANUAL) */
    }
}


/*******************************************************************************
* Function Name: USBUART2_ReadOutEP
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
...
*   transfer after calling the USBUART2_ReadOutEP() API. For example,
*   by checking EPstate:
*
...
{
    if ((pData != NULL) && (epNumber > USBUART2_EP0) && (epNumber < USBUART2_MAX_EP))
    {
    #if (!USBUART2_EP_MANAGEMENT_DMA_AUTO)
        /* Adjust requested length to available data. */
        length = (length > USBUART2_GetEPCount(epNumber)) ? USBUART2_GetEPCount(epNumber) : length;
    #endif /* (!USBUART2_EP_MANAGEMENT_DMA_AUTO) */
    ...
    #if (USBUART2_EP_MANAGEMENT_MANUAL)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                pData[i] = (uint8) USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr;
            }
        }
        ...
        #if (CY_PSOC4)
        {
            ...
            /* Configure source and destination. */
            USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBUART2_CyDmaSetConfiguration(channelNum, USBUART2_DMA_DESCR0, USBUART2_DMA_COMMON_CFG | (uint16)(length - 1u) |
            ...
            /* Configure DMA to transfer data. */
            (void) CyDmaChDisable(USBUART2_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBUART2_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBUART2_DmaTd[epNumber], LO16((uint32) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32)pData));
        ...
        #if (CY_PSOC4)
        {
            ...
            USBUART2_DmaEpLastBurstEl[epNumber] |= (USBUART2_DmaEpBurstCnt[epNumber] > 2u) ? USBUART2_DMA_DESCR_REVERT : 0u;
            ...
            USBUART2_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
            USBUART2_DmaEpBurstCntBackup[epNumber]   = USBUART2_DmaEpBurstCnt[epNumber];
            ...
            /* Set destination address. */
            USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR1, (void*) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr);

            USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR0, (void*) ((uint32) pData));
            USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR1, (void*) ((uint32) pData + USBUART2_DMA_BYTES_PER_BURST));

            /* Configure DMA descriptor. */
            USBUART2_CyDmaSetConfiguration(channelNum, USBUART2_DMA_DESCR0, USBUART2_DMA_COMMON_CFG  | lengthDescr0 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

            /* Configure DMA descriptor. */
            USBUART2_CyDmaSetConfiguration(channelNum, USBUART2_DMA_DESCR1, USBUART2_DMA_COMMON_CFG  | lengthDescr1 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);
            ...
            if (USBUART2_DmaEpBurstCntBackup[epNumber] > 1u)
            {
                USBUART2_CyDmaValidateDescriptor(channelNum, USBUART2_DMA_DESCR1);
            }
        ...
        #else
            (void) CyDmaChDisable(USBUART2_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBUART2_DmaTd[epNumber], length,  USBUART2_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBUART2_DmaTd[epNumber], LO16((uint32) &USBUART2_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32) pData));
...
/*******************************************************************************
* Function Name: USBUART2_LoadInEP16
****************************************************************************//**
*
*  This function performs different functionality depending on the Component’s
*  configured Endpoint Buffer Management. This parameter is defined in
...
void USBUART2_LoadInEP16(uint8 epNumber, const uint8 pData[], uint16 length)
{
    ...
    if ((epNumber > USBUART2_EP0) && (epNumber < USBUART2_MAX_EP))
    {
        ...
        if (NULL != pData)
        {
            ...
            {
                USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16 = dataBuf[i];
            }
        }
        ...
        USBUART2_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBUART2_EP[epNumber].epMode;
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                /* Configure source and destination. */
                USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR0, (void*) pData);
                USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            if (pData != NULL)
            {
                ...
                /* Set destination address. */
                USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR1, (void*) &USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    /* Adjust number of data elements transferred in last burst. */
                    USBUART2_DmaEpLastBurstEl[epNumber] = (0u != USBUART2_DmaEpLastBurstEl[epNumber]) ?
                                                                          ((USBUART2_DmaEpLastBurstEl[epNumber] >> 1u) - 1u) :
                                                                           (USBUART2_DMA_HALFWORDS_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBUART2_DmaEpBurstCnt[epNumber]) ? USBUART2_DmaEpLastBurstEl[epNumber] : (USBUART2_DMA_HALFWORDS_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBUART2_DmaEpBurstCnt[epNumber]) ? USBUART2_DmaEpLastBurstEl[epNumber] : (USBUART2_DMA_HALFWORDS_PER_BURST - 1u);
...
/*******************************************************************************
* Function Name: USBUART2_ReadOutEP16
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
*   configured Endpoint Buffer Management. This parameter is defined in the
...
uint16 USBUART2_ReadOutEP16(uint8 epNumber, uint8 pData[], uint16 length)
{
    ...
    adjLength =  length + ((uint32)length & 1u);
    ...
    #if (USBUART2_EP_MANAGEMENT_MANUAL)
        {
            ...
            {
                dataBuf[i] = (uint16) USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16;
            }
        }
        ...
        #if (USBUART2_EP_MANAGEMENT_DMA_MANUAL)
        {
            ...
            /* Configure source and destination. */
            USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBUART2_CyDmaSetDstAddress(channelNum, USBUART2_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBUART2_CyDmaSetConfiguration(channelNum, USBUART2_DMA_DESCR0, USBUART2_DMA_COMMON_CFG | (uint16)((adjLength >> 1u) - 1u) |
        ...
        #if (USBUART2_EP_MANAGEMENT_DMA_AUTO)
        {
            ...
            /* Get number of full bursts. */
            USBUART2_DmaEpBurstCnt[epNumber] = (uint8) (adjLength / USBUART2_DMA_BYTES_PER_BURST);

            /* Get number of elements in last burst. */
            USBUART2_DmaEpLastBurstEl[epNumber] = (uint8) (adjLength % USBUART2_DMA_BYTES_PER_BURST);
            ...
            USBUART2_DmaEpLastBurstEl[epNumber] |= USBUART2_DMA_DESCR_16BITS;
            ...
            /* Set destination address. */
            USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR0, (void*) &USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBUART2_CyDmaSetSrcAddress(channelNum, USBUART2_DMA_DESCR1, (void*) &USBUART2_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
...
/*******************************************************************************
* Function Name: USBUART2_EnableOutEP
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBUART2_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* \reentrant
*  No.
*
...
/*******************************************************************************
* Function Name: USBUART2_DisableOutEP
****************************************************************************//**
*
...
*    State                      |Description
*    ---------------------------|----------------------------------------------
*    USBUART2_FORCE_J   | Force a J State onto the D+/D– lines
*    USBUART2_FORCE_K   | Force a K State onto the D+/D– lines
*    USBUART2_FORCE_SE0 | Force a Single Ended 0 onto the D+/D– lines
*    USBUART2_FORCE_NONE| Return bus to SIE control
*
*
*******************************************************************************/
void USBUART2_Force(uint8 bState) 
{
    ...
    */
    USBUART2_USBIO_CR0_REG = bState;
}


/*******************************************************************************
* Function Name: USBUART2_GetEPAckState
****************************************************************************//**
*
...
uint8 USBUART2_GetEPAckState(uint8 epNumber) 
{
    ...
    if ((epNumber > USBUART2_EP0) && (epNumber < USBUART2_MAX_EP))
    {
        cr = USBUART2_SIE_EP_BASE.sieEp[epNumber].epCr0 & USBUART2_MODE_ACKD;
    }
...
/*******************************************************************************
* Function Name: USBUART2_SetPowerStatus
****************************************************************************//**
*
...
*  --------------------------------------------|---------------------------
*  USBUART2_DEVICE_STATUS_BUS_POWERED  | Set the device to bus powered
*  USBUART2_DEVICE_STATUS_SELF_POWERED | Set the device to self powered
*
* \globalvars
*
*  \ref USBUART2_deviceStatus - set power status
*
* \reentrant
*  No.
*
...
{
    if (powerStatus != USBUART2_DEVICE_STATUS_BUS_POWERED)
    {
    ...
    /***************************************************************************
    * Function Name: USBUART2_VBusPresent
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return ((0u != (USBUART2_VBUS_STATUS_REG & USBUART2_VBUS_VALID)) ? (uint8) 1u : (uint8) 0u);
    }
#endif /* (USBUART2_VBUS_MONITORING_ENABLE) */


/*******************************************************************************
* Function Name: USBUART2_RWUEnabled
****************************************************************************//**
*
...
*
* \globalvars
*  USBUART2_deviceStatus - checked to determine remote status
*
*******************************************************************************/
uint8 USBUART2_RWUEnabled(void) 
{
    uint8 result = USBUART2_FALSE;

    if (0u != (USBUART2_deviceStatus & USBUART2_DEVICE_STATUS_REMOTE_WAKEUP))
    {
        result = USBUART2_TRUE;
    }
...
/*******************************************************************************
* Function Name: USBUART2_GetDeviceAddress
****************************************************************************//**
*
...
{
    return (uint8)(USBUART2_CR0_REG & USBUART2_CR0_DEVICE_ADDRESS_MASK);
}


/*******************************************************************************
* Function Name: USBUART2_EnableSofInt
****************************************************************************//**
*
...
void USBUART2_EnableSofInt(void) 
{
    ...
    USBUART2_INTR_SIE_MASK_REG |= (uint32) USBUART2_INTR_SIE_SOF_INTR;
#else
...
/*******************************************************************************
* Function Name: USBUART2_DisableSofInt
****************************************************************************//**
*
...
void USBUART2_DisableSofInt(void) 
{
    ...
    /***************************************************************************
    * Function Name: USBUART2_DetectPortType
    ************************************************************************//**
    *
    ...
    *   Return Value                      |Description
    *   ----------------------------------|-------------------------------------
    *   USBUART2_BCD_PORT_SDP     | Standard downstream port detected
    *   USBUART2_BCD_PORT_CDP     | Charging downstream port detected
    *   USBUART2_BCD_PORT_DCP     | Dedicated charging port detected
    *   USBUART2_BCD_PORT_UNKNOWN | Unable to detect charging port type (proprietary charger type)
    *   USBUART2_BCD_PORT_ERR     | Error condition in detection process
    *
    *
    * \sideeffects
    *
    ...
    ***************************************************************************/
    uint8 USBUART2_Bcd_DetectPortType(void)
    {
        ...
        uint8 result = USBUART2_BCD_PORT_UNKNOWN;
        ...
        */
        bkPwrCtrl = USBUART2_POWER_CTRL_REG;
        USBUART2_POWER_CTRL_REG = USBUART2_DEFAULT_POWER_CTRL_VBUS\
                            & (~USBUART2_POWER_CTRL_ENABLE_VBUS_PULLDOWN)\
                            & (~USBUART2_POWER_CTRL_ENABLE_DM_PULLDOWN);


        /* Enable PHY detector and single-ended and differential receivers.
         * Enable charger detection.  */
        USBUART2_POWER_CTRL_REG |= USBUART2_DEFAULT_POWER_CTRL_PHY\
                                         | USBUART2_POWER_CTRL_ENABLE_CHGDET;
        ...
        */
        #if defined (USBUART2_NO_DCD)
        CyDelay(USBUART2_BCD_TIMEOUT);
        #else
        /* DCD implementation:*/

        {
            uint16 timeout = USBUART2_BCD_TIMEOUT;
            uint8 connectionApproved = 0u;
            uint8 connected = 0u;

            /*   BCD spec 1.2: Turns on Idp_src and D- pull-down resistor */
            USBUART2_POWER_CTRL_REG |= USBUART2_POWER_CTRL_ENABLE_DM_PULLDOWN;
            USBUART2_CHGDET_CTRL_REG |= USBUART2_CHGDET_CTRL_DCD_SRC_EN;

            /* BCD spec 1.2: Waits for D+ to be low for a time of Tdcd_dbnc*/
            while ((0u != timeout) && (0u == connectionApproved))
            {
                if (0u == (USBUART2_USBIO_CR1_REG & USBUART2_USBIO_CR1_DP0))
                {
                    connected++;
                }
                else
                ...
                connectionApproved = (USBUART2_BCD_TDCD_DBNC < connected) ? 1u:0u;
                CyDelay(1u);
                timeout--;
            }
        ...
        #endif /*(USBUART2_NO_DCD)*/

        /* Primary detection: enable VDP_SRC on D+ and IDM_SINK on D-. */
        USBUART2_CHGDET_CTRL_REG = USBUART2_CHGDET_CTRL_PRIMARY;
        CyDelay(USBUART2_BCD_PRIMARY_WAIT);
        cr1RegVal = USBUART2_USBIO_CR1_REG;

        /* Check is it SDP or DCP/CDP, read comparator 2 output. */
        if (0u == (USBUART2_CHGDET_CTRL_REG & USBUART2_CHGDET_CTRL_COMP_OUT))
        {
            /* Check status of D- line. */
            if (0u == (cr1RegVal & USBUART2_USBIO_CR1_DM0))
            {
                result = USBUART2_BCD_PORT_SDP;
            }
            else
            {
        ...
        {
            USBUART2_CHGDET_CTRL_REG = USBUART2_CHGDET_CTRL_DEFAULT \
                                                | USBUART2_CHGDET_CTRL_SECONDARY;
            CyDelay(USBUART2_BCD_SECONDARY_WAIT);
            cr1RegVal = USBUART2_USBIO_CR1_REG;
            ...
            {
                /* Check status of D+ line. */
                if (0u == (cr1RegVal & USBUART2_USBIO_CR1_DP0))
                {
                    result = USBUART2_BCD_PORT_CDP;
                }
                else
                {
                    /* ERROR: such combination is impossible. Abort charger
            ...
            else
            {
                ...
                else
                {
                    ...
                    */
                    result = USBUART2_BCD_PORT_UNKNOWN;
                }
            }
        }

        /* Restore CHGDET_CTRL. */
        USBUART2_CHGDET_CTRL_REG = 0u;

        /*Revert registers back*/
        USBUART2_POWER_CTRL_REG = bkPwrCtrl;
        USBUART2_USBIO_CR1_REG |= (uint32) USBUART2_USBIO_CR1_IOMODE;
        USBUART2_USB_CLK_EN_REG = ~USBUART2_USB_CLK_CSR_CLK_EN;
...
#if (USBUART2_LPM_ACTIVE)
    /***************************************************************************
    * Function Name: USBUART2_Lpm_GetBeslValue
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBUART2_LPM_STAT_REG & USBUART2_LPM_STAT_LPM_BESL_MASK);
    }


    /***************************************************************************
    * Function Name: USBUART2_Lpm_RemoteWakeUpAllowed
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBUART2_LPM_STAT_REG & USBUART2_LPM_STAT_LPM_REMOTE_WAKE);
    }


    /***************************************************************************
    * Function Name: USBUART2_Lpm_SetResponse
    ************************************************************************//**
    *
    ...
    *       - USBUART2_LPM_REQ_ACK - next LPM request will be
    *                                           responded with ACK
    *       - USBUART2_LPM_REQ_NACK - next LPM request will be
    *                                           responded with NACK
    *       - USBUART2_LPM_REQ_NYET - next LPM request will be
    *                                           responded with NYET
    *
    ***************************************************************************/
    void USBUART2_Lpm_SetResponse(uint32 response)
    {
        uint32 lpmCtrl = USBUART2_LPM_CTRL_REG & (uint32) ~USBUART2_LPM_CTRL_ACK_NYET_MASK;

        USBUART2_LPM_CTRL_REG = lpmCtrl | ((uint32) response & USBUART2_LPM_CTRL_ACK_NYET_MASK);
    }


    /***************************************************************************
    * Function Name: USBUART2_Lpm_GetResponse
    ************************************************************************//**
    *
    ...
    *       - USBUART2_LPM_REQ_ACK - next LPM request will be responded
    *                                        with ACK
    *       - USBUART2_LPM_REQ_NACK - next LPM request will be responded
    *                                        with NACK
    *       - USBUART2_LPM_REQ_NYET - next LPM request will be responded
    *                                        with NYET
    *
    ***************************************************************************/
    uint32 USBUART2_Lpm_GetResponse(void)
    {
        ...
        return  ((uint32) USBUART2_LPM_CTRL_REG & (uint32)USBUART2_LPM_CTRL_ACK_NYET_MASK);
    }
