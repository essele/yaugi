...
* functions.
*/
uint8 USBTMC_initVar = 0u;

#if (USBTMC_EP_MANAGEMENT_DMA)
    #if (CY_PSOC4)
        static void USBTMC_InitEpDma(void);

        /* DMA chanels assigend for endpoints. */
        const uint8 USBTMC_DmaChan[USBTMC_MAX_EP] =
        {
            0u,
            0u,
            0u,
    ...
    #else
        /* DMA chanels assigend for endpoints. */
        uint8 USBTMC_DmaChan[USBTMC_MAX_EP];
        
        /* DMA TDs require for PSoC 3/5LP operation. */
        uint8 USBTMC_DmaTd[USBTMC_MAX_EP];
    #endif /* (CY_PSOC4) */
    ...
    uint8  USBTMC_DmaEpBurstCnt   [USBTMC_MAX_EP];
    
    /* Number of bytes to transfer in last DMA burst. */
    uint8  USBTMC_DmaEpLastBurstEl[USBTMC_MAX_EP];

    /* Storage for arrays above. */
    uint8  USBTMC_DmaEpBurstCntBackup  [USBTMC_MAX_EP];
    uint32 USBTMC_DmaEpBufferAddrBackup[USBTMC_MAX_EP];
     
    /* DMA trigger mux output for usb.dma_req[0-7]. */
    const uint8 USBTMC_DmaReqOut[USBTMC_MAX_EP] =
    {
        0u,
        USBTMC_ep1_dma__TR_OUTPUT,
        USBTMC_ep2_dma__TR_OUTPUT,
        USBTMC_ep3_dma__TR_OUTPUT,
        0u,
        0u,
        0u,
        0u,
    ...
    const uint8 USBTMC_DmaBurstEndOut[USBTMC_MAX_EP] =
    {
        0u,
        USBTMC_BURSTEND_0_TR_OUTPUT,
        USBTMC_BURSTEND_1_TR_OUTPUT,
        USBTMC_BURSTEND_2_TR_OUTPUT,
        USBTMC_BURSTEND_3_TR_OUTPUT,
        USBTMC_BURSTEND_4_TR_OUTPUT,
        USBTMC_BURSTEND_5_TR_OUTPUT,
        USBTMC_BURSTEND_6_TR_OUTPUT,
        USBTMC_BURSTEND_7_TR_OUTPUT
    };
    
#else
    #if (USBTMC_EP_DMA_AUTO_OPT == 0u)
        static uint8 clearInDataRdyStatus = USBTMC_ARB_EPX_CFG_DEFAULT;
        uint8 USBTMC_DmaNextTd[USBTMC_MAX_EP];
        const uint8 USBTMC_epX_TD_TERMOUT_EN[USBTMC_MAX_EP] =
        {
            0u,
            0u,
            0u,
        ...
        volatile uint16 USBTMC_inLength[USBTMC_MAX_EP];
        const uint8 *USBTMC_inDataPointer[USBTMC_MAX_EP];
        volatile uint8 USBTMC_inBufFull[USBTMC_MAX_EP];
...
*   following list.
*
*       *USBTMC_3V_OPERATION* - Disable voltage regulator and pass-
*                                      through Vcc for pull-up
*
*       *USBTMC_5V_OPERATION* - Enable voltage regulator and use
*                                      regulator for pull-up
*
*       *USBTMC_DWR_POWER_OPERATION* - Enable or disable the voltage
...
{
    if (0u == USBTMC_initVar)
    {
        USBTMC_Init();
        USBTMC_initVar = 1u;
    }

    USBTMC_InitComponent(device, mode);
}


/*******************************************************************************
* Function Name: USBTMC_Init
****************************************************************************//**
*
...
* USBTMC_Init() because the USBTMC_Start() routine calls
...
void USBTMC_Init(void) 
{
    ...
    * USBTMC_InitComponent() if it is required.
    */

    /* Enable USBIO control on drive mode of D+ and D- pins. */
    USBTMC_USBIO_CR1_REG &= ~ (uint32) USBTMC_USBIO_CR1_IOMODE;

    /* Set number of LF CLK to detect UBS bus reset. */
    USBTMC_BUS_RST_CNT_REG = USBTMC_DEFUALT_BUS_RST_CNT;
    ...
    */
    USBTMC_POWER_CTRL_REG = USBTMC_DEFAULT_POWER_CTRL_VBUS;

    /* Enable PHY detector and single-ended and differential receivers. */
    USBTMC_POWER_CTRL_REG |= USBTMC_DEFAULT_POWER_CTRL_PHY;
    ...
    USBTMC_CR1_REG = USBTMC_DEFUALT_CR1;
    ...
    USBTMC_INTR_SIE_MASK_REG = USBTMC_DEFAULT_INTR_SIE_MASK;
    ...
    USBTMC_CR1_REG = USBTMC_CR1_ENABLE_LOCK;
    ...
    */
    ...
    USBTMC_USBIO_CR1_REG &= (uint8) ~USBTMC_USBIO_CR1_IOMODE;
    ...
    CyDelayUs(USBTMC_WAIT_REG_STABILITY_1US);
    ...
    USBTMC_DM_INP_DIS_REG &= (uint8) ~USBTMC_DM_MASK;
    USBTMC_DP_INP_DIS_REG &= (uint8) ~USBTMC_DP_MASK;
    ...
    CY_SET_REG8(USBTMC_ARB_RW1_WA_PTR,     0u);
    CY_SET_REG8(USBTMC_ARB_RW1_WA_MSB_PTR, 0u);

#if (USBTMC_EP_MANAGEMENT_DMA)
    /* Initialize transfer descriptor. This will be used to detect DMA state - initialized or not. */
    for (i = 0u; i < USBTMC_MAX_EP; ++i)
    {
        USBTMC_DmaTd[i] = DMA_INVALID_TD;

    #if (USBTMC_EP_MANAGEMENT_DMA_AUTO && (USBTMC_EP_DMA_AUTO_OPT == 0u))
        USBTMC_DmaNextTd[i] = DMA_INVALID_TD;
    ...
    CyIntSetPriority     (USBTMC_INTR_HI_VECT_NUM,  USBTMC_INTR_HI_PRIORITY);
    (void) CyIntSetVector(USBTMC_INTR_HI_VECT_NUM, &USBTMC_INTR_HI_ISR);

    /* Configure lo_int: set handler and priority. */
    CyIntSetPriority     (USBTMC_INTR_LO_VECT_NUM,  USBTMC_INTR_LO_PRIORITY);
    (void) CyIntSetVector(USBTMC_INTR_LO_VECT_NUM, &USBTMC_INTR_LO_ISR);

    /* Configure med_int: set handler and priority (routed through DSI). */
    CyIntSetPriority     (USBTMC_INTR_MED_VECT_NUM,  USBTMC_INTR_MED_PRIORITY);
    (void) CyIntSetVector(USBTMC_INTR_MED_VECT_NUM, &USBTMC_INTR_MED_ISR);

#else
    /* Set bus reset interrupt. */
    CyIntSetPriority(USBTMC_BUS_RESET_VECT_NUM, USBTMC_BUS_RESET_PRIOR);
    (void) CyIntSetVector(USBTMC_BUS_RESET_VECT_NUM,   &USBTMC_BUS_RESET_ISR);

    /* Set Control Endpoint Interrupt. */
    CyIntSetPriority(USBTMC_EP_0_VECT_NUM, USBTMC_EP_0_PRIOR);
    (void) CyIntSetVector(USBTMC_EP_0_VECT_NUM,   &USBTMC_EP_0_ISR);

    /* Set SOF interrupt. */
    #if (USBTMC_SOF_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_SOF_VECT_NUM,  USBTMC_SOF_PRIOR);
        (void) CyIntSetVector(USBTMC_SOF_VECT_NUM, &USBTMC_SOF_ISR);
    #endif /* (USBTMC_SOF_ISR_ACTIVE) */

    /* Set Data Endpoint 1 Interrupt. */
    #if (USBTMC_EP1_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_1_VECT_NUM,  USBTMC_EP_1_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_1_VECT_NUM,  &USBTMC_EP_1_ISR);
    #endif /* (USBTMC_EP1_ISR_ACTIVE) */

    /* Set Data Endpoint 2 Interrupt. */
    #if (USBTMC_EP2_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_2_VECT_NUM,  USBTMC_EP_2_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_2_VECT_NUM, &USBTMC_EP_2_ISR);
    #endif /* (USBTMC_EP2_ISR_ACTIVE) */

    /* Set Data Endpoint 3 Interrupt. */
    #if (USBTMC_EP3_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_3_VECT_NUM,  USBTMC_EP_3_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_3_VECT_NUM, &USBTMC_EP_3_ISR);
    #endif /* (USBTMC_EP3_ISR_ACTIVE) */

    /* Set Data Endpoint 4 Interrupt. */
    #if (USBTMC_EP4_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_4_VECT_NUM,  USBTMC_EP_4_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_4_VECT_NUM, &USBTMC_EP_4_ISR);
    #endif /* (USBTMC_EP4_ISR_ACTIVE) */

    /* Set Data Endpoint 5 Interrupt. */
    #if (USBTMC_EP5_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_5_VECT_NUM,  USBTMC_EP_5_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_5_VECT_NUM, &USBTMC_EP_5_ISR);
    #endif /* (USBTMC_EP5_ISR_ACTIVE) */

    /* Set Data Endpoint 6 Interrupt. */
    #if (USBTMC_EP6_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_6_VECT_NUM,  USBTMC_EP_6_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_6_VECT_NUM, &USBTMC_EP_6_ISR);
    #endif /* (USBTMC_EP6_ISR_ACTIVE) */

     /* Set Data Endpoint 7 Interrupt. */
    #if (USBTMC_EP7_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_7_VECT_NUM,  USBTMC_EP_7_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_7_VECT_NUM, &USBTMC_EP_7_ISR);
    #endif /* (USBTMC_EP7_ISR_ACTIVE) */

    /* Set Data Endpoint 8 Interrupt. */
    #if (USBTMC_EP8_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_EP_8_VECT_NUM,  USBTMC_EP_8_PRIOR);
        (void) CyIntSetVector(USBTMC_EP_8_VECT_NUM, &USBTMC_EP_8_ISR);
    #endif /* (USBTMC_EP8_ISR_ACTIVE) */

    /* Set ARB Interrupt. */
    #if (USBTMC_EP_MANAGEMENT_DMA && USBTMC_ARB_ISR_ACTIVE)
        CyIntSetPriority     (USBTMC_ARB_VECT_NUM,  USBTMC_ARB_PRIOR);
        (void) CyIntSetVector(USBTMC_ARB_VECT_NUM, &USBTMC_ARB_ISR);
    #endif /* (USBTMC_EP_MANAGEMENT_DMA && USBTMC_ARB_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

    /* Common: Configure GPIO interrupt for wakeup. */
#if (USBTMC_DP_ISR_ACTIVE)
    CyIntSetPriority     (USBTMC_DP_INTC_VECT_NUM,  USBTMC_DP_INTC_PRIORITY);
    (void) CyIntSetVector(USBTMC_DP_INTC_VECT_NUM, &USBTMC_DP_ISR);
#endif /* (USBTMC_DP_ISR_ACTIVE) */

#if (USBTMC_EP_MANAGEMENT_DMA && CY_PSOC4)
    /* Initialize DMA channels. */
    USBTMC_InitEpDma();
#endif /* (USBTMC_EP_MANAGEMENT_DMA && CY_PSOC4) */
}


/*******************************************************************************
* Function Name: USBTMC_InitComponent
****************************************************************************//**
*
...
* \globalvars
*   \ref USBTMC_device
*   \ref USBTMC_transferState
*   \ref USBTMC_configuration
*   \ref USBTMC_deviceStatus
*
*   \ref USBTMC_deviceAddress - Contains the current device address. This
...
*      SET_ADDRESS request.
*
*   \ref USBTMC_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
void USBTMC_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
    ...
    {
        USBTMC_hidProtocol[i] = USBTMC_PROTOCOL_REPORT;
    }
#endif /* USBTMC_ENABLE_HID_CLASS */

    /* Store device number to access descriptor. */
    USBTMC_device = device;

    /* Reset component internal variables. */
    USBTMC_transferState   = USBTMC_TRANS_STATE_IDLE;
    USBTMC_configurationChanged = 0u;
    USBTMC_configuration   = 0u;
    USBTMC_interfaceNumber = 0u;
    USBTMC_deviceAddress   = 0u;
    USBTMC_deviceStatus    = 0u;
    USBTMC_lastPacketSize  = 0u;

    /* Enable component interrupts. */
#if (CY_PSOC4)
    CyIntEnable(USBTMC_INTR_HI_VECT_NUM);
    CyIntEnable(USBTMC_INTR_MED_VECT_NUM);
    CyIntEnable(USBTMC_INTR_LO_VECT_NUM);
#else
    CyIntEnable(USBTMC_BUS_RESET_VECT_NUM);
    CyIntEnable(USBTMC_EP_0_VECT_NUM);

    #if (USBTMC_SOF_ISR_ACTIVE)
        CyIntEnable(USBTMC_SOF_VECT_NUM);
    #endif /* (USBTMC_SOF_ISR_ACTIVE) */

    #if (USBTMC_EP1_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_1_VECT_NUM);
    #endif /* (USBTMC_EP1_ISR_ACTIVE) */

    #if (USBTMC_EP2_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_2_VECT_NUM);
    #endif /* (USBTMC_EP5_ISR_ACTIVE) */

    #if (USBTMC_EP3_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_3_VECT_NUM);
    #endif /* (USBTMC_EP5_ISR_ACTIVE) */

    #if (USBTMC_EP4_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_4_VECT_NUM);
    #endif /* (USBTMC_EP5_ISR_ACTIVE) */

    #if (USBTMC_EP5_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_5_VECT_NUM);
    #endif /* (USBTMC_EP5_ISR_ACTIVE) */

    #if (USBTMC_EP6_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_6_VECT_NUM);
    #endif /* USBTMC_EP6_ISR_REMOVE */

    #if (USBTMC_EP7_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_7_VECT_NUM);
    #endif /* (USBTMC_EP7_ISR_ACTIVE) */

    #if (USBTMC_EP8_ISR_ACTIVE)
        CyIntEnable(USBTMC_EP_8_VECT_NUM);
    #endif /* (USBTMC_EP8_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */
    ...
    USBTMC_ARB_INT_EN_REG = USBTMC_DEFAULT_ARB_INT_EN;
        ...
        CyIntEnable(USBTMC_ARB_VECT_NUM);
    ...
    USBTMC_ARB_CFG_REG = USBTMC_DEFAULT_ARB_CFG;
        ...
        CyDmaEnable();
            ...
            /* Change DMA priority to be highest. */
             CyIntSetPriority(CYDMA_INTR_NUMBER, USBTMC_DMA_AUTO_INTR_PRIO);
        #endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (CY_PSOC4) */
            ...
            CyIntEnable(CYDMA_INTR_NUMBER);
                ...
                USBTMC_EP_DMA_Done_isr_StartEx(&USBTMC_EP_DMA_DONE_ISR);
                USBTMC_EP17_DMA_Done_SR_InterruptEnable();
                USBTMC_EP8_DMA_Done_SR_InterruptEnable();
            #endif /* (USBTMC_EP_DMA_AUTO_OPT == 0u) */
        #endif /* (CY_PSOC4) */
    #endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO) */
#endif /* (USBTMC_EP_MANAGEMENT_DMA) */
...
/*******************************************************************************
* Function Name: USBTMC_ReInitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   USBTMC_device - Contains the device number of the desired Device
...
*   USBTMC_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
*   \ref USBTMC_deviceStatus
*
*   \ref USBTMC_configurationChanged
*
*   USBTMC_intiVar -  This variable is set to zero
*
*******************************************************************************/
void USBTMC_Stop(void) 
{
    ...
    USBTMC_CR0_REG &= (uint32) ~USBTMC_CR0_ENABLE;

    /* Disable D+ pull-up. */
    USBTMC_USBIO_CR1_REG &= (uint32) ~ USBTMC_USBIO_CR1_USBPUEN;

#if (CY_PSOC4)
    /* Disable USBFS block. */
    USBTMC_POWER_CTRL_REG &= (uint32) ~USBTMC_POWER_CTRL_ENABLE;
#else
...
#if (CY_PSOC4)
    CyIntDisable(USBTMC_INTR_HI_VECT_NUM);
    CyIntDisable(USBTMC_INTR_LO_VECT_NUM);
    CyIntDisable(USBTMC_INTR_MED_VECT_NUM);
#else

    CyIntDisable(USBTMC_BUS_RESET_VECT_NUM);
    CyIntDisable(USBTMC_EP_0_VECT_NUM);

    #if (USBTMC_SOF_ISR_ACTIVE)
        CyIntDisable(USBTMC_SOF_VECT_NUM);
    #endif /* (USBTMC_SOF_ISR_ACTIVE) */

    #if (USBTMC_EP1_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_1_VECT_NUM);
    #endif /* (USBTMC_EP1_ISR_ACTIVE) */

    #if (USBTMC_EP2_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_2_VECT_NUM);
    #endif /* (USBTMC_EP2_ISR_ACTIVE) */

    #if (USBTMC_EP3_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_3_VECT_NUM);
    #endif /* (USBTMC_EP3_ISR_ACTIVE) */

    #if (USBTMC_EP4_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_4_VECT_NUM);
    #endif /* (USBTMC_EP4_ISR_ACTIVE) */

    #if (USBTMC_EP5_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_5_VECT_NUM);
    #endif /* (USBTMC_EP5_ISR_ACTIVE) */

    #if (USBTMC_EP6_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_6_VECT_NUM);
    #endif /* USBTMC_EP6_ISR_REMOVE */

    #if (USBTMC_EP7_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_7_VECT_NUM);
    #endif /* (USBTMC_EP7_ISR_ACTIVE) */

    #if (USBTMC_EP8_ISR_ACTIVE)
        CyIntDisable(USBTMC_EP_8_VECT_NUM);
    #endif /* (USBTMC_EP8_ISR_ACTIVE) */
...
/*******************************************************************************
* Function Name: USBTMC_CheckActivity
****************************************************************************//**
*
...
{
    uint32 cr1Reg = USBTMC_CR1_REG;

    /* Clear bus activity. */
    USBTMC_CR1_REG = (cr1Reg & (uint32) ~USBTMC_CR1_BUS_ACTIVITY);

    /* Get bus activity. */
    return ((0u != (cr1Reg & USBTMC_CR1_BUS_ACTIVITY)) ? (1u) : (0u));
}


/*******************************************************************************
* Function Name: USBTMC_GetConfiguration
****************************************************************************//**
*
...
{
    return (USBTMC_configuration);
}


/*******************************************************************************
* Function Name: USBTMC_IsConfigurationChanged
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBTMC_configurationChanged - This variable is set to 1 after
...
uint8 USBTMC_IsConfigurationChanged(void) 
{
    ...
    if (USBTMC_configurationChanged != 0u)
    {
        res = USBTMC_configurationChanged;
        USBTMC_configurationChanged = 0u;
    }
...
/*******************************************************************************
* Function Name: USBTMC_GetInterfaceSetting
****************************************************************************//**
*
...
{
    return (USBTMC_interfaceSetting[interfaceNumber]);
}


/*******************************************************************************
* Function Name: USBTMC_GetEPState
****************************************************************************//**
*
...
{
    return (USBTMC_EP[epNumber].apiEpState);
}


/*******************************************************************************
* Function Name: USBTMC_GetEPCount
****************************************************************************//**
*
...
uint16 USBTMC_GetEPCount(uint8 epNumber) 
{
    ...
    if ((epNumber > USBTMC_EP0) && (epNumber < USBTMC_MAX_EP))
    {
        ...
        cntr  = ((uint16) USBTMC_SIE_EP_BASE.sieEp[epNumber].epCnt0) & USBTMC_EPX_CNT0_MASK;
        cntr  = ((uint16) (cntr << 8u)) | ((uint16) USBTMC_SIE_EP_BASE.sieEp[epNumber].epCnt1);
        cntr -= USBTMC_EPX_CNTX_CRC_COUNT;
    }
...
#if (CY_PSOC4)
    /*******************************************************************************
    * Function Name: USBTMC_InitEpDma
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static void USBTMC_InitEpDma(void)
    {
        ...
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep1_dma_CHANNEL] = USBTMC_ep1_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA1_ACTIVE) */

    #if (USBTMC_DMA2_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep2_dma_CHANNEL] = USBTMC_ep2_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA2_ACTIVE) */

    #if (USBTMC_DMA3_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep3_dma_CHANNEL] = USBTMC_ep3_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA3_ACTIVE) */

    #if (USBTMC_DMA4_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep4_dma_CHANNEL] = USBTMC_ep4_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA4_ACTIVE) */

    #if (USBTMC_DMA5_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep5_dma_CHANNEL] = USBTMC_ep5_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA5_ACTIVE) */

    #if (USBTMC_DMA6_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep6_dma_CHANNEL] = USBTMC_ep6_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA6_ACTIVE) */

    #if (USBTMC_DMA7_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep7_dma_CHANNEL] = USBTMC_ep7_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA7_ACTIVE) */

    #if (USBTMC_DMA8_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBTMC_ep8_dma_CHANNEL] = USBTMC_ep8_dma_CHANNEL_CFG;
    #endif /* (USBTMC_DMA8_ACTIVE) */

    #if (USBTMC_EP_MANAGEMENT_DMA_AUTO)
        /* Initialize DMA channel callbacks. */
        #if (USBTMC_DMA1_ACTIVE)
            (void) USBTMC_ep1_dma_SetInterruptCallback(&USBTMC_EP1_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA1_ACTIVE) */

        #if (USBTMC_DMA2_ACTIVE)
            (void) USBTMC_ep2_dma_SetInterruptCallback(&USBTMC_EP2_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA2_ACTIVE) */

        #if (USBTMC_DMA3_ACTIVE)
            (void) USBTMC_ep3_dma_SetInterruptCallback(&USBTMC_EP3_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA3_ACTIVE) */

        #if (USBTMC_DMA4_ACTIVE)
            (void) USBTMC_ep4_dma_SetInterruptCallback(&USBTMC_EP4_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA4_ACTIVE) */

        #if (USBTMC_DMA5_ACTIVE)
            (void) USBTMC_ep5_dma_SetInterruptCallback(&USBTMC_EP5_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA5_ACTIVE) */

        #if (USBTMC_DMA6_ACTIVE)
            (void) USBTMC_ep6_dma_SetInterruptCallback(&USBTMC_EP6_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA6_ACTIVE) */

        #if (USBTMC_DMA7_ACTIVE)
            (void) USBTMC_ep7_dma_SetInterruptCallback(&USBTMC_EP7_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA7_ACTIVE) */

        #if (USBTMC_DMA8_ACTIVE)
            (void) USBTMC_ep8_dma_SetInterruptCallback(&USBTMC_EP8_DMA_DONE_ISR);
        #endif /* (USBTMC_DMA8_ACTIVE) */
    #endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO) */
    }
#else
    ...
    /***************************************************************************
    * Function Name: USBTMC_InitEP_DMA
    ************************************************************************//**
    *
    *  This function allocates and initializes a DMA channel to be used by the
    *  USBTMC_LoadInEP() or USBTMC_ReadOutEP() APIs for data
    ...
    ***************************************************************************/
                                                                    ...
                                                                    
    {
    ...
    #if (USBTMC_DMA1_ACTIVE)
        case USBTMC_EP1:
            USBTMC_DmaChan[epNumber] = USBTMC_ep1_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                    USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA1_ACTIVE) */

    #if (USBTMC_DMA2_ACTIVE)
        case USBTMC_EP2:
            USBTMC_DmaChan[epNumber] = USBTMC_ep2_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                    USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA2_ACTIVE) */
    ...
    #if (USBTMC_DMA3_ACTIVE)
            case USBTMC_EP3:
                USBTMC_DmaChan[epNumber] = USBTMC_ep3_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                        USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA3_ACTIVE) */
    ...
    #if (USBTMC_DMA4_ACTIVE)
            case USBTMC_EP4:
                USBTMC_DmaChan[epNumber] = USBTMC_ep4_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                        USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA4_ACTIVE) */
    ...
    #if (USBTMC_DMA5_ACTIVE)
            case USBTMC_EP5:
                USBTMC_DmaChan[epNumber] = USBTMC_ep5_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                        USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA5_ACTIVE) */
    ...
    #if (USBTMC_DMA6_ACTIVE)
        case USBTMC_EP6:
            USBTMC_DmaChan[epNumber] = USBTMC_ep6_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                    USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA6_ACTIVE) */
    ...
    #if (USBTMC_DMA7_ACTIVE)
        case USBTMC_EP7:
                USBTMC_DmaChan[epNumber] = USBTMC_ep7_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                        USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA7_ACTIVE) */
    ...
    #if (USBTMC_DMA8_ACTIVE)
        case USBTMC_EP8:
                USBTMC_DmaChan[epNumber] = USBTMC_ep8_DmaInitialize(USBTMC_DMA_BYTES_PER_BURST,
                                                                                        USBTMC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBTMC_DMA8_ACTIVE) */
        ...
        if ((epNumber > USBTMC_EP0) && (epNumber < USBTMC_MAX_EP))
        {
            USBTMC_DmaTd[epNumber] = CyDmaTdAllocate();

        #if (USBTMC_EP_MANAGEMENT_DMA_AUTO && (USBTMC_EP_DMA_AUTO_OPT == 0u))
            USBTMC_DmaNextTd[epNumber] = CyDmaTdAllocate();
        #endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO && (USBTMC_EP_DMA_AUTO_OPT == 0u)) */
        }
    }
#endif /* (CY_PSOC4) */
    ...
    *  to trigger DMA re-configuration when USBTMC_LoadInEP() or 
    *  USBTMC_ReadOutEP() functions are called the first time. 
    *  
    ...
    ***************************************************************************/
    void USBTMC_Stop_DMA(uint8 epNumber) 
    {
        ...
        i = (epNumber < USBTMC_MAX_EP) ? epNumber : USBTMC_EP1;

        do
        {
        #if (CY_PSOC4)
            if (0u != USBTMC_DmaChan[i])
            {
                USBTMC_CyDmaChDisable(USBTMC_DmaChan[i]);
            }
        #else
            if(USBTMC_DmaTd[i] != DMA_INVALID_TD)
            {
                (void) CyDmaChDisable(USBTMC_DmaChan[i]);
                CyDmaTdFree(USBTMC_DmaTd[i]);
                USBTMC_DmaTd[i] = DMA_INVALID_TD;
            }

            #if (USBTMC_EP_MANAGEMENT_DMA_AUTO && (USBTMC_EP_DMA_AUTO_OPT == 0u))
                if(USBTMC_DmaNextTd[i] != DMA_INVALID_TD)
                {
                    CyDmaTdFree(USBTMC_DmaNextTd[i]);
                    USBTMC_DmaNextTd[i] = DMA_INVALID_TD;
                }
            #endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO && (USBTMC_EP_DMA_AUTO_OPT == 0u)) */
        ...
        while ((i < USBTMC_MAX_EP) && (epNumber == USBTMC_MAX_EP));
    }
#endif /* (USBTMC_EP_MANAGEMENT_DMA) */
...
#if (USBTMC_EP_MANAGEMENT_DMA_AUTO && (USBTMC_EP_DMA_AUTO_OPT == 0u))
    /***************************************************************************
    * Function Name: USBTMC_LoadNextInEP
    ************************************************************************//**
    *
    * Summary:
    ...
    ***************************************************************************/
    void USBTMC_LoadNextInEP(uint8 epNumber, uint8 mode) 
    {
        ...
        if (mode == 0u)
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBTMC_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBTMC_inLength[epNumber] - USBTMC_DMA_BYTES_PER_BURST);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBTMC_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert, LO16((uint32)USBTMC_inDataPointer[epNumber] +
                                            USBTMC_DMA_BYTES_PER_BURST));
            USBTMC_inBufFull[epNumber] = 1u;
        }
        else
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBTMC_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBTMC_DMA_BYTES_REPEAT);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBTMC_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert,  LO16((uint32)USBTMC_inDataPointer[epNumber] +
                                   (USBTMC_DMA_BYTES_PER_BURST - USBTMC_DMA_BYTES_REPEAT)));
        }

        /* CyDmaChSetInitialTd API is optimized to initialize TD. */
        CY_DMA_CH_STRUCT_PTR[USBTMC_DmaChan[epNumber]].basic_status[1u] = USBTMC_DmaTd[epNumber];
    }
#endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO && (USBTMC_EP_DMA_AUTO_OPT == 0u)) */
#endif /* (CY_PSOC3 || CY_PSOC5) */
                                                                        ...
                                                                        
{
    if ((epNumber > USBTMC_EP0) && (epNumber < USBTMC_MAX_EP))
    {
        ...
        if (length > (USBTMC_EPX_DATA_BUF_MAX - USBTMC_EP[epNumber].buffOffset))
        {
            length = USBTMC_EPX_DATA_BUF_MAX - USBTMC_EP[epNumber].buffOffset;
        }
    #endif /* (!USBTMC_EP_MANAGEMENT_DMA_AUTO) */

        /* Set count and data toggle. */
        USBTMC_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint32) HI8(length) | USBTMC_EP[epNumber].epToggle;
        USBTMC_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint32) LO8(length);

    #if (USBTMC_EP_MANAGEMENT_MANUAL)
        if (NULL != pData)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr = pData[i];
            }
        }

        /* IN endpoint buffer is full - read to be read. */
        USBTMC_EP[epNumber].apiEpState = USBTMC_NO_EVENT_PENDING;

        /* Arm IN endpoint. */
        USBTMC_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBTMC_EP[epNumber].epMode;

    #else

        #if (CY_PSOC3 || CY_PSOC5LP)
            /* Initialize DMA if it was not initialized. */
            if (DMA_INVALID_TD == USBTMC_DmaTd[epNumber])
            {
                USBTMC_InitEP_DMA(epNumber, pData);
            }
            ...
            {
            #if (CY_PSOC4)
                uint32 channelNum = (uint32) USBTMC_DmaChan[epNumber];

                /* Disable DMA channel: start configuration. */
                USBTMC_CyDmaChDisable(channelNum);

                /* Configure source and destination. */
                USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR0, (void*) pData);
                USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                --length;
                USBTMC_CyDmaSetConfiguration(channelNum, USBTMC_DMA_DESCR0, USBTMC_DMA_COMMON_CFG | length |
                ...
                (void) CyDmaChDisable(USBTMC_DmaChan[epNumber]);
                (void) CyDmaTdSetConfiguration(USBTMC_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USBTMC_DmaTd[epNumber], LO16((uint32) pData), LO16((uint32) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr));

                /* Enable DMA channel. */
                (void) CyDmaChSetInitialTd(USBTMC_DmaChan[epNumber], USBTMC_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBTMC_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */

                /* Generate DMA request. */
                USBTMC_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint32)  USBTMC_ARB_EPX_CFG_DMA_REQ;
                USBTMC_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint32) ~USBTMC_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                USBTMC_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
                ...
                USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr);
                USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR1, (void*) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                USBTMC_CyDmaSetConfiguration(channelNum, USBTMC_DMA_DESCR0, USBTMC_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Configure DMA descriptor. */
                USBTMC_CyDmaSetConfiguration(channelNum, USBTMC_DMA_DESCR1, USBTMC_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Enable interrupt from DMA channel. */
                USBTMC_CyDmaSetInterruptMask(channelNum);
                ...
                (void) CyDmaChDisable(USBTMC_DmaChan[epNumber]);
                ...
                #if (USBTMC_EP_DMA_AUTO_OPT == 0u)
                    USBTMC_inLength[epNumber] = length;
                    USBTMC_inDataPointer[epNumber] = pData;

                    /* Configure DMA to send data only for first burst */
                    (void) CyDmaTdSetConfiguration(USBTMC_DmaTd[epNumber],
                        (length > USBTMC_DMA_BYTES_PER_BURST) ? USBTMC_DMA_BYTES_PER_BURST : length,
                        USBTMC_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBTMC_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    ...
                    */
                    (void) CyDmaTdSetConfiguration(USBTMC_DmaNextTd[epNumber], 1u,
                                                   USBTMC_DmaNextTd[epNumber],
                                                   USBTMC_epX_TD_TERMOUT_EN[epNumber]);

                    /* Configure DmaNextTd to clear Data Ready status. */
                    (void) CyDmaTdSetAddress(USBTMC_DmaNextTd[epNumber], LO16((uint32) &clearInDataRdyStatus),
                                                                                   LO16((uint32) &USBTMC_ARB_EP_BASE.arbEp[epNumber].epCfg));
                #else
                    /* Configure DMA to send all data. */
                    (void) CyDmaTdSetConfiguration(USBTMC_DmaTd[epNumber], length,
                                                   USBTMC_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBTMC_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr));
                #endif /* (USBTMC_EP_DMA_AUTO_OPT == 0u) */

                /* Clear any potential pending DMA requests before starting DMA channel to transfer data. */
                (void) CyDmaClearPendingDrq(USBTMC_DmaChan[epNumber]);
                /* Enable DMA. */
                (void) CyDmaChSetInitialTd(USBTMC_DmaChan[epNumber], USBTMC_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBTMC_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    USBTMC_DmaEpBurstCnt[epNumber] = (uint8) (length / USBTMC_DMA_BYTES_PER_BURST);

                    /* Get number of elements in the last burst. */
                    USBTMC_DmaEpLastBurstEl[epNumber] = (uint8) (length % USBTMC_DMA_BYTES_PER_BURST);

                    /* Get total number of bursts. */
                    USBTMC_DmaEpBurstCnt[epNumber] += (0u != USBTMC_DmaEpLastBurstEl[epNumber]) ? 1u : 0u;

                    /* Adjust number of data elements transferred in last burst. */
                    USBTMC_DmaEpLastBurstEl[epNumber] = (0u != USBTMC_DmaEpLastBurstEl[epNumber]) ?
                                                                          (USBTMC_DmaEpLastBurstEl[epNumber] - 1u) :
                                                                          (USBTMC_DMA_BYTES_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBTMC_DmaEpBurstCnt[epNumber]) ? USBTMC_DmaEpLastBurstEl[epNumber] : (USBTMC_DMA_BYTES_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBTMC_DmaEpBurstCnt[epNumber]) ? USBTMC_DmaEpLastBurstEl[epNumber] : (USBTMC_DMA_BYTES_PER_BURST - 1u);


                    /* Mark which descriptor is last one to execute. */
                    USBTMC_DmaEpLastBurstEl[epNumber] |= (0u != (USBTMC_DmaEpBurstCnt[epNumber] & 0x1u)) ?
                                                                            USBTMC_DMA_DESCR0_MASK : USBTMC_DMA_DESCR1_MASK;

                    /* Restore DMA settings for current transfer. */
                    USBTMC_CyDmaChDisable(channelNum);

                    /* Restore destination address for input endpoint. */
                    USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR0, (void*) ((uint32) USBTMC_DmaEpBufferAddrBackup[epNumber]));
                    USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR1, (void*) ((uint32) USBTMC_DmaEpBufferAddrBackup[epNumber] +
                                                                                                                   USBTMC_DMA_BYTES_PER_BURST));

                    /* Set number of elements to transfer. */
                    USBTMC_CyDmaSetNumDataElements(channelNum, USBTMC_DMA_DESCR0, lengthDescr0);
                    USBTMC_CyDmaSetNumDataElements(channelNum, USBTMC_DMA_DESCR1, lengthDescr1);
                    ...
                    if (USBTMC_DmaEpBurstCnt[epNumber] > 1u)
                    {
                    ...
                    USBTMC_DmaEpBurstCnt[epNumber] = USBTMC_DMA_GET_BURST_CNT(USBTMC_DmaEpBurstCnt[epNumber]);

                    /* Enable DMA channel: configuration complete. */
                    USBTMC_CyDmaChEnable(channelNum);

                #elif (USBTMC_EP_DMA_AUTO_OPT == 0u)
                    USBTMC_inLength[epNumber]  = length;
                    USBTMC_inBufFull[epNumber] = 0u;

                    (void) CyDmaChDisable(USBTMC_DmaChan[epNumber]);
                    /* Configure DMA to send data only for first burst. */
                    (void) CyDmaTdSetConfiguration(
                        USBTMC_DmaTd[epNumber], (length > USBTMC_DMA_BYTES_PER_BURST) ?
                        USBTMC_DMA_BYTES_PER_BURST : length,
                        USBTMC_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(USBTMC_DmaTd[epNumber], LO16((uint32)  USBTMC_inDataPointer[epNumber]),
                                                                               LO16((uint32) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    /* Clear Any potential pending DMA requests before starting DMA channel to transfer data. */
                    (void) CyDmaClearPendingDrq(USBTMC_DmaChan[epNumber]);
                    /* Enable DMA. */
                ...
                #if !defined (USBTMC_MANUAL_IN_EP_ARM)
                    /* Set IN data ready to generate DMA request to load data into endpoint buffer. */
                    USBTMC_ARB_EP_BASE.arbEp[epNumber].epCfg |= USBTMC_ARB_EPX_CFG_IN_DATA_RDY;
                #endif  /* (USBTMC_MANUAL_IN_EP_ARM) */
        ...
        #endif /* (USBTMC_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (USBTMC_EP_MANAGEMENT_MANUAL) */
    }
}


/*******************************************************************************
* Function Name: USBTMC_ReadOutEP
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
...
*   transfer after calling the USBTMC_ReadOutEP() API. For example,
*   by checking EPstate:
*
...
{
    if ((pData != NULL) && (epNumber > USBTMC_EP0) && (epNumber < USBTMC_MAX_EP))
    {
    #if (!USBTMC_EP_MANAGEMENT_DMA_AUTO)
        /* Adjust requested length to available data. */
        length = (length > USBTMC_GetEPCount(epNumber)) ? USBTMC_GetEPCount(epNumber) : length;
    #endif /* (!USBTMC_EP_MANAGEMENT_DMA_AUTO) */
    ...
    #if (USBTMC_EP_MANAGEMENT_MANUAL)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                pData[i] = (uint8) USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr;
            }
        }
        ...
        #if (CY_PSOC4)
        {
            ...
            /* Configure source and destination. */
            USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBTMC_CyDmaSetConfiguration(channelNum, USBTMC_DMA_DESCR0, USBTMC_DMA_COMMON_CFG | (uint16)(length - 1u) |
            ...
            /* Configure DMA to transfer data. */
            (void) CyDmaChDisable(USBTMC_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBTMC_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBTMC_DmaTd[epNumber], LO16((uint32) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32)pData));
        ...
        #if (CY_PSOC4)
        {
            ...
            USBTMC_DmaEpLastBurstEl[epNumber] |= (USBTMC_DmaEpBurstCnt[epNumber] > 2u) ? USBTMC_DMA_DESCR_REVERT : 0u;
            ...
            USBTMC_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
            USBTMC_DmaEpBurstCntBackup[epNumber]   = USBTMC_DmaEpBurstCnt[epNumber];
            ...
            /* Set destination address. */
            USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR1, (void*) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr);

            USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR0, (void*) ((uint32) pData));
            USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR1, (void*) ((uint32) pData + USBTMC_DMA_BYTES_PER_BURST));

            /* Configure DMA descriptor. */
            USBTMC_CyDmaSetConfiguration(channelNum, USBTMC_DMA_DESCR0, USBTMC_DMA_COMMON_CFG  | lengthDescr0 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

            /* Configure DMA descriptor. */
            USBTMC_CyDmaSetConfiguration(channelNum, USBTMC_DMA_DESCR1, USBTMC_DMA_COMMON_CFG  | lengthDescr1 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);
            ...
            if (USBTMC_DmaEpBurstCntBackup[epNumber] > 1u)
            {
                USBTMC_CyDmaValidateDescriptor(channelNum, USBTMC_DMA_DESCR1);
            }
        ...
        #else
            (void) CyDmaChDisable(USBTMC_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBTMC_DmaTd[epNumber], length,  USBTMC_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBTMC_DmaTd[epNumber], LO16((uint32) &USBTMC_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32) pData));
...
/*******************************************************************************
* Function Name: USBTMC_LoadInEP16
****************************************************************************//**
*
*  This function performs different functionality depending on the Component’s
*  configured Endpoint Buffer Management. This parameter is defined in
...
void USBTMC_LoadInEP16(uint8 epNumber, const uint8 pData[], uint16 length)
{
    ...
    if ((epNumber > USBTMC_EP0) && (epNumber < USBTMC_MAX_EP))
    {
        ...
        if (NULL != pData)
        {
            ...
            {
                USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16 = dataBuf[i];
            }
        }
        ...
        USBTMC_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBTMC_EP[epNumber].epMode;
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                /* Configure source and destination. */
                USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR0, (void*) pData);
                USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            if (pData != NULL)
            {
                ...
                /* Set destination address. */
                USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR1, (void*) &USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    /* Adjust number of data elements transferred in last burst. */
                    USBTMC_DmaEpLastBurstEl[epNumber] = (0u != USBTMC_DmaEpLastBurstEl[epNumber]) ?
                                                                          ((USBTMC_DmaEpLastBurstEl[epNumber] >> 1u) - 1u) :
                                                                           (USBTMC_DMA_HALFWORDS_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBTMC_DmaEpBurstCnt[epNumber]) ? USBTMC_DmaEpLastBurstEl[epNumber] : (USBTMC_DMA_HALFWORDS_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBTMC_DmaEpBurstCnt[epNumber]) ? USBTMC_DmaEpLastBurstEl[epNumber] : (USBTMC_DMA_HALFWORDS_PER_BURST - 1u);
...
/*******************************************************************************
* Function Name: USBTMC_ReadOutEP16
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
*   configured Endpoint Buffer Management. This parameter is defined in the
...
uint16 USBTMC_ReadOutEP16(uint8 epNumber, uint8 pData[], uint16 length)
{
    ...
    adjLength =  length + ((uint32)length & 1u);
    ...
    #if (USBTMC_EP_MANAGEMENT_MANUAL)
        {
            ...
            {
                dataBuf[i] = (uint16) USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16;
            }
        }
        ...
        #if (USBTMC_EP_MANAGEMENT_DMA_MANUAL)
        {
            ...
            /* Configure source and destination. */
            USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBTMC_CyDmaSetDstAddress(channelNum, USBTMC_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBTMC_CyDmaSetConfiguration(channelNum, USBTMC_DMA_DESCR0, USBTMC_DMA_COMMON_CFG | (uint16)((adjLength >> 1u) - 1u) |
        ...
        #if (USBTMC_EP_MANAGEMENT_DMA_AUTO)
        {
            ...
            /* Get number of full bursts. */
            USBTMC_DmaEpBurstCnt[epNumber] = (uint8) (adjLength / USBTMC_DMA_BYTES_PER_BURST);

            /* Get number of elements in last burst. */
            USBTMC_DmaEpLastBurstEl[epNumber] = (uint8) (adjLength % USBTMC_DMA_BYTES_PER_BURST);
            ...
            USBTMC_DmaEpLastBurstEl[epNumber] |= USBTMC_DMA_DESCR_16BITS;
            ...
            /* Set destination address. */
            USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR0, (void*) &USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBTMC_CyDmaSetSrcAddress(channelNum, USBTMC_DMA_DESCR1, (void*) &USBTMC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
...
/*******************************************************************************
* Function Name: USBTMC_EnableOutEP
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBTMC_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* \reentrant
*  No.
*
...
/*******************************************************************************
* Function Name: USBTMC_DisableOutEP
****************************************************************************//**
*
...
*    State                      |Description
*    ---------------------------|----------------------------------------------
*    USBTMC_FORCE_J   | Force a J State onto the D+/D– lines
*    USBTMC_FORCE_K   | Force a K State onto the D+/D– lines
*    USBTMC_FORCE_SE0 | Force a Single Ended 0 onto the D+/D– lines
*    USBTMC_FORCE_NONE| Return bus to SIE control
*
*
*******************************************************************************/
void USBTMC_Force(uint8 bState) 
{
    ...
    */
    USBTMC_USBIO_CR0_REG = bState;
}


/*******************************************************************************
* Function Name: USBTMC_GetEPAckState
****************************************************************************//**
*
...
uint8 USBTMC_GetEPAckState(uint8 epNumber) 
{
    ...
    if ((epNumber > USBTMC_EP0) && (epNumber < USBTMC_MAX_EP))
    {
        cr = USBTMC_SIE_EP_BASE.sieEp[epNumber].epCr0 & USBTMC_MODE_ACKD;
    }
...
/*******************************************************************************
* Function Name: USBTMC_SetPowerStatus
****************************************************************************//**
*
...
*  --------------------------------------------|---------------------------
*  USBTMC_DEVICE_STATUS_BUS_POWERED  | Set the device to bus powered
*  USBTMC_DEVICE_STATUS_SELF_POWERED | Set the device to self powered
*
* \globalvars
*
*  \ref USBTMC_deviceStatus - set power status
*
* \reentrant
*  No.
*
...
{
    if (powerStatus != USBTMC_DEVICE_STATUS_BUS_POWERED)
    {
    ...
    /***************************************************************************
    * Function Name: USBTMC_VBusPresent
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return ((0u != (USBTMC_VBUS_STATUS_REG & USBTMC_VBUS_VALID)) ? (uint8) 1u : (uint8) 0u);
    }
#endif /* (USBTMC_VBUS_MONITORING_ENABLE) */


/*******************************************************************************
* Function Name: USBTMC_RWUEnabled
****************************************************************************//**
*
...
*
* \globalvars
*  USBTMC_deviceStatus - checked to determine remote status
*
*******************************************************************************/
uint8 USBTMC_RWUEnabled(void) 
{
    uint8 result = USBTMC_FALSE;

    if (0u != (USBTMC_deviceStatus & USBTMC_DEVICE_STATUS_REMOTE_WAKEUP))
    {
        result = USBTMC_TRUE;
    }
...
/*******************************************************************************
* Function Name: USBTMC_GetDeviceAddress
****************************************************************************//**
*
...
{
    return (uint8)(USBTMC_CR0_REG & USBTMC_CR0_DEVICE_ADDRESS_MASK);
}


/*******************************************************************************
* Function Name: USBTMC_EnableSofInt
****************************************************************************//**
*
...
void USBTMC_EnableSofInt(void) 
{
    ...
    USBTMC_INTR_SIE_MASK_REG |= (uint32) USBTMC_INTR_SIE_SOF_INTR;
#else
...
/*******************************************************************************
* Function Name: USBTMC_DisableSofInt
****************************************************************************//**
*
...
void USBTMC_DisableSofInt(void) 
{
    ...
    /***************************************************************************
    * Function Name: USBTMC_DetectPortType
    ************************************************************************//**
    *
    ...
    *   Return Value                      |Description
    *   ----------------------------------|-------------------------------------
    *   USBTMC_BCD_PORT_SDP     | Standard downstream port detected
    *   USBTMC_BCD_PORT_CDP     | Charging downstream port detected
    *   USBTMC_BCD_PORT_DCP     | Dedicated charging port detected
    *   USBTMC_BCD_PORT_UNKNOWN | Unable to detect charging port type (proprietary charger type)
    *   USBTMC_BCD_PORT_ERR     | Error condition in detection process
    *
    *
    * \sideeffects
    *
    ...
    ***************************************************************************/
    uint8 USBTMC_Bcd_DetectPortType(void)
    {
        ...
        uint8 result = USBTMC_BCD_PORT_UNKNOWN;
        ...
        */
        bkPwrCtrl = USBTMC_POWER_CTRL_REG;
        USBTMC_POWER_CTRL_REG = USBTMC_DEFAULT_POWER_CTRL_VBUS\
                            & (~USBTMC_POWER_CTRL_ENABLE_VBUS_PULLDOWN)\
                            & (~USBTMC_POWER_CTRL_ENABLE_DM_PULLDOWN);


        /* Enable PHY detector and single-ended and differential receivers.
         * Enable charger detection.  */
        USBTMC_POWER_CTRL_REG |= USBTMC_DEFAULT_POWER_CTRL_PHY\
                                         | USBTMC_POWER_CTRL_ENABLE_CHGDET;
        ...
        */
        #if defined (USBTMC_NO_DCD)
        CyDelay(USBTMC_BCD_TIMEOUT);
        #else
        /* DCD implementation:*/

        {
            uint16 timeout = USBTMC_BCD_TIMEOUT;
            uint8 connectionApproved = 0u;
            uint8 connected = 0u;

            /*   BCD spec 1.2: Turns on Idp_src and D- pull-down resistor */
            USBTMC_POWER_CTRL_REG |= USBTMC_POWER_CTRL_ENABLE_DM_PULLDOWN;
            USBTMC_CHGDET_CTRL_REG |= USBTMC_CHGDET_CTRL_DCD_SRC_EN;

            /* BCD spec 1.2: Waits for D+ to be low for a time of Tdcd_dbnc*/
            while ((0u != timeout) && (0u == connectionApproved))
            {
                if (0u == (USBTMC_USBIO_CR1_REG & USBTMC_USBIO_CR1_DP0))
                {
                    connected++;
                }
                else
                ...
                connectionApproved = (USBTMC_BCD_TDCD_DBNC < connected) ? 1u:0u;
                CyDelay(1u);
                timeout--;
            }
        ...
        USBTMC_CHGDET_CTRL_REG = USBTMC_CHGDET_CTRL_PRIMARY;
        CyDelay(USBTMC_BCD_PRIMARY_WAIT);
        cr1RegVal = USBTMC_USBIO_CR1_REG;

        /* Check is it SDP or DCP/CDP, read comparator 2 output. */
        if (0u == (USBTMC_CHGDET_CTRL_REG & USBTMC_CHGDET_CTRL_COMP_OUT))
        {
            /* Check status of D- line. */
            if (0u == (cr1RegVal & USBTMC_USBIO_CR1_DM0))
            {
                result = USBTMC_BCD_PORT_SDP;
            }
            else
            {
        ...
        {
            USBTMC_CHGDET_CTRL_REG = USBTMC_CHGDET_CTRL_DEFAULT \
                                                | USBTMC_CHGDET_CTRL_SECONDARY;
            CyDelay(USBTMC_BCD_SECONDARY_WAIT);
            cr1RegVal = USBTMC_USBIO_CR1_REG;
            ...
            {
                /* Check status of D+ line. */
                if (0u == (cr1RegVal & USBTMC_USBIO_CR1_DP0))
                {
                    result = USBTMC_BCD_PORT_CDP;
                }
                else
                {
                    /* ERROR: such combination is impossible. Abort charger
            ...
            else
            {
                ...
                else
                {
                    ...
                    */
                    result = USBTMC_BCD_PORT_UNKNOWN;
                }
            }
        }

        /* Restore CHGDET_CTRL. */
        USBTMC_CHGDET_CTRL_REG = 0u;

        /*Revert registers back*/
        USBTMC_POWER_CTRL_REG = bkPwrCtrl;
        USBTMC_USBIO_CR1_REG |= (uint32) USBTMC_USBIO_CR1_IOMODE;
        USBTMC_USB_CLK_EN_REG = ~USBTMC_USB_CLK_CSR_CLK_EN;
...
#if (USBTMC_LPM_ACTIVE)
    /***************************************************************************
    * Function Name: USBTMC_Lpm_GetBeslValue
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBTMC_LPM_STAT_REG & USBTMC_LPM_STAT_LPM_BESL_MASK);
    }


    /***************************************************************************
    * Function Name: USBTMC_Lpm_RemoteWakeUpAllowed
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBTMC_LPM_STAT_REG & USBTMC_LPM_STAT_LPM_REMOTE_WAKE);
    }


    /***************************************************************************
    * Function Name: USBTMC_Lpm_SetResponse
    ************************************************************************//**
    *
    ...
    *       - USBTMC_LPM_REQ_ACK - next LPM request will be
    *                                           responded with ACK
    *       - USBTMC_LPM_REQ_NACK - next LPM request will be
    *                                           responded with NACK
    *       - USBTMC_LPM_REQ_NYET - next LPM request will be
    *                                           responded with NYET
    *
    ***************************************************************************/
    void USBTMC_Lpm_SetResponse(uint32 response)
    {
        uint32 lpmCtrl = USBTMC_LPM_CTRL_REG & (uint32) ~USBTMC_LPM_CTRL_ACK_NYET_MASK;

        USBTMC_LPM_CTRL_REG = lpmCtrl | ((uint32) response & USBTMC_LPM_CTRL_ACK_NYET_MASK);
    }


    /***************************************************************************
    * Function Name: USBTMC_Lpm_GetResponse
    ************************************************************************//**
    *
    ...
    *       - USBTMC_LPM_REQ_ACK - next LPM request will be responded
    *                                        with ACK
    *       - USBTMC_LPM_REQ_NACK - next LPM request will be responded
    *                                        with NACK
    *       - USBTMC_LPM_REQ_NYET - next LPM request will be responded
    *                                        with NYET
    *
    ***************************************************************************/
    uint32 USBTMC_Lpm_GetResponse(void)
    {
        ...
        return  ((uint32) USBTMC_LPM_CTRL_REG & (uint32)USBTMC_LPM_CTRL_ACK_NYET_MASK);
    }
